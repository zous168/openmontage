"""Local video auto-fallback when no remote video provider is configured."""
...
def test_local_generation_explicit_enable(monkeypatch):
    ...
    monkeypatch.setattr(_shared, "local_generation_enabled", lambda: True)
    monkeypatch.setattr(_shared, "local_auto_fallback_active", lambda: False)
    assert _shared.local_generation_status() == ToolStatus.AVAILABLE


def test_local_auto_fallback_when_no_remote(monkeypatch):
    monkeypatch.setattr(_shared, "_local_stack_ready", lambda: True)
    ...
    assert _shared.local_auto_fallback_active() is True
    assert _shared.local_generation_status() == ToolStatus.AVAILABLE


def test_local_auto_fallback_skipped_when_remote_exists(monkeypatch):
    monkeypatch.setattr(_shared, "_local_stack_ready", lambda: True)
    monkeypatch.setattr(_shared, "local_generation_enabled", lambda: False)
    monkeypatch.setattr(_shared, "local_auto_fallback_enabled", lambda: True)
...
def test_video_selector_prefers_local_on_auto_fallback(monkeypatch):
    ...
    class _ScoreStub:
        def __init__(self, tool_name: str, provider: str, weighted: float):
    ...
    def fake_rank(candidates, task_context):  # noqa: ANN001
        return [_ScoreStub("ltx_video_local", "ltx", 0.85)]

    monkeypatch.setattr("lib.scoring.rank_providers", fake_rank)
    monkeypatch.setattr(_shared, "local_auto_fallback_active", lambda: True)
    ...
    assert tool.name == "ltx_video_local"
