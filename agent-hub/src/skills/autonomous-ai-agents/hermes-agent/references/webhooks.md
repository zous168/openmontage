...
Add to `~/.marketing_hub/config.yaml`:
```yaml
platforms:
  webhook:
    enabled: true
...
Add to `${HERMES_HOME:-~/.marketing_hub}/.env`:
```bash
...
- Subscriptions persist to `~/.marketing_hub/webhook_subscriptions.json`

## How It Works

1. `hermes webhook subscribe` writes to `~/.marketing_hub/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.marketing_hub/logs/gateway.log | tail -20`
