...
def _build_gateway_argv(
    *,
    gateway_parent_pid: int | None = None,
    ...
    # Hub lifecycle 显式传入 gateway_parent_pid 时必须走 hub-managed：
    # 开发态 Hub 自身常无 PARENT_PID，若只看 env 会误走 pythonw + hermes_cli.main，
    # 与 Hub 管道转发/监督契约不一致，易导致「默认自启实际没挂住」。
    hub_managed = gateway_parent_pid is not None or _hub_managed_spawn()

    if hub_managed:
        argv = [python_exe, "-m", "gateway.run"]
    else:
    ...
    if hub_managed:
        env_overlay["PARENT_PID"] = str(
            gateway_parent_pid if gateway_parent_pid is not None else os.getpid()
        )
    hub_data = os.environ.get("HUB_DATA_DIR", "").strip()
    if not hub_data:
        hub_data = str(get_default_hermes_root().resolve())
    env_overlay["HUB_DATA_DIR"] = hub_data
    profile = os.environ.get("HERMES_PROFILE", "").strip()
    if profile:
        env_overlay["HERMES_PROFILE"] = profile
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        logs = get_hub_logs_dir()
        print(f"    type {logs}\\gateway.log")
        print(f"    type {logs}\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {logs}\\gateway.log       # View logs")
...
def _hub_noninteractive_gateway_start() -> bool:
    """Marketing Hub 仅一体部署：gateway 直接 detached 拉起，不走 schtasks 安装交互."""
    return True


def _start_detached_or_fail() -> None:
    try:
        ...
        pass
    pid = _spawn_detached()
    log_gateway_trace("start.detached", pid=pid)
    if pid > 0:
        _report_gateway_start(f"direct spawn (PID {pid})")
        return
    print("✗ Failed to start gateway (detached spawn)")
    sys.exit(1)
...
def start() -> None:
    ...
    log_gateway_trace(
        ...
        startup_installed=is_startup_entry_installed(),
        hub_noninteractive=_hub_noninteractive_gateway_start(),
    )
    if running_pids:
        print(f"✓ Gateway already running (PID: {', '.join(map(str, running_pids))})")
        return
    ...
    if not task_installed and not startup_installed:
        if _hub_noninteractive_gateway_start():
            log_gateway_trace("start.path", mode="detached_no_service")
            _start_detached_or_fail()
            return
        from hermes_cli.setup import prompt_yes_no
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            ...
            log_gateway_trace("start.aborted", reason="user_declined_install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            log_gateway_trace("start.aborted", reason="install_incomplete")
            return

    # Always spawn via pythonw + detached flags — schtasks /Run executes
    # gateway.cmd through cmd.exe and flashes a console window on every start.
    _start_detached_or_fail()
...
def stop() -> None:
    ...
    log_gateway_trace(
        ...
        drained=drained if pid is not None else None,
        pid=pid,
    )
