...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.marketing_hub/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.marketing_hub/auth.json``), refreshing it if needed. Fails fast with a
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.marketing_hub/.env`` idempotently.
