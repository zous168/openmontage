...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    if path.startswith("/api/"):
        ...
        log_auth_denial(
            ...
            layer="gated",
            code=reason,
            message=f"gated auth denied ({reason})",
            hub_ipc=hub_ipc_auth_configured(request),
        )
...
async def gated_auth_middleware(
    ...
    # Hub 集成：IPC + 设备登录统一由 local_guard_middleware 承担。
    if hub_ipc_auth_configured(request):
        return await call_next(request)
    ...
    from hermes_cli.dashboard_auth.local_guard import is_hub_gated_public_path

    if is_hub_gated_public_path(request):
        return await call_next(request)

    # 集成：登录前 bootstrap 路径（dev IPC token、设备 login）须放行——
    # 这些是拿到 IPC token *之前* 的端点，由 LocalGuard 自身把关（localhost）。否则
    # 翻转为 Hermes 根后 hub 登录会被本网关死锁（LT-007.02·真点验发现）。
    _hub_public = getattr(request.app.state, "hub_public_prefixes", ())
    if _hub_public and any(
        path == p or path.startswith(p) for p in _hub_public
    ):
        return await call_next(request)
