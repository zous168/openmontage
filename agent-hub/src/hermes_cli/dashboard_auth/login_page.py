...
Visual styling mirrors the design system (the
...
# ---------------------------------------------------------------------------
# Marketing Hub integrated login (ai_worker + local IPC)
# ---------------------------------------------------------------------------


def _integrated_safe_next_path(raw: str) -> str:
    path = (raw or "").strip()
...
def dev_seed_login_defaults() -> tuple[str, str]:
    """兼容旧测试：读 ``HUB_DEV_SEED_*``。登录页预填已改走 ``login_prefs`` 凭证区。"""
    import os

    login = (os.environ.get("HUB_DEV_SEED_AI_WORKER_LOGIN") or "").strip()
    password = (os.environ.get("HUB_DEV_SEED_PASSWORD") or "").strip()
    return login, password


def render_integrated_login_html(
    *,
    next_path: str = "/",
    default_login_name: str = "",
    default_password: str = "",
) -> str:
    """Marketing Hub gated 模式登录页（AI员工 账号 + IPC token）."""
    from hermes_cli.dashboard_auth.local_guard import IPC_TOKEN_COOKIE

    safe_next = html.escape(_integrated_safe_next_path(next_path), quote=True)
    safe_login = html.escape(default_login_name, quote=True)
    safe_password = html.escape(default_password, quote=True)
    login_value = f' value="{safe_login}"' if default_login_name else ""
    password_value = f' value="{safe_password}"' if default_password else ""
    return f"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>登录 - Marketing Hub</title>
  <style>
    * {{ box-sizing: border-box; }}
    body {{
      margin: 0;
    ...
    button:hover {{ background: #4096ff; border-color: #4096ff; }}
...
<body>
  <div class="wrap">
    <div class="panel">
      <h1 class="brand">Marketing Hub</h1>
      <p class="hint">请使用 AI员工 账号登录</p>
      <div id="err" class="err" role="alert"></div>
      <form id="form">
        <label for="login_name">账号</label>
        <input id="login_name" name="login_name" autocomplete="username" required{login_value} />
        <label for="password">密码</label>
        <input id="password" name="password" type="password" autocomplete="current-password" required{password_value} />
  ...
  <script>
    ...
    const IPC_COOKIE = "{IPC_TOKEN_COOKIE}";
    ...
    async function ensureIpcToken() {{
      try {{
        const resp = await fetch("/api/auth/dev/local-ipc-token");
        if (resp.ok) {{
          const data = await resp.json();
          if (data.token) {{
            setCookie(IPC_COOKIE, data.token);
            return data.token;
          }}
        }}
      }} catch (_) {{}}
      return getCookie(IPC_COOKIE);
    }}
    ...
    document.getElementById("form").addEventListener("submit", async (ev) => {{
      ...
      const ipc = await ensureIpcToken();
      const headers = {{ "Content-Type": "application/json" }};
      if (ipc) headers["X-Hub-Local-Token"] = ipc;
      try {{
        const resp = await fetch("/api/auth/login", {{
          ...
          body: JSON.stringify({{
            ...
            // 与 client 共用本机凭证区（device/login_prefs.json）
        ...
        if (!resp.ok) {{
          ...
          const msg = (detail && detail.message) || data.message || ("HTTP " + resp.status);
          showError(msg);
          btn.disabled = false;
          return;
        }}
        if (ipc) setCookie(IPC_COOKIE, ipc);
