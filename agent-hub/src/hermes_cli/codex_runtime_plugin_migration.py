...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    ``~/.marketing_hub`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    agent_home = get_hermes_home()
    base_home = get_default_hermes_root()
    hermes_home = str(agent_home)
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["HUB_DATA_DIR"] = str(base_home)
        if agent_home != base_home and agent_home.parent.name == "profiles":
            env["HERMES_PROFILE"] = agent_home.name
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.marketing_hub/config.yaml dict
