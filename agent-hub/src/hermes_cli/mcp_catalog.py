...
- Secrets prompted at install time go to ``~/.marketing_hub/.env`` (the
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.marketing_hub/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.marketing_hub/.env via save_env_value()."""
