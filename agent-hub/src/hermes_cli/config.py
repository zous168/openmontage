"""
Configuration management for Hermes Agent / MxAI Hub.

配置落在当前 Agent 数据根（``HUB_DATA_DIR`` / 安装目录 ``data``，开发时多为
仓库 ``.data``；历史兜底名 ``~/.marketing_hub`` 已不再作为产品默认）：
- ``{数据根}/config.yaml``  - 模型、toolsets、终端等
- ``{数据根}/.env``         - API keys 与密钥
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.marketing_hub/config.yaml`` causes ``load_config()``
...
# save_config() / patch_config_yaml() call invalidate_config_caches()
# after writes — global edits clear all entries (profile caches bake in
# the global overlay at fill time); profile-only edits drop one path.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.marketing_hub/.install_method`` file (written by installers)
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.marketing_hub/.install_method."""
    stamp = get_hermes_home() / ".install_method"
    try:
...
from hermes_constants import get_config_path, get_env_path, get_hermes_home  # noqa: F811,E402
...
def ensure_hermes_home():
    """Ensure ~/.marketing_hub directory structure exists with secure permissions.
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Hermes self-help block + full framework guidance (task completion,
        # memory/skills/steering, skills index, profile hints, …). Default
        # True for assistant/default CLI. MxAI business Agent profiles use
        # False for lean customer-service prompts (SOUL + KB only).
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.marketing_hub/checkpoints/`` size (MB).  When
    ...
    "dashboard": {
        "theme": "lightgray",  # Dashboard visual theme (default: light gray)
        # Dashboard UI language (en, zh, zh-hant, ja, de, es, fr, …). Empty =
        # unset; the SPA falls back to browser language on first visit.
        "locale": "",
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.marketing_hub/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.marketing_hub/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        # Holographic provider: facts injected per turn via prefetch (before LLM call).
        "prefetch_limit": 5,
    ...
    # always goes to ~/.marketing_hub/skills/.
    "skills": {
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.marketing_hub/skills/ into
        # ~/.marketing_hub/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    # stored approval from ~/.marketing_hub/shell-hooks-allowlist.json.
    ...
    # Logging — controls file logging to ~/.marketing_hub/logs/.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.marketing_hub/.env,
        ...
        # cache (~/.marketing_hub/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.marketing_hub/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.marketing_hub/sessions/session_{sid}.json`` on every turn
    ...
    "onboarding": {
        ...
        # message ever. "ask" -> offer to build a user profile (opt-in,
        # consent-gated). "off" -> plain intro only.
        # MxAI 默认 off：避免个人微信/助理首触刷 Hermes 英文自我介绍与建档邀约。
    ...
    # rather than storing them in ~/.marketing_hub/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.marketing_hub/.env (or your shell) and never in config.yaml.
            ...
            # ~/.marketing_hub/bin/ on first use.  When False you must install
...
def _dotenv_file_has_key(key: str) -> bool:
    """Return True if ``key`` is explicitly set in ``HERMES_HOME/.env``."""
    try:
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines: list[str] = []
    if messaging_cwd and _dotenv_file_has_key("MESSAGING_CWD"):
        lines.append(
    ...
    if (
        ...
        and _dotenv_file_has_key("TERMINAL_CWD")
    ):
        lines.append(
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    #   3. Creates `~/.marketing_hub/logs/curator/` if missing (belt-and-suspenders
    ...
    if current_ver < 23:
        try:
            ...
            curator_dir = get_hub_logs_dir() / "curator"
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.marketing_hub/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.marketing_hub/config.yaml.
...
def _integrated_global_config_overlay() -> Dict[str, Any]:
    """一体部署命名 profile：返回全局 ``config.yaml`` 作为 profile 配置的基底。

    Profile 自有 ``profiles/<name>/config.yaml`` 在其上覆盖；文件不存在时仍继承全局 model 等。
    """
    from hermes_constants import _agent_base_dir, get_hermes_home

    base = _agent_base_dir()
    home = get_hermes_home()
    try:
        if home.resolve() == base.resolve():
            return {}
    except OSError:
        if home == base:
            return {}
    global_path = base / "config.yaml"
    ...
    try:
        ...
        _warn_config_parse_failure(global_path, e)
        return {}
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        global_overlay = _integrated_global_config_overlay()
        if global_overlay:
            config = _deep_merge(config, global_overlay)
        ...
        try:
            ...
            from hermes_constants import _active_profile_name, get_hermes_home

            merged = expanded.get("model") or {}
            raw = user_config.get("model") if isinstance(user_config, dict) else None
            raw_default = (
                raw.get("default") if isinstance(raw, dict) else (raw if isinstance(raw, str) else "")
            )
            log_model_trace(
                "load_config",
                ...
                global_overlay=bool(global_overlay),
                raw_profile_model=raw_default,
                merged_model=merged.get("default") if isinstance(merged, dict) else merged,
                merged_provider=merged.get("provider") if isinstance(merged, dict) else "",
                cache_hit=False,
            )
        except Exception:
            pass
...
def is_integrated_named_profile_config() -> bool:
    """是否正在写入 ``profiles/<name>/config.yaml``（非全局根）。"""
    from hermes_constants import _agent_base_dir, get_hermes_home

    base = _agent_base_dir()
    home = get_hermes_home()
    try:
        return home.resolve() != base.resolve()
    except OSError:
        return home != base


def _is_global_config_path(config_path: Path) -> bool:
    """Return True when *config_path* is the hub-wide ``config.yaml``."""
    from hermes_constants import _agent_base_dir

    global_path = _agent_base_dir() / "config.yaml"
    try:
        return config_path.resolve() == global_path.resolve()
    except OSError:
        return config_path == global_path


def invalidate_config_caches(*, global_change: bool = False, path_key: str | None = None) -> None:
    """Drop process-level config caches after a write.

    Profile ``load_config()`` bakes in the global overlay at cache-fill time.
    A global ``config.yaml`` edit does not change profile file mtimes, so
    ``global_change=True`` clears every cached entry. Profile-only writes
    drop the affected path only.
    """
    with _CONFIG_LOCK:
        if global_change:
...
def invalidate_llm_derived_state() -> None:
    """Clear process-local LLM routing state that outlives ``load_config()`` cache.

    ``call_llm(provider=auto)`` keeps OpenAI clients + default models in
    ``auxiliary_client._client_cache``. Gateway/turn code may also pin a
    runtime model via ``set_runtime_main()``. Both must be dropped whenever
    model config or credentials change.
    """
    try:
        from agent.auxiliary_client import clear_runtime_main, shutdown_cached_clients
...
def _config_updates_touch_llm(updates: Dict[str, Any]) -> bool:
    if not updates:
        return False
    if "model" in updates or "auxiliary" in updates:
        return True
    providers = updates.get("providers")
    if isinstance(providers, dict) and providers:
        return True
    custom = updates.get("custom_providers")
    return isinstance(custom, (dict, list)) and bool(custom)


def env_key_touches_llm_runtime(key: str) -> bool:
    """Return True when an env var affects LLM provider routing or credentials."""
    name = (key or "").strip().upper()
    if not name:
        return False
    info = OPTIONAL_ENV_VARS.get(name)
    if isinstance(info, dict) and str(info.get("category") or "").strip().lower() == "provider":
        return True
    return name in {
        "OPENAI_API_KEY",
        "OPENAI_BASE_URL",
...
def patch_config_yaml(updates: Dict[str, Any]) -> None:
    """Merge *updates* into the on-disk YAML at ``get_config_path()`` only.

    Used for Hub integrated profile-scoped partial writes (e.g. ``model`` only).
    Does not expand defaults or copy the global config into the profile file.
    """
    if not updates:
        return
    with _CONFIG_LOCK:
        ...
        merged = _deep_merge(copy.deepcopy(existing), updates)
        merged = _normalize_root_model_keys(_normalize_max_turns_config(merged))
        atomic_yaml_write(config_path, merged)
        _secure_file(config_path)
        invalidate_config_caches(path_key=path_key)
        if _config_updates_touch_llm(updates):
            invalidate_llm_derived_state()
        try:
            from hermes_cli.model_trace import log_model_trace
            from hermes_constants import _active_profile_name

            m = merged.get("model") or {}
            log_model_trace(
                "patch_config_yaml",
                profile=_active_profile_name() or "default",
                config_path=str(config_path),
                merged_model=m.get("default") if isinstance(m, dict) else m,
                merged_provider=m.get("provider") if isinstance(m, dict) else "",
                update_keys=",".join(sorted(updates.keys())),
            )
        except Exception:
            pass
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.marketing_hub/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
            managed_error("save configuration")
            return
        ...
        if _is_global_config_path(config_path):
            ...
            invalidate_config_caches(path_key=path_key)
        if _config_updates_touch_llm(normalized):
            invalidate_llm_derived_state()


def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.marketing_hub/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.marketing_hub/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.marketing_hub/.env."""
    if is_managed():
    ...
    invalidate_env_cache()
    if env_key_touches_llm_runtime(key):
        invalidate_llm_derived_state()


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.marketing_hub/.env and os.environ.
    ...
    os.environ.pop(key, None)
    invalidate_env_cache()
    if found and env_key_touches_llm_runtime(key):
        invalidate_llm_derived_state()
    return found
...
def reload_env() -> int:
    """把 ``HERMES_HOME/.env``（本仓为 ``.data/.env``）合并进 ``os.environ``。

    该文件只作**补充层**：只对文件中出现的键做增/改，**绝不**因文件里没有某键
    就从进程环境删除它。入口注入（如 ``.env.dev`` / 启动脚本）才是基线真源；
    删掉「known 但不在 .env」会误伤入口已设的 ``API_SERVER_*`` 等（见
    Gateway spawn 前 ``reload_env`` 触发的回归）。

    若需显式清除某键，走 ``save_env_value`` / 删除 API（会主动 ``pop``），
    不要靠「文件缺席」隐含删除。
    """
    env_vars = load_env()
    count = 0
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.marketing_hub/.env or environment."""
