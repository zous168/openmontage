...
def _is_gateway_management_cli(command_lc: str) -> bool:
    """True when ``command_lc`` is a gateway *CLI* action, not ``gateway run``.

    ``hermes gateway restart`` (and stop/start/status/…) shares the
    ``hermes_cli.main gateway`` argv prefix with the long-running server.
    Process-table scans must not treat those short-lived management
    commands as live gateway PIDs — otherwise ``gateway restart`` kills
    itself mid-flight (seen on Windows as exit code 15).
    """
    return any(
        f"gateway {sub}" in command_lc
        for sub in (
            "restart",
            "stop",
            "start",
...
def _command_matches_gateway_scan(command_lc: str, patterns: tuple[str, ...]) -> bool:
    if _is_gateway_management_cli(command_lc):
        return False
    return any(pattern in command_lc for pattern in patterns)
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = (
        ...
        "hermes gateway run",
        # Windows: only match invocations that actually carry the ``gateway run``
        ...
        if is_windows():
            ...
            for line in result.stdout.split("\n"):
                ...
                if line.startswith("CommandLine="):
                    ...
                    if _command_matches_gateway_scan(current_cmd_lc, patterns) and (
                ...
                try:
                    ...
                    for entry in os.listdir("/proc"):
                        ...
                        try:
                            ...
                            if _command_matches_gateway_scan(cmdline_lc, patterns) and (
            ...
            if not _found_via_proc:
                ...
                for line in result.stdout.split("\n"):
                    ...
                    if pid is None:
                        continue
                    command_lc = command.lower()
                    if _command_matches_gateway_scan(command_lc, patterns) and (
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its profile/data path from unit env."""
    if not system:
        return
    ...
    if not unit_home:
        return
    unit_path = Path(unit_home)
    if unit_path.parent.name == "profiles":
        os.environ["HERMES_PROFILE"] = unit_path.name
        return
    from hermes_constants import set_hermes_home_override

    set_hermes_home_override(unit_path)
...
def _profile_arg(hermes_home: str | None = None) -> str:
    ...
    For ``~/.marketing_hub/profiles/<name>``, returns ``"--profile <name>"``.
...
def get_service_name() -> str:
    ...
    Default ``~/.marketing_hub`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.marketing_hub/profiles/coder`` returns ``hermes-gateway-coder``.
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.marketing_hub`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.marketing_hub/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.marketing_hub/hermes-agent  -> /home/alice/.marketing_hub/hermes-agent
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.marketing_hub                    → /home/alice/.marketing_hub
      /root/.marketing_hub/profiles/coder     → /home/alice/.marketing_hub/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    from hermes_constants import (
        MARKETING_HUB_DATA_DIRNAME,
    ...
    target_default = Path(target_home_dir) / MARKETING_HUB_DATA_DIRNAME

    # Default ~/.marketing_hub → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.marketing_hub → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.marketing_hub) — keep as-is
...
def _hub_managed_spawn() -> bool:
    """Hub 组合根拉起 Gateway 时走 ``gateway.run``，不经 ``hermes_cli.main`` CLI 栈."""
    return bool(os.environ.get("PARENT_PID", "").strip())


def _gateway_run_command() -> list[str]:
    """Build gateway subprocess argv.

    Hub-managed (MxAI sidecar Hub): ``python -m gateway.run``（唯一性由 pid 守卫 + autostart 保证）.
    Standalone Hermes CLI: ``python -m hermes_cli.main [--profile X] gateway run``.
    """
    python = get_python_path()
    if _hub_managed_spawn():
        return [python, "-m", "gateway.run"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup hermes gateway run`
    ...
    if is_windows():
        try:
            ...
            python_exe, venv_dir, extra_site = _resolve_detached_python(cmd[0])
            cmd[0] = python_exe
            env = os.environ.copy()
            ...
            if _hub_managed_spawn():
                ...
                env.update(gateway_spawn_env())
            env["VIRTUAL_ENV"] = str(venv_dir)
            if extra_site:
                existing = env.get("PYTHONPATH", "")
                parts = [p for p in [extra_site[0], existing] if p]
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup hermes gateway run "
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        ...
        log_file = get_hub_logs_dir() / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False):
    ...
    success = False
    try:
        success = asyncio.run(start_gateway(replace=False, verbosity=verbosity))
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.marketing_hub/plugins/ still require
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.marketing_hub/.env."
    )
    print_info(
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.marketing_hub/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.marketing_hub/.env: {', '.join(required)}")
    else:
        print_info(
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      2. ``HERMES_S6_SUPERVISED_CHILD`` is exported by
         ...
         run`` as a longrun, this guard short-circuits the
...
def _gateway_command_inner(args):
    ...
    if subcmd is None or subcmd == "run":
        ...
        run_gateway(verbose, quiet=quiet)
        return
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.marketing_hub/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
        ...
        log_gateway_trace(
            "cli.restart",
            platform=sys.platform,
            noninteractive=os.environ.get("HERMES_NONINTERACTIVE", ""),
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.marketing_hub/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
