...
under ``{data_root}/logs/`` (see ``get_hub_logs_dir()``).
...
from hermes_constants import display_hermes_home, get_default_hermes_root, get_hub_logs_dir
...
def tail_log(
    ...
    *,
    ...
    log_path = get_hub_logs_dir() / filename
    ...
    if follow:
        print(f"--- {get_default_hermes_root()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {get_default_hermes_root()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {get_default_hermes_root()}/logs/")
        return

    print(f"Log files in {get_default_hermes_root()}/logs/:\n")
    found = False
