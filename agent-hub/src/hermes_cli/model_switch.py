...
def list_authenticated_providers(
    ...
    *,
    ...
    # --- 0. 系统内置模型（platform LLM 网关）---
    # 凭据为设备 JWT，不走 API Key / OAuth 探测；就绪时置顶插入。
    try:
        from hermes_cli.official_gateway_models import (
            OFFICIAL_PROVIDER_LABEL,
            OFFICIAL_PROVIDER_SLUG,
            fetch_official_gateway_models,
            is_official_channel_ready,
        )

        if is_official_channel_ready() and OFFICIAL_PROVIDER_SLUG.lower() not in seen_slugs:
            official_models = fetch_official_gateway_models()
            if not official_models and current_provider.strip().lower() == OFFICIAL_PROVIDER_SLUG:
                # 网关暂时不可达时，仍展示当前已选模型，避免 picker 空白
                if current_model:
                    official_models = [current_model]
            results.append({
                "slug": OFFICIAL_PROVIDER_SLUG,
                "name": OFFICIAL_PROVIDER_LABEL,
                "is_current": current_provider.strip().lower() == OFFICIAL_PROVIDER_SLUG,
                "is_user_defined": False,
                "models": official_models[:max_models],
                "total_models": len(official_models),
                "source": "official-gateway",
                "auth_type": "device_jwt",
                "authenticated": True,
            })
            seen_slugs.add(OFFICIAL_PROVIDER_SLUG.lower())
            try:
                from hermes_cli.official_gateway_models import official_gateway_base_url

                _norm = _norm_url(official_gateway_base_url())
                if _norm:
                    _builtin_endpoints.add(_norm)
            except Exception:
                pass
    except Exception as exc:
        logger.debug("official provider listing skipped: %s", exc)
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Anthropic：仅认 Hermes 自有 OAuth / API Key / credential pool。
        # 不探测 ~/.claude（Claude Code）——产品侧避免本机 Code 登录把
        # Anthropic 自动塞进「设置主模型」选择器。
        if not has_creds and hermes_slug == "anthropic":
            try:
                ...
                logger.debug("Anthropic hermes oauth check failed: %s", exc)
        if not has_creds:
            continue
    ...
    for _cp in _canon_provs:
        ...
        # 系统内置：设备 JWT + 网关 URL（section 0 已处理；此处兜底）
        if not _cp_has_creds and _cp_config and getattr(_cp_config, "auth_type", "") == "device_jwt":
            try:
                from hermes_cli.official_gateway_models import is_official_channel_ready

                _cp_has_creds = is_official_channel_ready()
            except Exception:
                _cp_has_creds = False
        ...
        elif _cp_config and getattr(_cp_config, "auth_type", "") == "device_jwt":
            try:
                from hermes_cli.official_gateway_models import fetch_official_gateway_models

                _cp_model_ids = fetch_official_gateway_models()
            except Exception:
                _cp_model_ids = []
        else:
    ...
    # Sort: 系统内置置顶；其余当前 provider 优先，再按模型数降序
    results.sort(
        key=lambda r: (
            0 if str(r.get("slug") or "").lower() == "official" else 1,
            not r["is_current"],
            -r["total_models"],
        )
    )
