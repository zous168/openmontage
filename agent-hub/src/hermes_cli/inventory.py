...
def build_models_payload(
    ...
    *,
    ...
    from hermes_cli.models import PICKER_EXCLUDED_PROVIDERS
    ...
    # 产品侧隐藏若干提供商（Google AI Studio / xAI / Novita / NVIDIA 等）
    rows = [
        r
        for r in rows
        if str(r.get("slug") or "").strip().lower() not in PICKER_EXCLUDED_PROVIDERS
    ]
    if picker_hints:
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        # 已带 authenticated 的行（如官方渠道）仍补全 auth_type，供 GUI 跳过 API Key
        if "authenticated" in row:
            if not row.get("auth_type"):
                cfg = PROVIDER_REGISTRY.get(row["slug"])
                if cfg:
                    row["auth_type"] = cfg.auth_type
            continue
        ...
        row["key_env"] = key_env
        if auth_type == "device_jwt":
            row["warning"] = "请先登录设备账号以使用系统内置模型"
        elif auth_type == "api_key" and key_env:
            row["warning"] = f"paste {key_env} to activate"
        else:
            row["warning"] = (
                f"go to Settings → Main Model to configure ({auth_type})"
            )
