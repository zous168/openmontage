...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.marketing_hub/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.marketing_hub/pastes/pending.json``.
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.marketing_hub/pastes/pending.json``
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    return (get_hub_logs_dir() / filename) if filename else None
