...
Marketing Hub (integrated checkout)
----------------------------------
When Hermes CLI is run from the marketing-hub ``backend/src`` tree, the
dashboard is **not** started via ``hermes dashboard`` (default port 9119).
Use the Hub combo root instead::

    backend/scripts/start_dev.cmd   # → http://127.0.0.1:8000  (src.main:app)

That process mounts Hermes at ``/`` with ai_worker login, IPC gate,
and unified ``/api/auth/*``. A standalone ``hermes dashboard`` on :9119 is a
separate process without those integrations — do not use it for Hub dev.
Stop orphans with ``hermes dashboard --stop``.
"""
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        hub_data = os.environ.get("HUB_DATA_DIR", "").strip()
        profile = os.environ.get("HERMES_PROFILE", "").strip().lower()
        base = hub_data if hub_data else str(get_default_hermes_root())
        if hub_data:
            cfg_path = os.path.join(base, "config.yaml")
        elif profile and profile != "default":
            cfg_path = os.path.join(base, "profiles", profile, "config.yaml")
        else:
            cfg_path = os.path.join(base, "config.yaml")
...
HERMES_CLI_DIR = Path(__file__).parent.resolve()
WEB_SRC_DIR = HERMES_CLI_DIR / "web_src"
WEB_DIST_DIR = HERMES_CLI_DIR / "web_dist"
sys.path.insert(0, str(PROJECT_ROOT))


def _is_marketing_hub_integrated_checkout() -> bool:
    """True when CLI runs inside marketing-hub ``backend/src`` (Hub combo root present)."""
    return (PROJECT_ROOT / "main.py").is_file() and (
        PROJECT_ROOT / "hub" / "bootstrap.py"
    ).is_file()


def _refuse_standalone_dashboard_in_marketing_hub() -> None:
    """Block ``hermes dashboard`` in marketing-hub; point to ``start_dev.cmd``."""
    if not _is_marketing_hub_integrated_checkout():
        return
    if os.environ.get("HERMES_ALLOW_STANDALONE_DASHBOARD", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
        ...
        print(
            "⚠ HERMES_ALLOW_STANDALONE_DASHBOARD is set — starting standalone "
            "dashboard anyway. Hub dev should normally use :8000 "
            "(backend/scripts/start_dev.cmd).",
            file=sys.stderr,
        )
        return
    print(
        "Refusing to start standalone Hermes dashboard in Marketing Hub checkout.\n"
        "\n"
        "  Use the Hub combo root (Hermes is mounted at /):\n"
        "    backend/scripts/start_dev.cmd  →  http://127.0.0.1:8000\n"
        "    entry: src.main:app\n"
        "\n"
        "  ``hermes dashboard`` on :9119 is a separate process — no Hub IPC,\n"
        "  no ai_worker login page, no integrated device auth APIs.\n"
        "\n"
        "  Stop orphaned :9119:  hermes dashboard --stop\n"
        "\n"
        "  Rare override: HERMES_ALLOW_STANDALONE_DASHBOARD=1\n",
        file=sys.stderr,
    )
    sys.exit(2)
...
#
# Many modules cache agent data paths at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set HERMES_PROFILE so that
# every subsequent ``get_hermes_home()`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.marketing_hub/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # 1.5 若已设 HERMES_PROFILE 且无显式 -p，沿用当前 profile
    if profile_name is None and os.environ.get("HERMES_PROFILE", "").strip():
        return
    ...
    # 3. If we found a profile, set HERMES_PROFILE (not legacy HERMES_HOME)
    if profile_name is not None:
        try:
            from hermes_cli.profiles import normalize_profile_name, resolve_profile_env

            resolve_profile_env(profile_name)  # validate + ensure exists
...
# Load .env from ~/.marketing_hub/.env first, then project root as dev fallback.
...
def _ensure_tui_node() -> None:
    ...
    """
    from hermes_cli.dep_ensure import resolve_node_executable
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HUB_DATA_DIR": os.environ.get("HUB_DATA_DIR", hermes_home)},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _find_bundled_tui(hermes_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel or frozen install."""
    if hermes_cli_dir is None:
        hermes_cli_dir = Path(__file__).parent
    candidates = [hermes_cli_dir / "tui_dist" / "entry.js"]
    if getattr(sys, "frozen", False):
        meipass = getattr(sys, "_MEIPASS", None)
        if meipass:
            candidates.append(Path(meipass) / "hermes_cli" / "tui_dist" / "entry.js")
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if bin == "node":
            ...
            path = resolve_node_executable()
            if not path and not os.environ.get("HERMES_SKIP_NODE_BOOTSTRAP"):
                try:
        ...
        if not path:
            if getattr(sys, "frozen", False):
                print(
                    "node not found — install package should include node/node.exe next to agent-hub.exe.",
                    file=sys.stderr,
                )
            else:
    ...
    if not tui_dev:
        if ext_dir:
            p = Path(ext_dir)
            bundled_entry = p / "entry.js"
            if not bundled_entry.is_file():
                bundled_entry = p / "dist" / "entry.js"
            if bundled_entry.is_file():
                node = _node_bin("node")
                return [node, "--expose-gc", str(bundled_entry)], bundled_entry.parent

        # 1b. Bundled in wheel / PyInstaller (pip install / agent-hub.exe)
        ...
        if not tui_dir.is_dir():
            raise OSError(
                "TUI bundle missing: install package must ship hermes_cli/tui_dist/entry.js "
                f"or provide ui-tui sources (expected {tui_dir}). Rebuild agent-hub.exe."
            )
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.marketing_hub/skills/`` with the bundled skill library on first launch.
...
def cmd_chat(args):
    ...
    # user's ~/.marketing_hub/config.yaml and return built-in defaults. Set BEFORE
...
def select_provider_and_model(args=None):
    ...
    # a leftover OPENAI_BASE_URL in ~/.marketing_hub/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.marketing_hub/.env if the active provider is not 'custom'.
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.marketing_hub/.env`` by hand.
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Resolve built SPA output for *web_dir* (marketing-hub: ``hermes_cli/web_src``)."""
    if web_dir.parent.name == "hermes_cli":
        return web_dir.parent / "web_dist"
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The dashboard source lives under ``hermes_cli/web_src/`` (marketing-hub)
    or legacy ``web/``; Vite outputs to ``hermes_cli/web_dist/``.
    Uses the Vite manifest as the sentinel because it is written last and therefore
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.marketing_hub/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.marketing_hub/logs/update.log`` and to silently absorb
...
def _run_pre_update_backup(args) -> None:
    ...
    # Render path using display_hermes_home so the user sees ~/.marketing_hub/...
    try:
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # current — so copy it straight over ~/.marketing_hub/cache/model_catalog.json
    ...
    if action == "list":
        ...
        for p in profiles:
            ...
            if p.distribution_name:
                ...
                dist = "—"
            print(f"{marker}{name:<15} {model:<28} {alias:<12} {dist}")
        print()
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.marketing_hub)")
            else:
...
def cmd_dashboard(args):
    ...
    _refuse_standalone_dashboard_in_marketing_hub()
    ...
    if (
        ...
        env = os.environ.copy()
        # Drop profile scoping so the child binds the machine root dashboard.
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        if not _build_web_ui(WEB_SRC_DIR, fatal=True):
            sys.exit(1)
...
def main():
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.marketing_hub/.env.  Currently supports "
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.marketing_hub/checkpoints/",
