...
- Keep data: Remove code but keep ~/.marketing_hub/ (configs, sessions, logs)
"""
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.marketing_hub/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.marketing_hub/ for future reinstall
    """
    ...
    # full wipe (code + ~/.marketing_hub data); otherwise keep-data. Named profiles
    ...
    print(
        f"  Data:    {hermes_home / 'cron/'}, {hermes_home / 'sessions/'}, "
        f"{get_hub_logs_dir()}/"
    )
    print()
    ...
    *,
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.marketing_hub/, just remove the hermes-agent subdir
    ...
    # 5. Optionally remove ~/.marketing_hub/ data directory (and named profiles)
    if full_uninstall:
