...
from hermes_constants import display_hermes_env_path, display_hermes_home

PROJECT_ROOT = get_project_root()
HERMES_HOME = get_hermes_home()
_DHH = display_hermes_home()  # 用户可见数据根（HUB_DATA_DIR / 安装 data）
_DENV = display_hermes_env_path()

# Load environment variables from {数据根}/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when {数据根}/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # Check ~/.marketing_hub/.env (primary location for user config)
    ...
    # Check ~/.marketing_hub/config.yaml (primary) or project cli-config.yaml (fallback)
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            f"(check {_DENV} or run 'hermes setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'hermes setup' or set the provider's API key in {_DENV}, "
    ...
    _section("Platform Official Channel")
    try:
        from core.platform.gateway_base_url import resolve_llm_gateway_base_url

        cs_base = (os.getenv("CONTROL_SERVER_BASE_URL") or "").strip().rstrip("/")
        gw_explicit = (os.getenv("LLM_GATEWAY_BASE_URL") or "").strip().rstrip("/")
        gw_effective = resolve_llm_gateway_base_url()
        if cs_base:
            check_ok("CONTROL_SERVER_BASE_URL", cs_base)
        else:
            check_warn("CONTROL_SERVER_BASE_URL", "(unset — device login disabled)")
            issues.append("Set CONTROL_SERVER_BASE_URL for device login / platform credits")
        if gw_explicit:
            check_ok("LLM_GATEWAY_BASE_URL", gw_explicit)
        elif gw_effective:
            check_info(
                "LLM_GATEWAY_BASE_URL",
                f"(unset — using CONTROL_SERVER_BASE_URL → {gw_effective})",
            )
        else:
            check_warn(
                "LLM gateway",
                "(unset — set CONTROL_SERVER_BASE_URL or LLM_GATEWAY_BASE_URL)",
            )
            issues.append(
                "Set CONTROL_SERVER_BASE_URL (platform edge) or LLM_GATEWAY_BASE_URL for official metering"
            )
        ...
        try:
            ...
            device_auth = LocalDeviceAuthStore().load()
        except Exception as e:
            check_warn("Device login", f"(could not read session: {e})")
        if device_auth and (device_auth.access_token or "").strip():
            bal = float(getattr(device_auth, "credit_balance", 0) or 0)
            check_ok(
                "Device login",
                f"({device_auth.login_name or 'ok'}, credits={bal:g})",
            )
        else:
            check_warn("Device login", "(not logged in — official channel needs JWT)")
            if gw_effective:
                issues.append("Log in via dashboard /api/auth/login to enable official channel")

        if gw_effective and device_auth and (device_auth.access_token or "").strip():
            check_ok("Official channel ready", "(gateway URL + device JWT)")
        elif gw_effective:
            check_warn("Official channel ready", "(gateway set, waiting for device login)")
        else:
            check_info("Official channel inactive until platform base URL + login")
    except Exception as e:
        check_warn("Platform official channel", f"(could not check: {e})")
