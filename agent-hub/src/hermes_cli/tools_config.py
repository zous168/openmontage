...
Saves per-platform tool configuration to ~/.marketing_hub/config.yaml under
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("skills_view",     "📖 Skills (view only)",        "skill_view — load skill docs read-only"),
    ...
    ("mxai",            "📦 MxAI Marketing",            "queue, reports, risk — per-tool auth in Skills"),
...
# Legacy composite keys expanded into per-tool CONFIGURABLE_TOOLSETS entries.
# Must not survive in ``platform_toolsets`` once the user toggles granular keys —
# otherwise ``_get_platform_tools`` subset-inference re-enables the whole bundle.
MXAI_COMPOSITE_TOOLSET = "mxai"
LEGACY_COMPOSITE_TOOLSETS = frozenset({MXAI_COMPOSITE_TOOLSET})


def _mxai_per_tool_toolset_keys() -> frozenset[str]:
    try:
        from plugins.mxai.mcp_tools import MXAI_PER_TOOL_TOOLSETS

        return frozenset(MXAI_PER_TOOL_TOOLSETS)
    except Exception:
        return frozenset()


def _configurable_toolset_keys() -> Set[str]:
    return {ts_key for ts_key, _, _ in _get_effective_configurable_toolsets()}


def _dashboard_configurable_toolset_keys() -> Set[str]:
    """Keys the Dashboard may toggle — UI toolsets plus per-tool mxai keys."""
    return _configurable_toolset_keys() | _mxai_per_tool_toolset_keys()


def _mxai_tool_enabled_states(
    config: dict,
    platform: str,
    enabled_toolsets: Set[str],
    tools: List[str],
) -> Dict[str, bool]:
    mxai_granular = _mxai_per_tool_toolset_keys()
    raw = {
        str(x)
        for x in (config.get("platform_toolsets") or {}).get(platform) or []
    }
    composite = MXAI_COMPOSITE_TOOLSET in raw
    granular_in_raw = raw & mxai_granular

    if composite and not granular_in_raw:
        return {tool: True for tool in tools}
    if granular_in_raw:
        return {tool: tool in raw for tool in tools}

    # No explicit mxai in profile — only per-tool keys inferred at runtime count.
    inferred = enabled_toolsets & mxai_granular
    return {tool: tool in inferred for tool in tools}


def _mxai_bundle_all_enabled(
    config: dict,
    platform: str,
    enabled_toolsets: Set[str],
    tools: List[str],
) -> bool:
    states = _mxai_tool_enabled_states(config, platform, enabled_toolsets, tools)
    return bool(tools) and all(states.get(t, False) for t in tools)


def _normalize_mxai_explicit_for_save(explicit: Set[str]) -> Set[str]:
    """Profile 存整包 ``mxai`` 或单工具 key，不同时保留复合名与展开项。"""
    mxai_granular = _mxai_per_tool_toolset_keys()
    out = set(explicit)
    if MXAI_COMPOSITE_TOOLSET in out:
        out -= mxai_granular
    elif out & mxai_granular:
        out.discard(MXAI_COMPOSITE_TOOLSET)
    return out
    ...
    try:
        ...
        for entry in get_plugin_toolsets():
            if entry[0] in seen:
                continue
            if (
                entry[0] in _mxai_per_tool_toolset_keys()
                and MXAI_COMPOSITE_TOOLSET in seen
            ):
                continue
            seen.add(entry[0])
            result.append(entry)
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.marketing_hub/config.yaml")
        ...
        # to ~/.marketing_hub/.env), then continues straight into PKCE. If they
...
def _get_platform_tools(
    ...
    *,
    ...
    configurable_keys = _configurable_toolset_keys()
    plugin_ts_keys = _get_plugin_toolset_keys()
    platform_default_keys = {p["default_toolset"] for p in PLATFORMS.values()}

    mxai_granular = _mxai_per_tool_toolset_keys()
    if mxai_granular and any(ts in toolset_names for ts in mxai_granular):
        toolset_names = [
            ts for ts in toolset_names if ts not in LEGACY_COMPOSITE_TOOLSETS
        ]
    ...
    if has_explicit_config:
        ...
        if composite_tools:
            expanded = set()
            mxai_granular_active = mxai_granular and any(
                ts in toolset_names for ts in mxai_granular
            )
            for ts_key, _, _ in _get_effective_configurable_toolsets():
                if not _toolset_allowed_for_platform(ts_key, platform):
                    continue
                if mxai_granular_active and ts_key in mxai_granular:
                    continue
    ...
    skip |= LEGACY_COMPOSITE_TOOLSETS
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            ...
            elif pts in _mxai_per_tool_toolset_keys():
                # Per-tool mxai — explicit ``platform_toolsets`` only (or composite mxai).
                continue
            elif pts in configurable_keys and pts not in _DEFAULT_OFF_TOOLSETS:
                # Built-in configurable toolset (e.g. mxai per-tool keys) —
                # only direct ``platform_toolsets`` membership / composite
                # inference above; never auto-on via the plugin-default path.
                continue
    ...
    if disabled_toolsets:
        ...
        if MXAI_COMPOSITE_TOOLSET in disabled_set:
            enabled_toolsets -= _mxai_per_tool_toolset_keys()

    mxai_granular = _mxai_per_tool_toolset_keys()
    if MXAI_COMPOSITE_TOOLSET in enabled_toolsets and mxai_granular:
        enabled_toolsets |= mxai_granular

    return enabled_toolsets


def toggle_dashboard_platform_toolset(
    config: dict,
    platform: str,
    name: str,
    enabled: bool,
) -> None:
    """Dashboard Skills 单条 toolset 开关：只改 ``platform_toolsets`` 显式项。"""
    all_keys = _dashboard_configurable_toolset_keys()
    ...
    explicit = {x for x in raw if x in configurable_keys or x in mxai_granular}

    if name == MXAI_COMPOSITE_TOOLSET:
        if enabled:
            explicit |= {MXAI_COMPOSITE_TOOLSET}
            explicit -= mxai_granular
        else:
            explicit -= {MXAI_COMPOSITE_TOOLSET}
            explicit -= mxai_granular
    elif name in mxai_granular:
        non_mxai_explicit = explicit - mxai_granular - {MXAI_COMPOSITE_TOOLSET}
        has_composite = MXAI_COMPOSITE_TOOLSET in raw or MXAI_COMPOSITE_TOOLSET in explicit

        if enabled:
            if has_composite and not (explicit & mxai_granular):
                # 整包 mxai → 只开当前工具（勿在 enable 时展开全套）。
                mxai_explicit = {name}
            else:
                mxai_explicit = (explicit & mxai_granular) | {name}
            explicit = non_mxai_explicit | mxai_explicit
        else:
            if has_composite and not (explicit & mxai_granular):
                mxai_explicit = set(mxai_granular)
            else:
                mxai_explicit = set(explicit & mxai_granular)
            mxai_explicit.discard(MXAI_COMPOSITE_TOOLSET)
            mxai_explicit.discard(name)
            explicit = non_mxai_explicit | mxai_explicit
    else:
        if enabled:
            explicit.add(name)
    ...
    _save_platform_tools(config, platform, explicit)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Drop platform-scoped / non-configurable keys (incl. legacy composite mxai).
    enabled_toolset_keys = {
        ts for ts in enabled_toolset_keys
        if (ts in configurable_keys or ts in _mxai_per_tool_toolset_keys())
        and _toolset_allowed_for_platform(ts, platform)
    }
    enabled_toolset_keys = _normalize_mxai_explicit_for_save(enabled_toolset_keys)
