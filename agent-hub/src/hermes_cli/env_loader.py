...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.marketing_hub/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home) if hermes_home is not None else None
    if home_path is None:
        from hermes_constants import get_env_path

        home_path = get_env_path().parent
