...
In standard installs ``<root>`` is ``~/.marketing_hub``. In Docker / custom
deployments where ``HERMES_HOME`` points outside ``~/.marketing_hub`` (e.g.
...
def _worker_agent_data_dir(env: dict) -> Path:
    """Resolve worker Agent data dir from spawn env (no ``HERMES_HOME``)."""
    hub = (env.get("HUB_DATA_DIR") or "").strip()
    profile = (env.get("HERMES_PROFILE") or "").strip().lower()
    from hermes_constants import get_default_hermes_root

    base = Path(hub) if hub else get_default_hermes_root()
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    # 无 HUB_DATA_DIR 时回退默认数据根（~/.marketing_hub 或 %PROGRAMDATA%\MarketingHub）
    from hermes_constants import get_default_hermes_root
...
def _default_spawn(
    ...
    *,
    ...
    # Inject HERMES_PROFILE so the worker reads the profile-scoped config.yaml
    try:
        resolve_profile_env(profile_arg)  # validate profile exists
    ...
    if _kanban_worker_skill_available(str(_worker_agent_data_dir(env))):
