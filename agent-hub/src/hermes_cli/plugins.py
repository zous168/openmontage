...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hermes-agent;
   ...
   own discovery paths)
2. **User plugins**   – ``{HUB_DATA_DIR}/plugins/<name>/``（无 env 时 Unix 回退 ``~/.marketing_hub/plugins/``）
3. **Project plugins** – ``./.marketing_hub/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``hermes.plugins``
   entry-point group.
...
from hermes_cli.plugin_deps import RequiredService
...
# stderr in addition to ~/.marketing_hub/logs/agent.log. Aimed at plugin authors
...
ENTRY_POINTS_GROUP = "hermes.plugins"
...
class PluginManifest:
    ...
    # LT-006：声明式服务依赖（§6）。provides_services=本插件提供的契约 key；
    # requires_services=本插件消费的契约（含 optional/binding）。供依赖图 + 拓扑排序 + 校验。
    provides_services: List[str] = field(default_factory=list)
    requires_services: List["RequiredService"] = field(default_factory=list)
    ...
    #              in ~/.marketing_hub/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    # -- service registry (plugin↔plugin / hub DI, typed) --------------------

    def provide_service(self, contract: Any, impl: Any) -> None:
        """Register a typed service implementation under ``contract`` (LT-006).

        The way "plugin A calls plugin B's function" is meant to work: B
        ``provide_service(SomeContract, impl)``; A resolves it via
        :meth:`get_service` (or, for hub runtime objects without a ``ctx``, via
        ``core.service_registry.service_registry.get``). Neither side imports
        the other — both depend only on the contract in ``core/contracts/``.

        ``contract`` may be a ``Protocol`` class or a stable string key (e.g.
        ``"knowledge.retrieval@1"``).
        """
        from core.service_registry import service_registry

        service_registry.provide(contract, impl, plugin=self.manifest.name)
        logger.debug(
            "Plugin %s provided service: %s", self.manifest.name, contract,
        )

    def get_service(self, contract: Any) -> Any:
        """Resolve a typed service by ``contract`` (lazy; ``None`` if not provided).

        Soft consumers degrade on ``None``; hard dependencies should have been
        validated at startup (manifest ``requires_services``).
        """
        from core.service_registry import service_registry

        return service_registry.get(contract)

    # -- FastAPI 路由登记（插件挂到 Hermes web server · LT-007.01.01）----------
    ...
    def register_routes(
        ...
        *,
        ...
        """插件向 Hermes web server 登记 FastAPI 路由（延迟收口）.

        仅登记，不立即 ``include_router``——真正挂载由
        :meth:`PluginManager._finalize_routes` 在所有插件 ``register()`` /
        ``on_ready()`` 完成后统一执行，且须在 SPA catch-all 之前（见
        ``docs/Hub融入Hermes框架方案.md`` §5.5 约束 2）。

        ``dependencies``：挂载时附加到该 router 全部路由的 FastAPI 依赖（如
        /api/plugins/* 下的设备级租户绑定 bind_device_tenant_context）。
        """
        self._manager._pending_routers.append(
            {
                "router": router,
                "prefix": prefix,
                "tags": tags or [],
        ...
        logger.debug(
            "Plugin %s registered routes: prefix=%s", self.manifest.name, prefix,
        )
    ...
    def register_skill(
        ...
        ``~/.marketing_hub/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        # LT-006 三相生命周期：相2 ready / 相3 shutdown 的拓扑顺序（key 列表）
        ...
        # LT-007.01.01：插件经 ctx.register_routes() 登记的待挂 FastAPI 路由。
        # 每项 {router, prefix, tags}；由 _finalize_routes(web_app) 在 SPA
        # catch-all 之前统一 include_router（见 Hub 融入 Hermes 框架方案 §5.5）。
        self._pending_routers: List[Dict[str, Any]] = []
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        # 2. User plugins ({HUB_DATA_DIR}/plugins/)
        user_dir = get_default_hermes_root() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.marketing_hub/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".marketing_hub" / "plugins"
        ...
        # LT-006 三相生命周期·相2 ready：相1 provide(=register) 全部完成后，
        # 校验声明式依赖 + 按拓扑序调可选 on_ready(ctx)。
    ...
    def _enabled_loaded(self) -> List["LoadedPlugin"]:
        """已成功加载（register 成功、模块在）的插件，按加载序."""
        return [
            lp for lp in self._plugins.values()
            if lp.enabled and lp.module is not None
        ]

    def _run_ready_phase(self) -> None:
        """相2 ready：声明式依赖启动前校验（硬依赖缺 provider→fail-fast）+
        按拓扑序（仅 ready 边约束）调各插件可选 ``on_ready(ctx)``。

        provide(相1=register) 不消费别家服务，其顺序无关；消费在本相 / 懒解析，
        此时所有 provider 已注册。向后兼容：未定义 on_ready 的插件跳过。
        """
        from hermes_cli.plugin_deps import resolve_load_order, validate_providers

        loaded = self._enabled_loaded()
        manifests = [lp.manifest for lp in loaded]
        # 硬依赖 fail-fast + ready 边拓扑序（含 ready 环检测）。存量插件无声明 → 皆 no-op。
        validate_providers(manifests)
        ordered = resolve_load_order(manifests)

        by_key = {(lp.manifest.key or lp.manifest.name): lp for lp in loaded}
        self._lifecycle_order = [m.key or m.name for m in ordered]
        for m in ordered:
            lp = by_key.get(m.key or m.name)
            if lp is None or lp.module is None:
                continue
            fn = getattr(lp.module, "on_ready", None)
            if fn is None:
                continue
            try:
                fn(PluginContext(lp.manifest, self))
                logger.debug("Plugin '%s' on_ready() ok", lp.manifest.name)
            except Exception as exc:  # noqa: BLE001 — 单插件 ready 失败不拖垮宿主
                logger.warning(
                    "Plugin '%s' on_ready() failed: %s",
                    lp.manifest.name, exc, exc_info=_PLUGINS_DEBUG,
                )

    def shutdown(self) -> None:
        """相3 shutdown：逆拓扑序调各插件可选 ``on_shutdown(ctx)``（资源释放）.

        provider 后于 consumer 关闭。向后兼容：未定义 on_shutdown 的插件跳过。
        """
        for key in reversed(self._lifecycle_order):
            lp = self._plugins.get(key)
            if lp is None or not lp.enabled or lp.module is None:
                continue
            fn = getattr(lp.module, "on_shutdown", None)
            if fn is None:
                continue
            try:
                fn(PluginContext(lp.manifest, self))
                logger.debug("Plugin '%s' on_shutdown() ok", lp.manifest.name)
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    "Plugin '%s' on_shutdown() failed: %s",
                    lp.manifest.name, exc, exc_info=_PLUGINS_DEBUG,
                )

    def _finalize_routes(self, web_app) -> int:
        """把插件经 ``ctx.register_routes()`` 登记的路由统一挂到 web app（LT-007.01.01）.

        在三相 ``on_ready`` 全部完成后调用，且 **须早于 SPA catch-all**
        （组合根经 ``finalize_routes(app)`` 收口，见 Hub 融入 Hermes 框架方案
        §5.5 约束 2）。

        **可重入**（LT-007.01.02）：挂载后清空 ``_pending_routers``，故 import 期
        与发现后可多次调用——每次只挂尚未挂载的新登记，已挂的不重复。无登记时 no-op。
        返回本次挂载条数。
        """
        pending = self._pending_routers
        self._pending_routers = []
        for entry in pending:
            web_app.include_router(
                entry["router"],
                prefix=entry["prefix"],
                tags=entry["tags"],
                dependencies=entry.get("dependencies") or None,
            )
        if pending:
            logger.info("plugins.routes.finalized count=%d", len(pending))
        return len(pending)
    ...
    def _parse_manifest(
        ...
        try:
            ...
            return PluginManifest(
                ...
                provides_services=data.get("provides_services", []),
                requires_services=[
                    RequiredService.parse(r)
                    for r in data.get("requires_services", []) or []
                ],
                source=source,
                path=str(plugin_dir),
                kind=kind,
    ...
    def register_bundled_middleware(self, kind: str, callback: Callable) -> None:
        """Register platform/bundled middleware (not tied to a plugin manifest).

        Idempotent: the same callback object is not registered twice.
        """
        if kind not in VALID_MIDDLEWARE:
            logger.warning(
                "register_bundled_middleware: unknown kind '%s' (valid: %s)",
                kind,
                ", ".join(sorted(VALID_MIDDLEWARE)),
            )
        callbacks = self._middleware.setdefault(kind, [])
...
def register_bundled_middleware(kind: str, callback: Callable) -> None:
    """Register hub/platform middleware outside plugin ``register()``."""
    get_plugin_manager().register_bundled_middleware(kind, callback)
...
def get_manifest_toolset_tools(toolset_key: str) -> List[str]:
    """Return tool names for a toolset key from enabled plugin manifests.

    Used when ``resolve_toolset`` returns empty — e.g. plugin enabled but
    ``register()`` skipped (unsupported platform) so tools never hit the registry.
    """
    manager = get_plugin_manager()
    try:
        from tools.registry import registry
    ...
    for loaded in manager._plugins.values():
        ...
        for tool_name in manifest.provides_tools:
            entry = registry.get_entry(tool_name) if registry else None
            ts = entry.toolset if entry else tool_name
            if ts == toolset_key or tool_name == toolset_key:
                keys.add(tool_name)
    return sorted(keys)
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    # Manifest fallback: enabled plugins whose tools never registered (e.g.
    # platform gate in register()). Per-tool toolset key = tool name when
    # unregistered; matches mxai / google_meet per-tool registration.
    for _name, loaded in manager._plugins.items():
        if not loaded.enabled:
            continue
        manifest = loaded.manifest
        ...
        for tool_name in manifest.provides_tools:
            ...
            if entry:
                continue
            toolset_tools.setdefault(tool_name, []).append(tool_name)
            toolset_plugin[tool_name] = loaded
