    ...
    *,
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.marketing_hub/.env or run 'hermes model' to configure. To use "
...
# LT-045.01.02 · 官方渠道（platform LLM 网关）。
# base_url：LLM_GATEWAY_BASE_URL；未设则回退 CONTROL_SERVER_BASE_URL（平台一体同口）。
# 本地 dev CS/网关分端口时可显式设 LLM_GATEWAY_BASE_URL 覆盖。
from core.platform.gateway_base_url import (
    LLM_GATEWAY_BASE_URL_ENV,
    resolve_llm_gateway_base_url,
)


def _official_gateway_base_url() -> str:
    """官方渠道网关地址；未配置返回空串（→ 官方渠道不启用）。"""
    return resolve_llm_gateway_base_url()


def _official_openai_compatible_base_url(gateway_root: str) -> str:
    """供 OpenAI SDK 使用的 base_url（须含 ``/v1``）。

    ``CONTROL_SERVER_BASE_URL`` / 平台一体边缘口是根地址（``host:8080``），
    网关 Chat Completions 在 ``/v1/chat/completions``。SDK 会拼 ``/chat/completions``，
    若 base 无 ``/v1`` 会打到 ``/chat/completions`` → 404 Cannot POST。
    已带 ``/v1`` 的 ``LLM_GATEWAY_BASE_URL`` 不再追加。
    """
    base = (gateway_root or "").strip().rstrip("/")
    if not base:
        return ""
    if base.endswith("/v1"):
        return base
...
def _current_device_jwt() -> str:
    """当前设备登录 JWT（临近过期先 refresh）；未登录/读失败返回空串。"""
    try:
        from core.platform.device.device_auth_service import get_fresh_device_access_token

        return get_fresh_device_access_token()
    except Exception:  # noqa: BLE001 — 会话文件不可读时官方渠道优雅不启用
        return ""


def _resolve_official_runtime(
    *,
    requested_provider: str,
    explicit_api_key: Optional[str] = None,
    explicit_base_url: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """装配 ``providers.official``（platform LLM 网关）运行时。

    仅在下列条件全部满足时接管，否则返回 None 让原有解析链继续（BYOK /
    已配置 provider 保持不变）：
      - 无 explicit_api_key / explicit_base_url 覆盖（尊重 BYOK / 显式端点）；
      - requested_provider 为 "auto"（未显式指定 → 默认选官方）或 "official"；
      - LLM_GATEWAY_BASE_URL 已配置；
      - 存在可用的设备 JWT（已登录）。

    **显式选择官方渠道（requested_provider == "official"）时严格化**：网关未配置 /
    未登录时**直接抛 AuthError，绝不静默回退到其它 provider/model**——否则会掩盖
    平台计量失效、并让用户误以为仍在用官方渠道（见「不应该回退到其他模型」）。
    仅 "auto"（用户未显式选官方）才保留优雅降级（返回 None → BYOK）。
    """
    if (explicit_api_key or "").strip() or (explicit_base_url or "").strip():
        return None
    requested_norm = (requested_provider or "").strip().lower()
    if requested_norm not in {"official", "auto"}:
        return None
    explicit_official = requested_norm == "official"
    base_url = _official_gateway_base_url()
    if not base_url:
        if explicit_official:
            raise AuthError(
                "官方渠道（系统内置模型）未配置网关地址：请设置 CONTROL_SERVER_BASE_URL"
                "（或 LLM_GATEWAY_BASE_URL）。已显式选择官方渠道，不会回退到其它模型。",
                code="official_gateway_unconfigured",
            )
        return None
    jwt = _current_device_jwt()
    if not jwt:
        if explicit_official:
            raise AuthError(
                "官方渠道（系统内置模型）需要登录：未检测到设备会话（JWT）。"
                "请重新登录后重试。已显式选择官方渠道，不会回退到其它模型。",
                code="official_gateway_not_logged_in",
            )
        return None
    return {
        "provider": "official",
        "api_mode": "chat_completions",
        "base_url": _official_openai_compatible_base_url(base_url),
        "api_key": jwt,
...
def resolve_runtime_provider(
    *,
    ...
    # LT-045.01.02: 官方渠道（platform LLM 网关）优先。仅当 provider 未显式指定
    # （auto）或显式请求 "official"、且无 explicit_* 覆盖、网关已配置且已登录时
    # 接管；否则返回 None，原有 BYOK / 已配置 provider 解析链完全不变。
    official_runtime = _resolve_official_runtime(
        requested_provider=requested_provider,
        explicit_api_key=explicit_api_key,
        explicit_base_url=explicit_base_url,
    )
