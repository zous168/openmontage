...
is persisted in ``{数据根}/auth.json`` with cross-process file locking.
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    # 系统内置：凭据为设备 JWT，端点为 LLM_GATEWAY_BASE_URL（运行时注入，无 API Key）
    "official": ProviderConfig(
        id="official",
        name="系统内置模型",
        auth_type="device_jwt",
        inference_base_url="",
        api_key_env_vars=(),
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.marketing_hub/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Go to Settings → Main Model to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.marketing_hub/auth.json
# =============================================================================
...
def _auth_file_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        real_home_auth = (get_default_hermes_root() / "auth.json").resolve(strict=False)
        try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.marketing_hub/auth.json`` even when HERMES_HOME is set to a profile
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = (
                Path(real_home_env) / ".marketing_hub" / "auth.json"
            )
            try:
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        "未配置推理模型：请在设置中选择系统内置模型，或在 "
        f"{display_hermes_env_path()} 配置 API Key"
...
#
# Tokens live in ~/.marketing_hub/auth/google_oauth.json (managed by agent.google_oauth).
...
# Spotify auth — PKCE tokens stored in ~/.marketing_hub/auth.json
# =============================================================================
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # ``~/.marketing_hub/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.marketing_hub/.env, and return it.
    ...
    print()
    print(f"Saved HERMES_SPOTIFY_CLIENT_ID to {display_hermes_env_path()}")
    print()
    return raw
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.marketing_hub/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.marketing_hub/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.marketing_hub/auth.json)."""
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.marketing_hub/auth.json
# =============================================================================
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.marketing_hub/auth.json``. A single MITM during initial
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    # with a clear error so the user can reconfigure via Settings → Main Model to refetch.
...
# ``get_default_hermes_root()`` returns — ``~/.marketing_hub`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    Linux/macOS classic installs land at ``~/.marketing_hub/shared/``, native
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.marketing_hub/auth.json) called POST /api/oauth/token with Hermes's "
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.marketing_hub/auth.json."""
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.marketing_hub/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Go to Settings → Main Model and select "
                "MiniMax (OAuth).",
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, go to Settings → Main Model again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Go to Settings → Main Model again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Go to Settings → Main Model or configure an API key to use Hermes.")
        else:
