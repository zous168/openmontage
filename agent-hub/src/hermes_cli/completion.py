...
_hermes_profiles() {{
    local profiles_dir="$HOME/.marketing_hub/profiles"
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.marketing_hub/profiles" ]]; then
        profiles+=($HOME/.marketing_hub/profiles/*(N/:t))
    fi
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.marketing_hub/profiles",
        "        for d in $HOME/.marketing_hub/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
