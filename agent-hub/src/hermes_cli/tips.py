...
TIPS = [
    ...
    "The plan skill saves markdown plans under .marketing_hub/plans/ in the active workspace.",
    ...
    "Checkpoints use shadow repos in ~/.marketing_hub/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.marketing_hub/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.marketing_hub/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.marketing_hub/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.marketing_hub/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.marketing_hub/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.marketing_hub/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.marketing_hub/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.marketing_hub/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'Drop a YAML in ~/.marketing_hub/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.marketing_hub/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.marketing_hub/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.marketing_hub/skills/ so drop-in skills appear without restarting the session.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.marketing_hub/plugins/ — trust-gated by design.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.marketing_hub/dashboard-plugins/.',
]
