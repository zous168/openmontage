...
Profiles live under ``~/.marketing_hub/profiles/<name>/`` by default.

The "default" profile is ``~/.marketing_hub`` itself — backward compatible,
...
# default profile (``~/.marketing_hub``).  Named profiles never contain these
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.marketing_hub``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.marketing_hub) profile.
...
def _get_profiles_root() -> Path:
    ...
    ``~/.marketing_hub``, profiles live under ``HERMES_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    ...
    In standard deployments this is ``~/.marketing_hub``.
    In Docker/custom deployments where HERMES_HOME is outside ``~/.marketing_hub``
...
def _read_config_model(profile_dir: Path) -> tuple:
    """Read model/provider from a profile's **own** config.yaml for UI listing.

    一体部署运行时 ``load_config()`` 会合并全局 + profile 覆盖；此处仅用于
    Profiles 页展示——命名 profile 无 ``profiles/<name>/config.yaml`` 时返回
    ``(None, None)`` 表示继承 default/全局，勿把全局 model 标成该 profile 的。
    """
    from hermes_constants import get_default_hermes_root

    default_root = get_default_hermes_root()
    try:
        is_default = profile_dir.resolve() == default_root.resolve()
    except OSError:
        is_default = profile_dir == default_root

    local = profile_dir / "config.yaml"
    if local.exists():
        config_path = local
    elif is_default:
...
def _effective_config_model(profile_dir: Path) -> tuple[str | None, str | None]:
    """Merged model from ``load_config()`` while scoped to *profile_dir*."""
...
def list_profiles() -> List[ProfileInfo]:
    ...
    try:
        ...
        for pinfo in profiles:
            effective_model, effective_provider = _effective_config_model(pinfo.path)
            log_model_trace(
                "profiles.agent",
                name=pinfo.name,
                display_model=pinfo.model,
                display_provider=pinfo.provider,
                effective_model=effective_model,
                effective_provider=effective_provider,
                config_file=str(pinfo.path / "config.yaml"),
            )
    except Exception:
        pass
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.marketing_hub)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.marketing_hub/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
        ...
        # clone_all 同样去掉 model，避免整树拷贝带上源主模型
        try:
            from plugins.mxai.cfg.agent_model_config import strip_model_section_from_config_file

            strip_model_section_from_config_file(profile_dir / "config.yaml")
        except Exception:
            pass
    else:
                ...
                if src.exists():
                    ...
                    if filename == ".env":
                        try:
                            ...
                            pass
                    # CR-76：新 Profile 默认跟随全局主模型，禁止把源 config 的 model 段拷成「假独立绑定」
                    if filename == "config.yaml":
                        try:
                            from plugins.mxai.cfg.agent_model_config import (
                                strip_model_section_from_config_file,
                            )

                            strip_model_section_from_config_file(dst)
                        except Exception:
                            pass
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.marketing_hub).\n"
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.marketing_hub/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """当前 profile 名（Dashboard/Gateway 常驻 ``HUB_DATA_DIR`` 根，即 ``default``）。"""
    return "default"
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.marketing_hub itself — its parent is ~/ and its
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.marketing_hub itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.marketing_hub). "
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    are imported, to set the ``HERMES_PROFILE`` environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
