...
def resolve_node_executable() -> str | None:
    """解析 node 可执行文件：优先 ``HERMES_NODE``，其次 PATH 上的 ``node``。"""
    raw = (os.environ.get("HERMES_NODE") or "").strip().strip('"')
    if raw:
        candidate = Path(raw)
...
_DEP_CHECKS = {
    "node": lambda: resolve_node_executable() is not None,
    "browser": lambda: (
    ...
    "ffmpeg": lambda: _has_ffmpeg_unified(),
}


def _has_ffmpeg_unified() -> bool:
    try:
