...
`hermes dashboard` on port 9119 serves the **built** bundle from `hermes_cli/web_dist/`, not the Vite dev server — changes in `web_src/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).

The Vite dev server proxies `/api` requests to the FastAPI backend (default `http://127.0.0.1:9119`; in marketing-hub use `HERMES_DASHBOARD_URL=http://127.0.0.1:8000`).
...
This outputs to `../web_dist/`, which the FastAPI server serves as a static SPA.
