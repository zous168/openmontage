...
export default function FilesPage() {
    ...
    setEnd(
      <div className="flex items-center gap-2">
        <Button
          ...
          onClick={() => void load()}
          disabled={loading}
          aria-label={t.files?.refreshAriaLabel ?? "Refresh files"}
        >
  ...
  }, [headerPath, load, loading, setAfterTitle, setEnd, t.files]);
  ...
  const goToPath = async () => {
    ...
    if (!nextPath) {
      showToast(t.files?.toastPathRequired ?? "Path required", "error");
      return;
    }
    await load(nextPath);
  };
  ...
  const createDirectory = async () => {
    ...
    if (!activePath) {
      showToast(t.files?.toastDirectoryUnavailable ?? "Directory unavailable", "error");
      return;
    }
    if (!name) {
      showToast(t.files?.toastFolderNameRequired ?? "Folder name required", "error");
      return;
    }
    setCreating(true);
    try {
      ...
      showToast(t.files?.toastFolderCreated ?? "Folder created", "success");
      await load();
    } catch (e) {
      showToast(
        t.files?.toastCreateFailed?.replace("{error}", String(e)) ?? `Create failed: ${e}`,
        "error",
      );
    } finally {
      setCreating(false);
  ...
  const uploadFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      showToast(
        t.files?.toastFilesUploaded?.replace("{count}", String(files.length)) ??
      ...
      showToast(
        t.files?.toastUploadFailed?.replace("{error}", String(e)) ?? `Upload failed: ${e}`,
        "error",
      );
    } finally {
      setUploading(false);
  ...
  const downloadFile = async (entry: ManagedFileEntry) => {
    ...
    try {
      ...
      showToast(
        t.files?.toastDownloadFailed?.replace("{error}", String(e)) ?? `Download failed: ${e}`,
        "error",
      );
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast(t.files?.toastDeleted ?? "Deleted", "success");
      setPendingDelete(null);
      await load();
    } catch (e) {
      showToast(
        t.files?.toastDeleteFailed?.replace("{error}", String(e)) ?? `Delete failed: ${e}`,
        "error",
      );
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="flex min-w-0 max-w-full flex-col gap-4">
        ...
        {canChangePath ? (
          ...
          >
            <Input
              ...
              aria-label={t.files?.pathAriaLabel ?? "Path"}
              placeholder={t.files?.pathPlaceholder ?? "Path"}
      ...
      <button
        ...
        onDrop={handleDrop}
        disabled={!canUpload}
        aria-label={t.files?.uploadAreaAriaLabel ?? "Upload files"}
        ...
        <span className="flex min-w-0 items-center gap-3">
          ...
          <span className="min-w-0">
            <span className="block text-sm font-semibold uppercase tracking-[0.08em] text-foreground">
              {uploading
                ? (t.files?.uploadingLabel ?? "Uploading")
                : draggingFiles
                  ? (t.files?.releaseToUpload ?? "Release to upload")
      ...
      <Card className="min-w-0 max-w-full overflow-hidden">
        <CardContent className="overflow-x-auto p-0">
          ...
          <div className="grid min-w-[42rem] grid-cols-[minmax(12rem,1fr)_7rem_10rem_5.5rem] items-center gap-3 border-b border-border px-4 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-text-tertiary">
            <span>{t.files?.columnName ?? "Name"}</span>
            <span>{t.files?.columnSize ?? "Size"}</span>
            <span>{t.files?.columnModified ?? "Modified"}</span>
            <span className="text-right">{t.files?.columnActions ?? "Actions"}</span>
          </div>
                ...
                <span className="flex justify-end gap-1">
                  {entry.is_directory ? (
                    <Button
                      ...
                      aria-label={
                        t.files?.openAriaLabel?.replace("{name}", entry.name) ?? `Open ${entry.name}`
                      }
                    >
                      <FolderOpen />
                    </Button>
                    ...
                    <Button
                      ...
                      aria-label={
                        t.files?.downloadAriaLabel?.replace("{name}", entry.name) ??
                        `Download ${entry.name}`
                      }
                    >
                      <Download />
                  ...
                  <Button
                    ...
                    aria-label={
                      t.files?.deleteAriaLabel?.replace("{name}", entry.name) ?? `Delete ${entry.name}`
                    }
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{t.files?.createFolderTitle ?? "Create folder"}</DialogTitle>
            <DialogDescription>
              {t.files?.createFolderTarget?.replace("{path}", activePath || (t.files?.loadingFallback ?? "Loading")) ??
      ...
      <DeleteConfirmDialog
        ...
        title={
          pendingDelete
            ? (t.files?.deleteTitleWithName?.replace("{name}", pendingDelete.name) ??
                `Delete ${pendingDelete.name}?`)
            : (t.files?.deleteTitleFallback ?? "Delete item?")
        }
        description={
          pendingDelete?.is_directory
            ? (t.files?.deleteFolderDescription ?? "This removes the folder and everything inside it.")
            : (t.files?.deleteFileDescription ?? "This removes the file.")
        }
      />
    </div>
  );
