          ...
          <CardContent className="py-12">
            <div className="mx-auto flex max-w-2xl flex-col gap-3 text-sm text-muted-foreground">
              <h2 className="font-mondwest text-display text-base tracking-wider text-foreground">
                {t.analytics?.tokenAnalyticsHiddenTitle ?? "Token analytics hidden"}
              </h2>
              <p>
                {t.analytics?.tokenAnalyticsHiddenBody1 ?? "The token, cost, and per-day analytics on this page are a local debug estimate. They only count successful main-agent responses with a usable"}{" "}
                <span className="font-mono">usage</span>{" "}
                {t.analytics?.tokenAnalyticsHiddenBody1After ?? "block, and silently exclude auxiliary calls (context compression, title generation, vision, session search, web extract, smart approvals, MCP routing, plugin LLM access) plus provider-side retries and fallback attempts. Cache writes are missing entirely."}
              </p>
              <p>
                {t.analytics?.tokenAnalyticsHiddenBody2 ?? "On models with heavy auxiliary traffic (Kimi K2.6, MiniMax M2.7) the local total can be 10x–100x lower than what your provider bills. Hiding these numbers is safer than letting them look authoritative."}
              </p>
              <p>
                {t.analytics?.tokenAnalyticsHiddenBody3 ?? "Check your provider dashboard (OpenRouter, Anthropic, etc.) for actual usage and billing. To re-enable the local debug estimate anyway, set"}{" "}
                <span className="font-mono">
                  dashboard.show_token_analytics: true
                </span>{" "}
                {t.analytics?.tokenAnalyticsHiddenBody3In ?? "in"} <a href="/config" className="underline">{t.analytics?.tokenAnalyticsConfigLink ?? "Config"}</a>.
              </p>
            </div>
          </CardContent>
        </Card>
