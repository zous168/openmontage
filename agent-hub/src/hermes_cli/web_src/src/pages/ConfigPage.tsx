...
import { resolveConfigFieldLabel } from "@/lib/config-field-i18n";
...
export default function ConfigPage() {
  ...
  function formatFieldsCount(count: number): string {
    return t.config.fieldsCount
  ...
  const searchMatchedFields = useMemo(() => {
    ...
    return Object.entries(schema).filter(([key, s]) => {
      ...
      const zhLabel = resolveConfigFieldLabel(key, locale);
      return (
        key.toLowerCase().includes(lowerSearch) ||
        humanLabel.toLowerCase().includes(lowerSearch) ||
        zhLabel.toLowerCase().includes(lowerSearch) ||
        String(s.category ?? "")
          .toLowerCase()
  ...
  }, [isSearching, lowerSearch, locale, schema]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {t.config.globalScopeHint ? (
        <p className="text-muted-foreground text-xs border-l-2 border-muted-foreground/30 pl-3">
          {t.config.globalScopeHint}
        </p>
      ) : null}
      ...
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:shrink-0">
          ...
          >
            {yamlMode ? t.common.form : t.config.yamlModeLabel}
          </Button>
          ...
          <aside aria-label={t.config.filters} className="sm:w-56 sm:shrink-0">
            <div className="sm:sticky sm:top-4">
              <div className="flex flex-col border border-border bg-muted/20">
                ...
                <div className="hidden sm:block px-3 pt-2 pb-1 font-sans text-xs text-text-tertiary">
                  {t.config.sections}
                </div>
                ...
                <CardHeader className="py-3 px-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge tone="secondary" className="text-xs">
                      {formatFieldsCount(searchMatchedFields.length)}
                    </Badge>
                  </div>
                </CardHeader>
              ...
              <Card>
                <CardHeader className="py-3 px-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge tone="secondary" className="text-xs">
                      {formatFieldsCount(activeFields.length)}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent className="grid gap-2 px-4 pb-4">
      ...
      <ConfirmDialog
        ...
        description={t.config.confirmResetDescription.replace(
          "{count}",
