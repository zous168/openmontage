...
import { api, parseSessionRowKey, sessionRowKey } from "@/lib/api";
import type {
  ProfilesSessionStoreStats,
  SessionInfo,
  SessionMessage,
  SessionSearchResult,
  StatusResponse,
...
import { LlmRequestLogPanel } from "@/components/LlmRequestLogPanel";
...
function SessionRow({
  ...
  const [expandedTab, setExpandedTab] = useState<"messages" | "llm">("messages");
  const { t } = useI18n();
  ...
  const actionButtons = (
    <>
      ...
      <Button
        ...
        aria-label={t.sessions2?.renameSession ?? "Rename session"}
        title={t.sessions2?.renameSession ?? "Rename session"}
        onClick={(e) => {
          e.stopPropagation();
          setRenameValue(
      ...
      <Button
        ...
        size="icon"
        className="text-muted-foreground hover:text-foreground"
        aria-label={t.sessions2?.exportSession ?? "Export session"}
        title={t.sessions2?.exportSessionTitle ?? "Export session JSON"}
        onClick={(e) => {
          e.stopPropagation();
          onExport(session.id, session.profile);
        }}
      >
        <Download />
      </Button>
    ...
    >
                ...
                {renaming ? (
                  ...
                  >
                    <Input
                      ...
                      placeholder={t.sessions2?.sessionTitlePlaceholder ?? "Session title"}
                    ...
                    <Button
                      ...
                      size="icon"
                      className="text-muted-foreground hover:text-success"
                      aria-label={t.sessions2?.saveTitle ?? "Save title"}
                      title={t.sessions2?.saveTitle ?? "Save title"}
                      disabled={renameSaving}
                    ...
                    <Button
                      ...
                      size="icon"
                      className="text-muted-foreground hover:text-foreground"
                      aria-label={t.sessions2?.cancelRename ?? "Cancel rename"}
                      title={t.sessions2?.cancelRename ?? "Cancel rename"}
                      disabled={renameSaving}
      ...
      {isExpanded && (
        <div className="min-w-0 border-t border-border bg-background/50 p-4">
          <Segmented
            value={expandedTab}
            onChange={(value) => setExpandedTab(value as "messages" | "llm")}
            options={[
              {
                value: "messages",
                label: t.sessions2?.tabMessages ?? "Messages",
              },
              {
                value: "llm",
                label: t.sessions2?.tabLlmRequests ?? "LLM requests",
              },
            ]}
            className="mb-3"
          />
          {expandedTab === "llm" ? (
            <LlmRequestLogPanel sessionId={session.id} profile={session.profile} />
          ) : (
            <>
              {loading && (
...
export default function SessionsPage() {
  ...
  const [stats, setStats] = useState<ProfilesSessionStoreStats | null>(null);
  ...
  useEffect(() => {
    setEnd(
      ...
      >
        ...
        {t.sessions2?.pruneOldSessions ?? "Prune old sessions"}
      </Button>,
    );
    return () => {
      setEnd(null);
  ...
  const loadSessions = useCallback(
    ...
    [managementProfile, sourceFilter],
  );

  const loadStats = useCallback(() => {
    api
      .getProfilesSessionStats(managementProfile)
      .then(setStats)
      .catch(() => {});
  }, [managementProfile]);
  ...
  }, [managementProfile, clearSelection]);

  useEffect(() => {
    setPage(0);
    setExpandedId(null);
    clearSelection();
    if (sourceFilter) setView("list");
  ...
  useEffect(() => {
    const loadOverview = () => {
      api
        ...
        .catch(() => {});
      api
        .getSessions(50, 0, managementProfile, sourceFilter ?? undefined)
  ...
  const sessionDelete = useConfirmDelete({
    onDelete: useCallback(
      async (rowKey: string) => {
        const { id, profile } = parseSessionRowKey(rowKey);
        try {
          await api.deleteSession(id, profile);
          setSessions((prev) =>
            prev.filter((s) => sessionRowKey(s) !== rowKey),
          );
          setTotal((prev) => prev - 1);
          if (expandedId === rowKey) setExpandedId(null);
          setSelectedIds((prev) => {
  ...
  const handleSelectClick = useCallback(
    (event: React.MouseEvent, index: number, visibleList: SessionInfo[]) => {
      const rowKey = visibleList[index] ? sessionRowKey(visibleList[index]) : null;
      if (!rowKey) return;
      setSelectedIds((prev) => {
        const next = new Set(prev);
        const wasSelected = next.has(rowKey);
        ...
        if (event.shiftKey && anchor !== null && anchor < visibleList.length) {
          ...
          for (let i = lo; i <= hi; i++) {
            const key = visibleList[i] ? sessionRowKey(visibleList[i]) : null;
            if (!key) continue;
  ...
  const selectAllOnPage = useCallback((visibleList: SessionInfo[]) => {
    setSelectedIds((prev) => {
      const next = new Set(prev);
      for (const s of visibleList) next.add(sessionRowKey(s));
      return next;
    });
  }, []);
    ...
    try {
      ...
      for (const rowKey of rowKeys) {
        ...
        try {
          ...
          /* skip rows that fail (e.g. already gone) */
        }
      }
      showToast(
      ...
      setDeleteSelectedOpen(false);
      const deletedSet = new Set(rowKeys);
      setSessions((prev) => prev.filter((s) => !deletedSet.has(sessionRowKey(s))));
    ...
    async (id: string, title: string, profile?: string) => {
      try {
        ...
        const matches = (s: SessionInfo) =>
          s.id === id &&
          (s.profile ?? "default") === (profile ?? "default");
        setSessions((prev) =>
          prev.map((s) => (matches(s) ? { ...s, title } : s)),
        );
        setOverviewSessions((prev) =>
          prev.map((s) => (matches(s) ? { ...s, title } : s)),
        );
        showToast(t.sessions2?.sessionRenamed ?? "Session renamed", "success");
        loadStats();
      } catch {
        showToast(t.sessions2?.failedToRenameSession ?? "Failed to rename session", "error");
      }
    },
    [showToast, loadStats, t.sessions2],
  );
  ...
  const handleExport = useCallback(
    async (id: string, profile?: string) => {
      try {
        ...
        showToast(t.sessions2?.failedToExportSession ?? "Failed to export session", "error");
      }
    },
    [showToast, t.sessions2],
  );

  const handlePrune = useCallback(async () => {
    const days = parseInt(pruneDays, 10);
    if (!Number.isFinite(days) || days < 0) {
      showToast(t.sessions2?.enterValidDays ?? "Enter a valid number of days", "error");
      return;
    }
    setPruning(true);
    try {
      ...
      showToast(
        (t.sessions2?.prunedSessions ?? "Pruned {count} sessions").replace(
          "{count}",
          String(resp.removed),
        ),
        "success",
      ...
      showToast(t.sessions2?.failedToPruneSessions ?? "Failed to prune sessions", "error");
    } finally {
      setPruning(false);
    }
  }, [pruneDays, showToast, loadSessions, loadStats, t.sessions2]);

  const pendingSession = sessionDelete.pendingId
    ? sessions.find((s) => sessionRowKey(s) === sessionDelete.pendingId)
    : null;
    ...
    <div className="flex min-w-0 w-full max-w-full flex-col gap-4">
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{t.sessions2?.pruneDialogTitle ?? "Prune old sessions"}</DialogTitle>
            <DialogDescription>
              {t.sessions2?.pruneDialogDescription ?? "Permanently remove archived sessions whose last activity is older than the given number of days. Active sessions are never pruned."}
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-1.5">
            ...
            >
              {t.sessions2?.olderThanDays ?? "Older than (days)"}
            </label>
            <Input
              id="prune-days"
              type="number"
      ...
      {stats && (
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 border border-border bg-background-base/40 px-4 py-3">
          <div className="flex flex-col">
            ...
            <span className="text-xs text-muted-foreground">{t.sessions2?.statsTotal ?? "Total"}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-lg font-semibold tabular-nums leading-none text-success">
              {stats.active_store}
            </span>
            <span className="text-xs text-muted-foreground">{t.sessions2?.statsActiveInStore ?? "Active in store"}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-lg font-semibold tabular-nums leading-none">
              {stats.archived}
            </span>
            <span className="text-xs text-muted-foreground">{t.sessions2?.statsArchived ?? "Archived"}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-lg font-semibold tabular-nums leading-none">
              {stats.messages}
            </span>
            <span className="text-xs text-muted-foreground">{t.sessions2?.statsMessages ?? "Messages"}</span>
          </div>
          {Object.keys(stats.by_source).length > 0 && (
            <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
              {sourceFilter && (
                ...
                >
                  {t.sessions2?.clearSourceFilter ?? "All sources"}
                </Button>
              )}
              {Object.entries(stats.by_source)
                ...
                return (
                  <button
                    ...
                    title={
                      active
                        ? (t.sessions2?.clearSourceFilter ?? "Clear filter")
                        : (t.sessions2?.filterBySource ?? "Filter by source").replace(
                            "{source}",
                            src,
                          )
                    }
      ...
      {showList && selectedIds.size > 0 && (
        ...
        >
          ...
          {filtered.some((s) => !selectedIds.has(sessionRowKey(s))) && (
            <Button
              ghost
              size="sm"
            ...
            <div className="flex min-w-0 flex-col gap-1.5">
              {filtered.map((s, index) => {
                ...
                <SessionRow
                  ...
                  onDelete={() => sessionDelete.requestDelete(rowKey)}
...
interface SessionRowProps {
  ...
  isSelected: boolean;
  onDelete: () => void;
  onExport: (id: string, profile?: string) => void;
