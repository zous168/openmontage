...
function runtimeStatusLabel(
  status: HubAgentPluginRow["runtime_status"],
  t: Translations,
): string {
...
export default function PluginsPage() {
  ...
  const onInstall = async () => {
    ...
    try {
      ...
      showToast(
        t.pluginsPage.installedToast.replace("{name}", r.plugin_name ?? id),
        "success",
      );
  ...
  const onRescan = async () => {
    ...
    try {
      ...
      showToast(
        t.pluginsPage.rescanSuccess.replace("{count}", String(rc.count)),
        "success",
      );
      await loadHub();
    } catch (e) {
  ...
  const setRuntimeLoading = async (name: string, fn: () => Promise<unknown>) => {
    ...
    try {
      ...
      showToast(e instanceof Error ? e.message : t.pluginsPage.actionFailed, "error");
    } finally {
      setRowBusy(null);
    }
  };
      ...
      <CardContent className="flex flex-col gap-4 px-6 py-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-3">
            ...
            <Badge tone={badgeTone}>{runtimeStatusLabel(row.runtime_status, t)}</Badge>
        ...
        {row.toolset ? (
          <p className="text-xs tracking-[0.05em] text-text-tertiary">
            Toolset: <span className="font-mono-ui">{row.toolset}</span>
            {row.kind === "backend" ? (
              <span className="text-text-tertiary"> · bundled backend</span>
            ) : null}
          </p>
        ) : null}

        {(row.provides_tools?.length ?? 0) > 0 ? (
          <div className="flex flex-wrap gap-1">
            {row.provides_tools!.map((tool) => (
              <Badge key={tool} tone="secondary" className="text-xs font-mono-ui">
                {tool}
              </Badge>
            ))}
          </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          void setRuntimeLoading(row.name, async () => {
            ...
            showToast(
              t.pluginsPage.removedToast.replace("{name}", row.name),
              "success",
            );
          });
        }}
        title={t.pluginsPage.removeConfirm}
        description={t.pluginsPage.removeConfirmDescription.replace("{name}", row.name)}
        destructive
