...
import { cn, hubPanelTitle } from "@/lib/utils";
...
function terminalLineHeightForWidth(layoutWidthPx: number): number {
  return layoutWidthPx < 1024 ? 1.08 : 1.2;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    if (
      ...
      setBanner(
        chatCopy?.sessionTokenUnavailable ??
  ...
  const applyChatModelToSession = useCallback(
    ({ provider, model }: { provider: string; model: string }) => {
      const ws = wsRef.current;
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      const cmd = `/model ${model} --provider ${provider}`;
      ws.send(cmd);
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const url = buildWsUrl(authParam, resumeParam, channel, scopedProfile, locale);
    ...
    ws.onclose = (ev) => {
      ...
      if (ev.code === 4401) {
        setBanner(
          ev.reason
            ? (chatCopy?.authFailedWithReason ?? "Auth failed ({reason}). Reload to refresh the session.").replace(
                "{reason}",
                ev.reason,
              )
            : chatCopy?.authFailed ??
      ...
      if (ev.code === 4403) {
        ...
        setBanner(
          ev.reason
            ? (chatCopy?.refusedWithReason ?? "Refused: {reason}.").replace(
                "{reason}",
                ev.reason,
              )
            : chatCopy?.originMismatch ??
      ...
      term.write(
        `\r\n\x1b[90m${(chatCopy?.sessionEnded ?? "[session ended (code {code})]").replace("{code}", String(ev.code))}\x1b[0m\r\n`,
      );
    };
  ...
  }, [channel, resumeParam, scopedProfile, locale]);
        ...
        >
          ...
          >
            <Typography
              className={cn(
                hubPanelTitle,
                "text-[1.0625rem] leading-snug text-midground",
              )}
          ...
          >
            <ChatSidebar
              channel={channel}
              chatProfile={scopedProfile}
              onSessionModelRefresh={applyChatModelToSession}
            />
          </div>
        </div>
      </>,
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          ...
          <Button
            ...
            title={chatCopy?.copyLastResponse ?? "Copy last assistant response as raw markdown"}
            aria-label={chatCopy?.copyLastResponse ?? "Copy last assistant response"}
            className={cn(
              "absolute z-10",
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              ...
              <span className="hidden min-[400px]:inline tracking-wide">
                {copyState === "copied"
                  ? (chatCopy?.copied ?? "copied")
                  : (chatCopy?.copyLastResponse ?? "copy last response")}
              </span>
            </span>
          </Button>
        </div>
