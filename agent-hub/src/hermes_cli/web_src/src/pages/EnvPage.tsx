...
/** Map env-var key prefixes to a provider id + ordering. */
const PROVIDER_GROUP_DEFS: { prefix: string; id: string; priority: number }[] = [
  { prefix: "NOUS_", id: "nousPortal", priority: 0 },
  { prefix: "ANTHROPIC_", id: "anthropic", priority: 1 },
  { prefix: "DASHSCOPE_", id: "dashscopeQwen", priority: 2 },
  { prefix: "HERMES_QWEN_", id: "dashscopeQwen", priority: 2 },
  { prefix: "DEEPSEEK_", id: "deepseek", priority: 3 },
  { prefix: "GOOGLE_", id: "gemini", priority: 4 },
  { prefix: "GEMINI_", id: "gemini", priority: 4 },
  { prefix: "GLM_", id: "glmZai", priority: 5 },
  { prefix: "ZAI_", id: "glmZai", priority: 5 },
  { prefix: "Z_AI_", id: "glmZai", priority: 5 },
  { prefix: "HF_", id: "huggingFace", priority: 6 },
  { prefix: "KIMI_", id: "kimiMoonshot", priority: 7 },
  { prefix: "MINIMAX_CN_", id: "minimaxChina", priority: 9 },
  { prefix: "MINIMAX_", id: "minimax", priority: 8 },
  { prefix: "OPENCODE_GO_", id: "opencodeGo", priority: 10 },
  { prefix: "OPENCODE_ZEN_", id: "opencodeZen", priority: 11 },
  { prefix: "OPENROUTER_", id: "openrouter", priority: 12 },
  { prefix: "XIAOMI_", id: "xiaomiMimo", priority: 13 },
  { prefix: "MOARK_", id: "moark", priority: 14 },
];

const OTHER_PROVIDER_GROUP_ID = "other";

function getProviderGroupId(key: string): string {
  for (const g of PROVIDER_GROUP_DEFS) {
    if (key.startsWith(g.prefix)) return g.id;
  }
  return OTHER_PROVIDER_GROUP_ID;
}

function getProviderPriority(groupId: string): number {
  const entry = PROVIDER_GROUP_DEFS.find((g) => g.id === groupId);
  return entry?.priority ?? 99;
}

function providerGroupLabel(
  groupId: string,
  groups: Translations["env"]["providerGroups"],
): string {
...
function isCredentialKey(varKey: string): boolean {
  return varKey.endsWith("_API_KEY") || varKey.endsWith("_TOKEN");
}
    ...
    <div className="grid gap-2 border border-border p-4 min-w-0 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {info.is_set && (
            <Button
              ...
              aria-label={
                isRevealed
                  ? t.env.hideValueAria.replace("{key}", varKey)
                  : t.env.revealValueAria.replace("{key}", varKey)
              }
            >
          ...
          {info.is_set && isCredentialKey(varKey) && onTest && (
            <Button
              size="sm"
              outlined
              prefix={testingKey === varKey ? <Spinner /> : <Play />}
              onClick={() => onTest(varKey, "")}
              disabled={testingKey === varKey || saving === varKey}
            >
      ...
      {isEditing && (
        <div className="flex items-center gap-2">
          ...
          {isCredentialKey(varKey) && onTest && edits[varKey] && (
            <Button
              size="sm"
              outlined
              prefix={testingKey === varKey ? <Spinner /> : <Play />}
              onClick={() => onTest(varKey, edits[varKey])}
              disabled={testingKey === varKey || saving === varKey}
            >
              {t.env.testConnection}
            </Button>
...
function ProviderGroupCard({
  ...
  const testApiKey = apiKeys.find(([k, info]) => info.is_set && isCredentialKey(k))?.[0];
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="font-semibold text-sm tracking-wide">
            {providerGroupLabel(group.id, t.env.providerGroups)}
          </span>
          {hasAnyConfigured && (
            <Badge tone="success" className="text-xs">
              {t.env.configuredCount.replace(
                "{count}",
        ...
        <div className="flex items-center gap-2 shrink-0">
          {testApiKey && onTest && (
            <Button
              ...
              outlined
              className="shrink-0"
              prefix={testingKey === testApiKey ? <Spinner /> : <Play />}
              onClick={(e) => {
                e.stopPropagation();
                onTest(testApiKey, "");
              }}
              disabled={testingKey === testApiKey}
            >
              {t.env.testConnection}
            </Button>
          )}
...
export default function EnvPage() {
  ...
  const sections = useMemo(() => {
    const items: { id: string; label: string }[] = [
      { id: "section-oauth", label: t.env.sectionOAuth },
      { id: "section-providers", label: t.env.sectionProviders },
    ];
    if (vars) {
      ...
      >
        {sections.map((s) => (
          <button
            ...
            className="shrink-0 cursor-pointer px-2 py-0.5 font-sans text-xs text-text-secondary hover:text-foreground border border-border/50 hover:border-foreground/30 transition-colors"
          >
            {s.label}
          </button>
        ))}
  ...
  const handleSave = async (key: string) => {
    ...
    try {
      ...
      showToast(t.env.savedToast.replace("{key}", key), "success");
    } catch (e) {
  ...
  const handleTestCredential = async (key: string, value: string) => {
    const testFail = t.env.testFailed ?? "Verification failed";
    setTestingKey(key);
    try {
      const result = await api.validateProviderCredential(key, value);
      const testOk = t.env.testSuccess ?? "Connection OK";
      const testOkModels =
        t.env.testSuccessModels ?? "Connection OK — {count} models available";
      const testUnreachable =
        t.env.testUnreachable ?? "Could not reach provider";
      const testNoProbe =
        t.env.testNoProbe ?? "No automatic test for this key";
      if (result.ok) {
        const count = result.model_count ?? result.models?.length ?? 0;
        if (count > 0) {
          showToast(testOkModels.replace("{count}", String(count)), "success");
        } else if (!result.reachable) {
          showToast(testNoProbe, "success");
        } else {
          showToast(testOk, "success");
        }
      } else if (!result.reachable) {
        showToast(testUnreachable, "error");
      } else {
        showToast(result.message || testFail, "error");
      }
    } catch (e) {
      showToast(`${testFail}: ${e}`, "error");
    } finally {
      setTestingKey(null);
    }
  };
  ...
  const { providerGroups, nonProviderGrouped } = useMemo(() => {
    ...
    for (const entry of providerEntries) {
      const groupId = getProviderGroupId(entry[0]);
    ...
    const groups: ProviderGroup[] = Array.from(groupMap.entries())
      .map(([id, entries]) => ({
        id,
        priority: getProviderPriority(id),
        entries,
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {section.unsetEntries.length > 0 && (
            <button
              ...
              className="shrink-0 cursor-pointer border-0 bg-transparent p-0 font-sans text-xs text-text-secondary transition-colors hover:text-foreground"
            >
