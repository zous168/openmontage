...
export function AutoField({
  ...
  const label = resolveConfigFieldLabel(schemaKey, locale);
  const descriptionRaw = resolveConfigFieldDescription(schemaKey, schema, locale);
  const description =
    descriptionRaw && descriptionRaw !== label ? descriptionRaw : undefined;
  const arrayItemLabel = (index: number) =>
    t.config.arrayItemLabel.replace("{index}", String(index));
  ...
  if (isRecord(value) || (Array.isArray(value) && value.some((item) => isRecord(item)))) {
    return (
      <div className="grid gap-3 border border-border p-3">
        ...
        <NestedValueEditor fieldKey={schemaKey} value={value} onChange={onChange} arrayItemLabel={arrayItemLabel} />
      </div>
    );
  }
    ...
    return (
      <div className="grid gap-1.5">
        ...
        <Input
          ...
          placeholder={t.config.listPlaceholder}
        />
      </div>
    );
  }
