...
function themeLabel(t: Translations, th: ThemeListEntry): string {
  return t.theme?.presets?.[th.name]?.label ?? th.label;
}

function themeDescription(t: Translations, th: ThemeListEntry): string | undefined {
  const desc = t.theme?.presets?.[th.name]?.description ?? th.description;
  return desc || undefined;
}

function fontLabel(t: Translations, f: FontChoice): string {
  return t.theme?.fonts?.[f.id] ?? f.label;
}
...
export function ThemeSwitcher({ collapsed = false, dropUp = false }: ThemeSwitcherProps) {
  ...
  const label = current ? themeLabel(t, current) : themeName;
    ...
    <div ref={wrapperRef} className="relative">
      ...
      >
        <span className="inline-flex items-center gap-1.5">
          ...
          {!collapsed && (
            <Typography className="hidden sm:inline text-xs font-sans tracking-normal">
              {label}
            </Typography>
          )}
        </span>
        ...
        const dropdown = (
          ...
          >
            <div className="border-b border-current/20 px-3 py-2">
              <Typography className="text-xs font-sans tracking-normal text-text-tertiary">
                {sheetTitle}
              </Typography>
            </div>
  ...
  return (
    <>
      {availableThemes.map((th) => {
        ...
        const label = themeLabel(t, th);
        const description = themeDescription(t, th);
        ...
        return (
          ...
          >
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-0.5">
              <Typography className="truncate text-xs font-sans tracking-normal">
                {label}
              </Typography>
              {description && (
        ...
        return (
          <div key={cat}>
            <div className="px-3 pb-0.5 pt-1.5">
              <Typography className="text-xs font-medium tracking-normal text-text-tertiary">
                {catLabel}
              </Typography>
            </div>
            {fonts.map((f) => {
