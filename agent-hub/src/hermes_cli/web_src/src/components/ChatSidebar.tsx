...
import { api, buildWsAuthParam, getManagementProfile, HERMES_BASE_PATH } from "@/lib/api";
...
interface ChatSidebarProps {
  channel: string;
  className?: string;
  /** Chat PTY profile (e.g. coder) — model writes land in profiles/<name>/config.yaml. */
  chatProfile?: string;
  onSessionModelRefresh?: (args: { provider: string; model: string }) => void;
}
...
export function ChatSidebar({
  ...
  /** Live model from the chat PTY gateway (events channel). */
  const [ptyInfo, setPtyInfo] = useState<SessionInfo>({});
  /** Config default from REST — fallback before PTY emits session.info. */
  const [configInfo, setConfigInfo] = useState<SessionInfo>({});
  const [sidecarInfo, setSidecarInfo] = useState<SessionInfo>({});
  const [tools, setTools] = useState<ToolEntry[]>([]);
  const [modelOpen, setModelOpen] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const refreshConfigModel = useCallback(async () => {
    try {
      const mi = await api.getModelInfo(chatProfile || undefined);
      if (mi.model) {
        setConfigInfo({ model: mi.model, provider: mi.provider });
      }
    } catch {
      // best-effort
    }
  ...
  useEffect(() => {
    ...
    const offSessionInfo = gw.on<SessionInfo>("session.info", (ev) => {
      if (ev.payload) {
        // Sidecar runs on the dashboard gateway (global config). Keep only
        // non-model fields — the PTY gateway owns the live model badge.
        setSidecarInfo((prev) => ({
          ...prev,
          cwd: ev.payload?.cwd ?? prev.cwd,
          credential_warning:
            ev.payload?.credential_warning ?? prev.credential_warning,
        }));
      }
    });
    ...
    // Adopt whichever session the gateway hands us for model badge / info.
    gw.connect()
      .then(() => {
        if (cancelled) {
          return;
  ...
  const applyPickedModel = useCallback(
    (provider: string, model: string) => {
      setPtyInfo({ model, provider });
    },
    [],
  );

  const canPickModel = state === "open";
  const info = useMemo(
    () => ({
      ...sidecarInfo,
      ...configInfo,
    ...
    [sidecarInfo, configInfo, ptyInfo],
  );
  ...
  return (
    ...
    >
      ...
      {modelOpen && canPickModel && (
        <ModelPickerDialog
          loader={() => api.getModelOptions(chatProfile || undefined)}
          onApply={async ({
            ...
            const profileName = persistGlobal
              ? undefined
              : (chatProfile || getManagementProfile() || undefined);
            const result = await api.setModelAssignment(
              ...
              persistGlobal ? { skipProfileScope: true } : undefined,
            );
            if (result?.confirm_required) return result;
            await refreshConfigModel();
            applyPickedModel(provider, model);
          }}
          promptSessionRefresh={!!onSessionModelRefresh}
          onSessionRefresh={(args) => {
            applyPickedModel(args.provider, args.model);
            onSessionModelRefresh?.(args);
          }}
