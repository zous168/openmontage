...
import { cn, hubSidebarLink, hubSidebarSection } from "@/lib/utils";
import { Backdrop } from "@/components/Backdrop";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayStatusText } from "@/components/SidebarStatusStrip";
...
const BUILTIN_NAV_REST: NavItem[] = [
  ...
  { path: "/seat", label: "坐席接待", icon: Headset },
  { path: "/memory", labelKey: "memory", label: "Memory", icon: Brain },
  { path: "/files", label: "Files", icon: FolderOpen },
  { path: "/database", labelKey: "database", label: "Database", icon: Database },
  {
    path: "/analytics",
    labelKey: "analytics",
    label: "Analytics",
...
/** Sidebar scope grouping — visual only; does not change API behavior. */
const NAV_SCOPE_SECTIONS: {
  id: "runtime" | "agent" | "global";
  labelKey: "navSectionRuntime" | "navSectionAgent" | "navSectionGlobal";
  paths: readonly string[];
}[] = [
  {
    id: "runtime",
    labelKey: "navSectionRuntime",
    paths: ["/chat", "/sessions", "/seat", "/analytics", "/logs", "/memory"],
  },
  {
    id: "global",
    labelKey: "navSectionGlobal",
    paths: [
      "/config",
      "/env",
      "/models",
...
type NavScopeSectionDef = typeof NAV_SCOPE_SECTIONS[number];

interface NavScopeSection {
  id: "runtime" | "agent" | "global";
  labelKey: NavScopeSectionDef["labelKey"];
  items: NavItem[];
}

function resolveNavSectionLabel(
  t: Translations,
  labelKey: NavScopeSectionDef["labelKey"],
): string {
  switch (labelKey) {
    case "navSectionRuntime":
      return t.app.navSectionRuntime ?? "This instance";
    case "navSectionAgent":
      return t.app.navSectionAgent ?? "Profile data";
    case "navSectionGlobal":
      return t.app.navSectionGlobal ?? "Global";
  }
}

function groupCoreNavByScope(coreItems: NavItem[]): NavScopeSection[] {
  const byPath = new Map(coreItems.map((item) => [item.path, item]));
  const used = new Set<string>();
  const sections: NavScopeSection[] = [];

  for (const def of NAV_SCOPE_SECTIONS) {
    const items: NavItem[] = [];
    ...
    if (items.length > 0) {
      sections.push({ id: def.id, labelKey: def.labelKey, items });
    }
  }

  const orphans = coreItems.filter((item) => !used.has(item.path));
  if (orphans.length > 0) {
    const globalSection = sections.find((section) => section.id === "global");
    if (globalSection) {
      globalSection.items.push(...orphans);
    } else {
      sections.push({
        id: "global",
        labelKey: "navSectionGlobal",
...
function buildNavItems(
  ...
  for (const manifest of manifests) {
    ...
    const pluginItem: NavItem = {
      ...
      label: pluginTabLabels?.[manifest.name] ?? manifest.label,
...
function partitionSidebarNav(
  ...
  const merged = buildNavItems(builtIn, manifests, pluginTabLabels);
...
export default function App() {
  ...
  const sidebarNav = useMemo(
    () => partitionSidebarNav(builtinNav, manifests, t.pluginsPage.pluginTabLabels),
    [builtinNav, manifests, t.pluginsPage.pluginTabLabels],
  );
  const sidebarNavSections = useMemo(
    () => groupCoreNavByScope(sidebarNav.coreItems),
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography
          className="font-bold text-[0.95rem] leading-snug tracking-normal text-midground normal-case"
          ...
          >
            ...
            >
              ...
              >
                ...
                <Typography
                  className="font-bold text-[1.05rem] leading-snug tracking-normal text-midground normal-case"
                  style={{ mixBlendMode: "plus-lighter" }}
                >
                  {t.app.brand}
                </Typography>
            ...
            >
              {sidebarNavSections.map((section, sectionIdx) => (
                <div
                  key={section.id}
                  aria-labelledby={`hermes-sidebar-${section.id}-heading`}
                  className={cn(
                    "flex flex-col",
                    sectionIdx > 0 && "border-t border-current/10",
                  )}
                  role="group"
                >
                  <span
                    ...
                    id={`hermes-sidebar-${section.id}-heading`}
                  >
                    {resolveNavSectionLabel(t, section.labelKey)}
                  </span>
                    ...
                    isChatRoute ? (
                      ...
                      >
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <Spinner />
                          <span>{t.app.loadingChat ?? "Loading chat…"}</span>
                        </div>
                      </div>
                    ) : null
                  ) : (
    ...
    >
        ...
        className={({ isActive }) =>
          cn(
            ...
            isActive
              ? "bg-midground/10 text-midground font-medium"
          ...
          <>
            <Icon
              className={cn(
                ...
                isActive ? "text-midground" : "text-text-tertiary group-hover/nav:text-midground",
              )}
            />
            ...
            {isActive && (
              <span
                aria-hidden
                className="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-full bg-midground"
              />
            )}
          </>
        )}
  ...
  return createPortal(
    <span
      className={cn(
        ...
        "font-sans text-sm normal-case tracking-normal text-midground antialiased",
      )}
      style={{
