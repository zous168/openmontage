...
import { BUILTIN_THEMES, defaultTheme, DEFAULT_DASHBOARD_THEME_NAME } from "./presets";
import {
  FONT_CHOICES,
...
function isKnownThemeName(
  name: string,
  availableThemes: ThemeListEntry[],
  userThemeDefs: Record<string, DashboardTheme>,
): boolean {
  return (
...
function readStoredThemeName(): string {
  ...
  const stored =
    window.localStorage.getItem(STORAGE_KEY) ?? DEFAULT_DASHBOARD_THEME_NAME;
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  /** True once the user picks a theme/font this session — blocks late hydrate
   *  responses from undoing an in-flight selection. */
  ...
  useEffect(() => {
    ...
    api
      .getThemes()
      .then((resp) => {
        if (cancelled || preferenceTouchedRef.current) return;
        ...
        const themeCatalog = new Set<string>([
          ...Object.keys(BUILTIN_THEMES),
          ...(resp.themes?.map((t) => t.name) ?? []),
        ]);
        ...
        // Browser-local preference diverged from server (e.g. a prior PUT
        // failed silently). Push local → server once instead of clobbering
        // the user's pick on every refresh.
        if (
          stored !== migratedActive &&
          themeCatalog.has(stored)
        ) {
          api
            .setTheme(stored)
            .then(() => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((resp) => {
        ...
        const stored = readStoredFontId();

        const adoptServer = () => {
          if (cancelled || preferenceTouchedRef.current) return;
          setFontId(serverId);
          _ACTIVE_FONT_OVERRIDE = serverId;
          if (typeof window !== "undefined") {
            window.localStorage.setItem(FONT_STORAGE_KEY, serverId);
          }
        };

        if (stored !== serverId && getFontChoice(stored)) {
          api
            .setFontPref(stored)
            .then(() => {
              if (cancelled || preferenceTouchedRef.current) return;
              setFontId(stored);
              _ACTIVE_FONT_OVERRIDE = stored;
              if (typeof window !== "undefined") {
                window.localStorage.setItem(FONT_STORAGE_KEY, stored);
              }
            })
            .catch(() => adoptServer());
          return;
  ...
  const setTheme = useCallback(
    (name: string) => {
      ...
      const next = isKnownThemeName(name, availableThemes, userThemeDefs)
        ? name
        : DEFAULT_DASHBOARD_THEME_NAME;
      const previous = readStoredThemeName();
      setThemeName(next);
      if (typeof window !== "undefined") {
        window.localStorage.setItem(STORAGE_KEY, next);
      }
      void api.setTheme(next).catch(() => {
        preferenceTouchedRef.current = false;
        setThemeName(previous);
        if (typeof window !== "undefined") {
          window.localStorage.setItem(STORAGE_KEY, previous);
        }
        console.warn("[hermes] Failed to persist dashboard theme:", next);
      });
    },
  ...
  const setFont = useCallback((id: string) => {
    ...
    const previous = readStoredFontId();
    setFontId(next);
    _ACTIVE_FONT_OVERRIDE = next;
    if (typeof window !== "undefined") {
      window.localStorage.setItem(FONT_STORAGE_KEY, next);
    }
    void api.setFontPref(next).catch(() => {
      preferenceTouchedRef.current = false;
      setFontId(previous);
      _ACTIVE_FONT_OVERRIDE = previous;
      if (typeof window !== "undefined") {
        window.localStorage.setItem(FONT_STORAGE_KEY, previous);
      }
      console.warn("[hermes] Failed to persist dashboard font:", next);
    });
  }, []);
