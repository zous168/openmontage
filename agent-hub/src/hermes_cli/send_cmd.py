...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        print("channel discovery can populate ~/.marketing_hub/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.marketing_hub/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.marketing_hub/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.marketing_hub/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.marketing_hub/.env and ~/.marketing_hub/config.yaml into os.environ so the
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.marketing_hub/.env + ~/.marketing_hub/config.yaml) — no LLM, "
