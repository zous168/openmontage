...
# 主模型选择器不展示（仍可在 config / CLI 显式指定）
PICKER_EXCLUDED_PROVIDERS: frozenset[str] = frozenset({
    "gemini",
    "google-gemini-cli",
    "xai",
    "xai-oauth",
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    # 系统内置（platform LLM 网关）：置顶，设备登录 + LLM_GATEWAY_BASE_URL 后可用
    ProviderEntry("official",       "系统内置模型",             "系统内置模型（平台官方渠道，登录后可用，按算力计费）"),
    ProviderEntry("openrouter",     "OpenRouter",               "OpenRouter (Pay-per-use API aggregator)"),
    ProviderEntry("lmstudio",       "LM Studio",                "LM Studio (Local desktop app with built-in model server)"),
    ProviderEntry("alibaba",        "Qwen Cloud",               "Qwen Cloud / DashScope (Qwen + multi-provider)"),
    ProviderEntry("moark",          "模力方舟 (Gitee AI)",      "模力方舟 — Gitee AI OpenAI-compatible API (moark.com)"),
...
try:
    ...
    for _pp in _list_providers_for_canonical():
        if _pp.name in _canonical_slugs:
            continue
        if _pp.name in PICKER_EXCLUDED_PROVIDERS:
            continue
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.marketing_hub/.env``.
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "official":
        try:
            ...
            return fetch_official_gateway_models()
        except Exception:
            return []
    if normalized == "openrouter":
