...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Hermes Agent web dashboard for managing config, API keys, "
            "and sessions.\n\n"
            "Marketing Hub: do NOT use this in the integrated checkout — start "
            "``backend/scripts/start_dev.cmd`` (:8000, ``src.main:app``) instead. "
            "Standalone ``hermes dashboard`` on :9119 lacks integrated login, IPC, "
            "and device auth APIs. See ``hermes_cli.main`` module docstring."
        ),
    ...
    dashboard_parser.add_argument(
        ...
        default=9119,
        help=(
            "Port (default 9119). Not used in Marketing Hub integrated dev — "
            "use Hub :8000 via backend/scripts/start_dev.cmd"
        ),
    )
    dashboard_parser.add_argument(
    ...
    # client with Nous Portal and write the client_id into ~/.marketing_hub/.env.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.marketing_hub/.env, and prints "
