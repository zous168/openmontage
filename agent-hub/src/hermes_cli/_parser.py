...
Marketing Hub integrated checkout:
    Do NOT run ``hermes dashboard`` — use backend/scripts/start_dev.cmd (:8000).
    Hermes is mounted at / on src.main:app with integrated login and /api/auth/*.
...
def build_top_level_parser():
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.marketing_hub/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.marketing_hub/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
