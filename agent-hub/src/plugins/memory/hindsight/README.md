...
echo "HINDSIGHT_API_KEY=your-key" >> ~/.marketing_hub/.env
```
...
Daemon startup logs: `~/.marketing_hub/logs/hindsight-embed.log`
...
Config file: `~/.marketing_hub/hindsight/config.json`
...
The LLM API key is stored in `~/.marketing_hub/.env` as `HINDSIGHT_LLM_API_KEY`.
