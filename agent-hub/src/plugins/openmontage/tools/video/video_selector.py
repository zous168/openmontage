...
class VideoSelector(BaseTool):
    ...
    def execute(self, inputs: dict[str, object]) -> ToolResult:
        ...
        if profile in {"ugc_native", "ugc_native_executable"}:
            from plugins.openmontage.lib.video_prompt_validator import (
        ...
        if tool is None:
            from plugins.openmontage.tools.video._shared import (
        ...
        if adapted.get("operation") == "image_to_video" and adapted.get("reference_image_path"):
            ...
            if "image_url" in tool_props and "image_url" not in adapted:
                try:
                    from plugins.openmontage.tools.video._shared import upload_image_fal
    ...
    def _select_best_tool(
        ...
        """
        from plugins.openmontage.lib.scoring import rank_providers, ProviderScore
        from plugins.openmontage.tools.video._shared import LOCAL_VIDEO_PROVIDER_ENV_MAP
