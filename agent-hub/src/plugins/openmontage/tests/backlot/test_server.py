...
from plugins.openmontage.backlot import server as server_mod
from plugins.openmontage.backlot import state as state_mod
from plugins.openmontage.backlot import app_settings as app_settings_mod
...
class TestBacklotServerApi:
    ...
    def test_reference_url_extracts_from_douyin_share_text(self):
        from plugins.openmontage.backlot.bootstrap import normalize_media_url, validate_production_inputs
    ...
    def test_stage_media_upload_and_ingest(self, client, projects_root, tmp_path, monkeypatch):
        import plugins.openmontage.backlot.bootstrap as boot
    ...
    def test_patch_system_pipeline_ui(self, client, tmp_path, monkeypatch):
        import yaml
        from plugins.openmontage.lib import pipeline_loader as pl
        ...
        monkeypatch.setattr("plugins.openmontage.backlot.pipeline_admin.PIPELINE_DEFS_DIR", defs)
        monkeypatch.setattr("plugins.openmontage.backlot.bootstrap.PIPELINE_DEFS_DIR", defs)
    ...
    def test_get_pipeline_config_and_stage_skill(self, client, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pl, "PIPELINE_DEFS_DIR", defs)
        monkeypatch.setattr("plugins.openmontage.backlot.pipeline_admin.PIPELINE_DEFS_DIR", defs)
        monkeypatch.setattr("plugins.openmontage.backlot.pipeline_admin._SKILLS_ROOT", tmp_path / "skills")
    ...
    def test_pipeline_stage_structure_crud(self, client, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.openmontage.backlot.pipeline_admin.PIPELINE_DEFS_DIR", defs)
        monkeypatch.setattr("plugins.openmontage.backlot.pipeline_admin._SKILLS_ROOT", tmp_path / "skills")
        monkeypatch.setattr("plugins.openmontage.backlot.pipeline_admin._ARTIFACTS_SCHEMA_DIR", tmp_path / "schemas" / "artifacts")
    ...
    def test_patch_system_env_vars(self, client, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.openmontage.backlot.env_config.ENV_PATH", env_file)
        monkeypatch.setattr("plugins.openmontage.backlot.env_config.ENV_EXAMPLE_PATH", example_file)
