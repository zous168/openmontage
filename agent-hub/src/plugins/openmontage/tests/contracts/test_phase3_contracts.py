...
from plugins.openmontage.schemas.artifacts import list_schemas
from plugins.openmontage.styles.playbook_loader import load_playbook, list_playbooks, validate_playbook
from plugins.openmontage.tools.base_tool import ToolTier, ToolStatus
...
from plugins.openmontage.tools.audio.piper_tts import PiperTTS
...
from plugins.openmontage.tools.audio.google_music import GoogleMusic
from plugins.openmontage.tools.video.veo_video import VeoVideo
...
class TestGoogleCredentials:
    def test_get_genai_client_with_google_api_key(self):
        from plugins.openmontage.tools.google_credentials import get_genai_client
...
class TestVeoVideo:
    ...
    @patch("plugins.openmontage.tools.video._shared.probe_output")
...
class TestRemotionScaffold:
    # Remotion 跟数据根走（要 npm install），不在插件代码目录里。
    from plugins.openmontage.lib.paths import COMPOSER_DIR as REMOTION_DIR
