...
from plugins.openmontage.lib.assets_composition import resolve_assets_composition_strategy
...
def test_force_ugc_native_overrides_fallback(monkeypatch):
    monkeypatch.setattr(
        "plugins.openmontage.tools.video._shared.static_composition_fallback_active",
        lambda: True,
    )
    assert (
