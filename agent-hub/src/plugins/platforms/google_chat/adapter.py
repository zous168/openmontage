...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _hub = (_os.environ.get("HUB_DATA_DIR") or "").strip()
            _hermes_home = _Path(_hub) if _hub else _Path.home() / ".marketing_hub"
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        # 走 override-aware 的 get_hermes_home()（进程内多 profile 下按 ContextVar
        # 覆盖解析），而非裸读进程级 HERMES_HOME 环境变量。
        from hermes_constants import get_hermes_home

        return _Path(str(get_hermes_home())) / "google_chat_bot_id.json"
...
def interactive_setup() -> None:
    ...
    prompt for env vars, persist them to ``~/.marketing_hub/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.marketing_hub/.env")
