    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            try:
                ...
                hermes_home = get_default_hermes_root().resolve()
            except Exception:
                hermes_home = Path.home().joinpath(".marketing_hub").resolve()
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.marketing_hub/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.marketing_hub/.env")
        return
