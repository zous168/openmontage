...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.marketing_hub/.env")
...
def register(ctx) -> None:
    ...
    # LT-006.04.03：经 ServiceRegistry 暴露出站投递器类（teams.outbound.writer@1），
    # 供 teams_pipeline 解析（消 plugin↔plugin 直 import）。直接用进程级注册表，
    # 不依赖 ctx.provide_service 是否在当前加载上下文可用。
    from core.contracts.teams_outbound import TEAMS_OUTBOUND_WRITER
    from core.service_registry import service_registry

    service_registry.provide(
        TEAMS_OUTBOUND_WRITER, TeamsSummaryWriter, plugin="teams-platform"
    )
