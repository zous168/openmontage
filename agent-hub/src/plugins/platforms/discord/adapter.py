    ...
    def pcm_to_wav(pcm_data: bytes, output_path: str,
        ...
        if not ffmpeg:
            raise FileNotFoundError(
                "ffmpeg not found (MXAI_FFMPEG_EXE / imageio / PATH)"
            )
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.marketing_hub/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.marketing_hub/skills/ for new or removed skills")
