...
Credential storage mirrors every other Hermes channel:
    ...
    * runtime SDK creds  -> ``~/.marketing_hub/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.marketing_hub/auth.json`` under
...
def _auth_json_path() -> Path:
    """Resolve ``~/.marketing_hub/auth.json`` honouring the active Hermes profile."""
    try:
        ...
        try:
            ...
            return Path.home() / ".marketing_hub" / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.marketing_hub/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.marketing_hub/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.marketing_hub/.env`` (canonical runtime store).
