...
Runtime SDK credentials live in `~/.marketing_hub/.env` (the same place every other
...
Management metadata lives in `~/.marketing_hub/auth.json` under `credential_pool`:
