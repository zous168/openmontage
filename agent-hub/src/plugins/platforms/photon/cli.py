...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    #    and persist both (runtime creds -> ~/.marketing_hub/.env, ids -> auth.json).
    try:
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.marketing_hub/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.marketing_hub/.env)")
