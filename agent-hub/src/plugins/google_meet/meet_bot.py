...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        try:
            from plugins.mxai.content.ffmpeg_bin import find_ffmpeg as _find_ff
            ...
            try:
                ...
                proc = _sp.Popen(
                    [
                        ...
                        "-i", str(pcm_path),
                        "-f", "audiotoolbox",
                        "-audio_device_index", _mac_audio_device_index(device_name, ffmpeg),
                        "-",
                    ],
                    stdin=_sp.DEVNULL,
                    stdout=_sp.DEVNULL,
                ...
                state.set(error="ffmpeg not found — set MXAI_FFMPEG_EXE or install ffmpeg for realtime on macOS")
            except Exception as e:
...
def _mac_audio_device_index(device_name: str, ffmpeg_bin: str = "ffmpeg") -> str:
