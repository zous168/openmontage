...
def build_pipeline_runtime(gateway: Any) -> TeamsMeetingPipeline:
    ...
    if teams_config and teams_config.enabled and teams_delivery.get("enabled"):
        # LT-006.04.03：从 ServiceRegistry 解析 Teams 出站投递器类（platforms/teams 在
        # register 时 provide），不再直 import platforms（消 plugin↔plugin 交叉）。可选依赖——
        # 未 provide（platforms/teams 未加载）时优雅降级，等价旧 ImportError 路径。
        from core.contracts.teams_outbound import TEAMS_OUTBOUND_WRITER
        from core.service_registry import service_registry

        if service_registry.is_provided(TEAMS_OUTBOUND_WRITER):
            writer_cls = service_registry.get(TEAMS_OUTBOUND_WRITER)
            teams_sender = writer_cls(platform_config=teams_config)
        else:
            logger.debug(
