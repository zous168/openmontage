...
def _resolve_official_video_gateway():
    """官方算力渠道：llm-gateway /fal pass-through（无本机 FAL_KEY 时）。"""
    from tools.official_media_gateway import resolve_official_media_gateway

    return resolve_official_media_gateway()


def _submit_fal_video_via_official(endpoint: str, arguments: Dict[str, Any]):
    """经网关 pass-through 提交并轮询；返回与 ``handle.get()`` 同形的 dict。"""
    from tools.official_media_gateway import fal_queue_result, fal_queue_submit

    resolved = _resolve_official_video_gateway()
    if resolved is None:
        raise RuntimeError("Official video gateway is not available")
    base_url, jwt = resolved
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    """Submit a FAL video request using official gateway, direct credentials, or managed queue.

    Returns a request handle whose ``.get()`` blocks until the result is ready,
    **or** (official path) the completed result dict directly.
    """
    official = _resolve_official_video_gateway()
    if official is not None:
        # Marker object: generate() detects dict vs handle
        class _OfficialReady:
        ...
        return _OfficialReady(_submit_fal_video_via_official(endpoint, arguments))
...
def _check_fal_video_available() -> bool:
    """True if FAL video is reachable (BYOK key, official gateway, or managed)."""
    from tools.tool_backend_helpers import fal_key_is_configured

    if fal_key_is_configured():
        return True
    if _resolve_official_video_gateway() is not None:
        return True
...
class FALVideoGenProvider(VideoGenProvider):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "configure LLM_GATEWAY_BASE_URL and log in (official channel), "
        ...
        # Official gateway path uses httpx only — no fal_client package required.
        if _resolve_official_video_gateway() is None:
            try:
                _load_fal_client()
            except ImportError:
                return error_response(
