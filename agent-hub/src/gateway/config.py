...
# -----------------------------------------------------------------------------
def _clawbot_credentials_ready(cfg: PlatformConfig) -> bool:
    extra = cfg.extra or {}
    return bool(
        extra.get("bind_status")
        and str(cfg.token or extra.get("token") or "").strip()
        and str(extra.get("account_id") or "").strip()
    )


def platform_setup_hint(platform: Platform, config: PlatformConfig) -> str:
    """Human-readable reason when an enabled platform is not ready to connect."""
    extra = config.extra or {}
    if platform == Platform.CLAWBOT:
        if not extra.get("bind_status"):
            return "ClawBot: scan the QR code in Channels to bind WeChat first."
        if not str(config.token or extra.get("token") or "").strip():
            return "ClawBot: bind incomplete — rescan the QR code to obtain the bot token."
        if not str(extra.get("account_id") or "").strip():
            return "ClawBot: bind incomplete — missing account_id; rescan the QR code."
        return "ClawBot setup is incomplete."
    if platform == Platform.WEIXIN:
        if not str(config.token or extra.get("token") or "").strip():
            return "Weixin startup skipped: configure WEIXIN_TOKEN or platforms.weixin.token."
        if not str(extra.get("account_id") or "").strip():
            return "Weixin startup skipped: configure WEIXIN_ACCOUNT_ID or platforms.weixin.extra.account_id."
        return "Weixin setup is incomplete."
    return f"{platform.value} setup is incomplete."
...
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    ...
    Platform.CLAWBOT: lambda cfg: _clawbot_credentials_ready(cfg),
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.marketing_hub/config.yaml (primary user-facing config)
    3. ~/.marketing_hub/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
