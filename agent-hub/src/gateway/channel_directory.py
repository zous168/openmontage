...
~/.marketing_hub/channel_directory.json.  The send_message tool reads this file for
...
async def build_channel_directory(adapters: Dict[Any, Any]) -> Dict[str, Any]:
    ...
    directory = _merge_platform_runtime_fields(load_directory(), directory, "clawbot", ("context_token",))
...
def _merge_platform_runtime_fields(
    old_directory: Dict[str, Any],
    new_directory: Dict[str, Any],
    platform_name: str,
    fields: tuple[str, ...],
) -> Dict[str, Any]:
    """Rebuild 时保留各联系人上的运行时字段（如 ClawBot ``context_token``）。"""
    old_channels = {
        ch.get("id"): ch
        for ch in (old_directory.get("platforms") or {}).get(platform_name, [])
        if ch.get("id")
    }
    if not old_channels:
        return new_directory
    new_platforms = new_directory.setdefault("platforms", {})
    new_channels = new_platforms.setdefault(platform_name, [])
    for ch in new_channels:
        entry_id = ch.get("id")
        if not entry_id:
            continue
        old_ch = old_channels.get(entry_id)
        if not old_ch:
            continue
        for field in fields:
            value = old_ch.get(field)
            if value:
                ch[field] = value
    return new_directory


def patch_platform_channel_fields(
    platform_name: str,
    channel_id: str,
    *,
    name: str | None = None,
    ...
    **fields: Any,
    ...
    """增量更新某平台联系人条目（用于 ClawBot ``context_token`` 等热写路径）。"""
    ...
    try:
        ...
        logger.warning("Channel directory: failed to patch %s/%s: %s", platform_name, channel_id, e)


def get_platform_channel_field(
    platform_name: str,
    channel_id: str,
    field: str,
) -> Optional[str]:
...
def format_directory_for_display() -> str:
    ...
    try:
        from tools.send_message_tool import send_message_ready_platforms

        allowed = set(send_message_ready_platforms())
    except Exception:
        allowed = None
    ...
    if not any(platforms.values()):
        if allowed is not None and not allowed:
            return "No messaging platforms are currently enabled."
    ...
    if allowed:
        sample = sorted(allowed)[0]
        lines.append(f'Bare platform name (e.g. "{sample}") sends to the default recipient.')
    else:
        lines.append('Bare platform name sends to the default recipient for that platform.')
