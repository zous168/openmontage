...
``~/.marketing_hub`` but can be overridden via the environment variable.  This means
...
def _looks_like_gateway_process(pid: int) -> bool:
    ...
    patterns = (
        ...
        "hermes-gateway",
        "gateway/run.py",
        "gateway.run",  # frozen hub: ``python -m gateway.run`` → sys.argv
    )
...
def _record_looks_like_gateway(record: dict[str, Any]) -> bool:
    ...
    patterns = (
        ...
        "hermes gateway",
        "gateway/run.py",
        "gateway.run",  # frozen hub attached spawn
    )
    return any(pattern in cmdline for pattern in patterns)
...
def acquire_gateway_runtime_lock() -> bool:
    ...
    if _gateway_lock_handle is not None:
        logger.info(
            "[gateway.lifecycle] runtime_lock already_held pid=%d lock=%s",
            os.getpid(),
    ...
    if not _try_acquire_file_lock(handle):
        handle.close()
        existing = _read_gateway_lock_record(path)
        logger.warning(
            "[gateway.lifecycle] runtime_lock busy pid=%d lock=%s holder_record=%s",
            os.getpid(),
            path,
            existing,
        )
    ...
    logger.info(
        "[gateway.lifecycle] runtime_lock acquired pid=%d lock=%s record=%s",
        os.getpid(),
        path,
        record,
    )
...
def write_pid_file() -> None:
    ...
    logger.info(
        "[gateway.lifecycle] write_pid_file begin pid=%d path=%s hermes_home=%s argv=%s",
        os.getpid(),
        path,
        get_hermes_home(),
        payload.get("argv"),
    ...
    try:
        ...
        logger.error(
            "[gateway.lifecycle] write_pid_file race_lost pid=%d path=%s existing=%s",
            os.getpid(),
            path,
            existing,
        )
    ...
    try:
        ...
        logger.error(
            "[gateway.lifecycle] write_pid_file failed pid=%d path=%s error=%s",
            os.getpid(),
            path,
            exc,
        )
        ...
        raise
    logger.info(
        "[gateway.lifecycle] write_pid_file ok pid=%d path=%s",
        os.getpid(),
        path,
    )
...
def write_runtime_status(
    *,
    ...
    if running_pid is None:
        owns_status = _looks_like_gateway_process(os.getpid())
    else:
        owns_status = running_pid == os.getpid()
    ...
    elif platform is _UNSET and gateway_state is _UNSET and exit_reason is _UNSET and restart_requested is _UNSET and active_agents is _UNSET:
        return
    elif not owns_status:
        return
...
def runtime_status_owned_by_live_gateway(runtime: dict[str, Any] | None) -> bool:
    """True when ``runtime`` was written by the currently running gateway process."""
    ...
    if running_pid is None:
        return False
    try:
        return int(runtime.get("pid")) == running_pid
...
def probe_running_pid(
    pid_path: Optional[Path] = None,
    *,
    cleanup_stale: bool = True,
) -> dict[str, Any]:
    """Structured gateway presence probe for autostart logs / dashboards.

    Returns a JSON-serializable dict describing pid/lock paths, each check
    step, and the resolved status (``running`` | ``lock_inactive`` |
    ``lock_held_unidentified``).
    """
    ...
    probe: dict[str, Any] = {
        ...
        "pid_file_exists": resolved_pid_path.exists(),
        "lock_file_exists": resolved_lock_path.exists(),
        "lock_active": False,
        "candidate_pid": None,
        "matched_via": None,
        "status": "not_running",
    ...
    def _note(code: str, **extra: Any) -> None:
        entry: dict[str, Any] = {"code": code}
        entry.update(extra)
        probe["checks"].append(entry)

    lock_active = is_gateway_runtime_lock_active(resolved_lock_path)
    probe["lock_active"] = lock_active
    if not lock_active:
        _note("lock_inactive")
        if cleanup_stale:
            _note("cleanup_stale_metadata")
            _cleanup_invalid_pid_path(resolved_pid_path, cleanup_stale=cleanup_stale)
        probe["status"] = "lock_inactive"
        return probe

    for source, record in (
        ("pid_file", _read_pid_record(resolved_pid_path)),
        ("lock_file", _read_gateway_lock_record(resolved_lock_path)),
    ):
        if record is None:
            _note("no_record", source=source)
            continue

        pid = _pid_from_record(record)
        if pid is None:
            _note("invalid_record", source=source, record=record)
            continue

        probe["candidate_pid"] = pid

        if not _pid_exists(pid):
            _note("pid_not_alive", source=source, pid=pid)
            continue
        ...
        cmdline_ok = _looks_like_gateway_process(pid)
        record_ok = _record_looks_like_gateway(record)
        if cmdline_ok or record_ok:
            probe["candidate_pid"] = pid
            probe["matched_via"] = source
            probe["status"] = "running"
            _note(
                "accepted",
                source=source,
                pid=pid,
                via_cmdline=cmdline_ok,
                via_record=record_ok,
                argv=record.get("argv"),
            )
            return probe
    ...
    _note("lock_held_no_valid_candidate")
    probe["status"] = "lock_held_unidentified"
    return probe
...
def get_running_pid(
    ...
    *,
    ...
    """
    probe = probe_running_pid(pid_path, cleanup_stale=cleanup_stale)
    if probe["status"] == "running":
        return int(probe["candidate_pid"])
    return None
