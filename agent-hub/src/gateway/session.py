...
def build_session_key(
    ...
    ``profile`` 是该会话所属的业务 profile（如客服 ``customer_profile``），落进 key
    的第二段 ``agent:{profile}:...``，使不同 profile 的会话在共享 ``state.db`` 中天然
    区分、各自 scope 到自身人设/记忆。未指定时为 ``default``（历史等价于旧 ``main``）。
    ...
    """
    # 默认 profile 仍用历史段名 ``main``（保持旧 key 逐字节不变，避免与各处写死/
    # 解析 ``agent:main`` 的代码冲突）；指定了具体业务 profile 才用其名。
    profile = "main" if (not profile or profile == "default") else profile
    ...
    if source.chat_type == "dm":
        ...
        if dm_participant_id:
            if source.thread_id:
                return f"agent:{profile}:{platform}:dm:{dm_participant_id}:{source.thread_id}"
            return f"agent:{profile}:{platform}:dm:{dm_participant_id}"
        if source.thread_id:
    ...
    key_parts = [f"agent:{profile}", platform, source.chat_type]
...
class SessionStore:
    ...
    def _generate_session_key(self, source: SessionSource) -> str:
        ...
        profile = self.resolve_profile(source)
    ...
    def resolve_profile(self, source: SessionSource) -> str:
        """该 source 所属业务 profile（落进 session_key 第二段）。

        读渠道配置 ``platforms.<平台>.extra.profile``（如客服渠道配 ``customer_profile``），
        缺省 ``default``。会话级指定（如 web 浮窗按会话）由 api_server 侧经 session_key
        透传，本期渠道级足够。
        """
        # CR-133：clawbot/wecom 作为标准消息通道，profile 由 platforms.<平台>.extra.profile
        # 配置驱动（下方），不再走 mxai 助理特判旁路。
        try:
            platform_cfg = self.config.platforms.get(source.platform)
            if platform_cfg is not None:
                prof = (platform_cfg.extra.get("profile") or "").strip()
                if prof:
                    return prof
        except Exception:
            pass
