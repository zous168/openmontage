...
# OpenAPI / Swagger（本机 api_server 活契约；路径级描述，非全量 schema）
_API_SERVER_OPENAPI_ROUTES: tuple[tuple[str, str, str], ...] = (
    ("get", "/health", "健康检查"),
    ("get", "/health/detailed", "详细健康状态"),
    ("get", "/v1/health", "OpenAI 兼容健康检查"),
    ("get", "/v1/models", "可用模型列表"),
    ("get", "/v1/capabilities", "API 能力清单"),
    ("get", "/v1/skills", "已安装 skills"),
    ("get", "/v1/toolsets", "工具集列表"),
    ...
    ("get", "/v1/runs/{run_id}/events", "Run 事件流（SSE）"),
    ("post", "/v1/runs/{run_id}/approval", "Run 审批"),
    ...
    ("post", "/api/sessions/{session_id}/fork", "分叉会话"),
    ("post", "/api/sessions/{session_id}/chat", "会话聊天"),
    ("post", "/api/sessions/{session_id}/chat/stream", "会话聊天（流式）"),
    ("post", "/api/sessions/{session_id}/chat/approval", "会话聊天审批"),
    ("get", "/api/jobs", "列出 Cron Jobs"),
    ("post", "/api/jobs", "创建 Cron Job"),
    ("get", "/api/jobs/{job_id}", "读取 Cron Job"),
    ...
    ("post", "/api/jobs/{job_id}/run", "手动触发 Cron Job"),
)

_API_SERVER_PUBLIC_PATHS: frozenset[str] = frozenset({
    "/health",
    "/health/detailed",
    "/v1/health",
    "/openapi.json",
...
def build_api_server_openapi_spec(*, host: str = DEFAULT_HOST, port: int = DEFAULT_PORT) -> dict[str, Any]:
    """生成 api_server OpenAPI 3.0 文档（路径 + 摘要；业务接口需 Bearer）。"""
    server_url = f"http://{host}:{port}"
    paths: dict[str, Any] = {}
    for method, path, summary in _API_SERVER_OPENAPI_ROUTES:
        op: dict[str, Any] = {
            "summary": summary,
            "responses": {"200": {"description": "OK"}},
        }
        if path not in _API_SERVER_PUBLIC_PATHS:
    ...
    return {
        ...
        "info": {
            ...
            "description": (
                "Agent OpenAI 兼容 HTTP 面（内部端口，默认 18789）。"
        ...
        "components": {
            "securitySchemes": {
                "bearerAuth": {
                    "type": "http",
                    "scheme": "bearer",
                    "description": "API_SERVER_KEY（.env 或 gateway 配置）",
                }
            }
        },
        "paths": paths,
...
def render_api_server_swagger_html() -> str:
    """Swagger UI HTML（CDN，指向同端口 /openapi.json）。"""
    return """<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8"/>
  <title>Hermes API Server — Swagger</title>
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Active approval session key for in-flight session chat streams
        # (session_id → gateway session key used by tools.approval).
        self._session_chat_approval_keys: Dict[str, str] = {}
        self._session_chat_agents: Dict[str, Dict[str, Any]] = {}
        self._session_db: Optional[Any] = None  # Lazy-init SessionDB for session continuity（default / 全局根回退）
        # per-profile SessionDB 缓存：profile 名 → SessionDB（落 profiles/{profile}/state.db）。
        # 渠道会话物理隔离（LT-031 L3-A）；profile 数有界（渠道固定），惰性建、进程常驻。
        self._session_db_cache: Dict[str, Any] = {}
    ...
    def _parse_profile_header(self, request: "web.Request") -> str:
        """读取 ``X-Hermes-Profile``（业务 profile 名，如 ``customer_profile``）。

        端点已由 ``_check_auth`` 鉴权，此处只做轻量合法性校验；缺省 / 非法一律降级
        为 ``default``（不报错）。仅允许与 profiles 目录名一致的安全字符。
        """
        raw = (request.headers.get("X-Hermes-Profile", "") or "").strip()
    ...
    def _profile_scope(self, profile: str):
        """该 profile 的 ``HERMES_HOME`` 作用域（非 default 才 set 覆盖）。

        **必须包在执行线程内**：``_run_agent`` 的 ``_run`` 经 ``run_in_executor(None,
        _run)`` 跑（不 ``copy_context``），故覆盖须在该线程里设置，方能被 Agent 构造、
        ``run_conversation`` 及其经 ``propagate_context_to_thread`` 派生的后台线程读到。
        会话仍落显式注入的共享 ``self._session_db``（显式注入胜过覆盖解析）。
        """
        ...
        if profile and profile != "default":
            ...
            return hermes_profile_scope(get_profile_dir(profile))
        return nullcontext()
    ...
    def _ensure_session_db(self, profile: Optional[str] = None):
        """Lazily initialise and return a SessionDB instance, **按 profile 物理隔离**。

        契约（LT-031 L3-A）：
        - ``profile in (None, "", "default")`` → 全局根 ``SessionDB()``（``state.db``）。
          这是 CLI / gateway / A-Main assistant 既有会话的归宿，**default == 全局根
          HERMES_HOME**（``get_profile_dir("default")`` 已核实返回全局根），不破坏其兼容。
        - 其它渠道 profile（douyin / wechat / …）→ ``profiles/{profile}/state.db``，
          经 ``get_profile_dir(profile)`` 解析；``SessionDB.__init__`` 自建父目录。

        每个 profile 的 SessionDB 进程常驻缓存（profile 数有界，渠道固定），惰性建。
        ``hermes sessions list`` 仍可见全局根会话；渠道会话各自落隔离库。
        """
        ...
        cached = self._session_db_cache.get(key)
        if cached is not None:
            return cached
        try:
            from hermes_state import SessionDB
            # lazy import 以便测试 monkeypatch ``hermes_cli.profiles.get_profile_dir`` 生效
            from hermes_cli.profiles import get_profile_dir

            db = SessionDB(db_path=get_profile_dir(key) / "state.db")
            self._session_db_cache[key] = db
            return db
        except Exception as e:
            logger.debug("per-profile SessionDB unavailable for %s: %s", key, e)
            # 回退全局根，保证可用性（与 default 同库，行为不破坏）。
            return self._ensure_session_db(None)
    ...
    def _create_agent(
        ...
        agent = AIAgent(
            ...
            **runtime_kwargs,
            ...
            session_db=self._ensure_session_db(profile),
    ...
    async def _handle_openapi(self, request: "web.Request") -> "web.Response":
        """GET /openapi.json — OpenAPI 活契约（免鉴权，本机 loopback）。"""
        spec = build_api_server_openapi_spec(host=self._host, port=self._port)
        return web.json_response(spec)

    async def _handle_swagger_docs(self, request: "web.Request") -> "web.Response":
        """GET /docs — Swagger UI（免鉴权）。"""
        return web.Response(
            text=render_api_server_swagger_html(),
    ...
    @staticmethod
    def _profile_from_session_id(session_id: Optional[str]) -> Optional[str]:
        """从渠道 session_id ``mxai-{profile}-inbound-*`` 反解 profile（LT-031 L3-A）。

        匹配则返回 profile（用于定位 ``profiles/{profile}/state.db``）；非渠道 session
        （CLI / gateway / OpenAI 兼容路径）返回 ``None`` → ``_ensure_session_db`` 回退全局根。
        """
        if not session_id:
            return None
        sid = str(session_id)
        if sid in (
            "mxai-assistant-floating-chat",
            "mxai-assistant-clawbot-chat",
            "mxai-assistant-wecom-chat",
        ):
            return "assistant"
        m = re.match(r"^mxai-(.+)-inbound-", sid)
        if m:
            return m.group(1)
        m = re.match(r"^mxai-(.+)-debug-", sid)
        if m:
            return m.group(1)
        return None

    def _session_profile(self, request: "web.Request", session_id: Optional[str]) -> Optional[str]:
        """会话归属 profile：**优先 ``X-Hermes-Profile`` header**（与建会话/列表/运行同口径），
        header 缺省 / ``default`` 时回退 ``session_id`` 命名反解。

        修复路由不一致：原查会话/history/persist 仅按 ``mxai-{profile}-inbound-`` 正则反解，
        致 A-Main 助理（``mxai-assistant-floating-chat``）与 AI 调试（``-debug-``）等非该规约
        会话查不到对应 ``profiles/{profile}/state.db`` → chat/stream 404（建会话按 header 落
        profile 库、查会话按正则落全局根，两库不一致）。header-first 使「建」「查」同库；
        无 header（CLI / OpenAI 兼容）仍按 session_id 自描述回退，零回归。
        """
        prof = self._parse_profile_header(request)
        if prof and prof != "default":
            return prof
        return self._profile_from_session_id(session_id)

    def _get_existing_session_or_404(
        self, request: "web.Request", session_id: str
    ) -> tuple[Optional[Dict[str, Any]], Optional["web.Response"]]:
        db = self._ensure_session_db(self._session_profile(request, session_id))
        if db is None:
    ...
    def _conversation_history_for_session(self, request: "web.Request", session_id: str) -> List[Dict[str, Any]]:
        db = self._ensure_session_db(self._session_profile(request, session_id))
        if db is None:
            return []
        try:
    ...
    def _ensure_session_turn_persisted(
        self,
        request: "web.Request",
        ...
        *,
        ...
        """保证本轮 user+assistant 落库到 session_db（``/api/sessions/{id}`` = 持久化会话）。

        AIAgent 的 ``_flush_messages_to_session_db`` 以 ``len(conversation_history)`` 作为
        起始下标 flush 本轮新消息。本路径每轮新建 agent 并传入**完整历史**作为
        ``conversation_history``；长会话触发上下文压缩后，agent 的工作 ``messages`` 会被
        压短到该下标之前，``messages[start_idx:]`` 变空 → **本轮 assistant 漏写**（实测仅
        user 落库，坐席页/重开看不到 AI 回复）。这里在每轮结束补齐缺失的 user/assistant，
        保证历史完整。短会话已由 agent 正常落库 → 命中去重、跳过，不重复写。
        """
        db = self._ensure_session_db(self._session_profile(request, session_id))
        if db is None or not session_id:
            return
        try:
            tail = db.get_messages_as_conversation(session_id)[-6:]
        except Exception:
            tail = []

        def _present(role: str, text: str) -> bool:
            t = (text or "").strip()
            return bool(t) and any(
                isinstance(m, dict)
                and m.get("role") == role
                and (m.get("content") or "").strip() == t
                for m in tail
            )

        try:
            stored_user = persist_user_message if isinstance(persist_user_message, str) and persist_user_message.strip() else user_message
            if isinstance(stored_user, str) and stored_user.strip() and not _present("user", stored_user):
                db.append_message(session_id=session_id, role="user", content=stored_user)
            if isinstance(assistant_text, str) and assistant_text.strip() and not _present("assistant", assistant_text):
                db.append_message(session_id=session_id, role="assistant", content=assistant_text)
        except Exception as exc:  # noqa: BLE001
            logger.warning("[api_server] session-chat persist guard failed: %s", exc)

    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Hermes sessions（按 X-Hermes-Profile 隔离）。

        渠道 profile header → 只返回该 profile 的 ``profiles/{profile}/state.db`` 会话；
        缺省 / default → 全局根（CLI / gateway / A-Main）。
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err

        db = self._ensure_session_db(self._parse_profile_header(request))
        if db is None:
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        ...
        # 新建会话落该 profile 库（渠道客户端 POST 带 X-Hermes-Profile）。
        db = self._ensure_session_db(self._parse_profile_header(request))
        if db is None:
            return web.json_response(_openai_error("Session database unavailable", code="session_db_unavailable"), status=503)
        ...
        session_source = str(raw_source).strip() if raw_source else "api_server"
        if not session_source:
            session_source = "api_server"
        # user_id：mxai 渠道客户端用它承载 customer_uid（客户↔session 关联，LT-031）。
        ...
        session = db.get_session(session_id) or {"id": session_id, "source": session_source, "model": model, "title": title}
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if auth_err:
            return auth_err
        session, err = self._get_existing_session_or_404(request, request.match_info["session_id"])
        if err:
            return err
    ...
    async def _handle_patch_session(self, request: "web.Request") -> "web.Response":
        ...
        session, err = self._get_existing_session_or_404(request, session_id)
        if err:
            return err
        body, err = await self._read_json_body(request)
        if err:
    ...
    async def _handle_session_messages(self, request: "web.Request") -> "web.Response":
        ...
        if auth_err:
            return auth_err
        session_id = request.match_info["session_id"]
        _, err = self._get_existing_session_or_404(request, session_id)
        if err:
            return err
        db = self._ensure_session_db(self._session_profile(request, session_id))
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        source, err = self._get_existing_session_or_404(request, source_id)
        if err:
            return err
        body, err = await self._read_json_body(request)
        if err:
            return err
        # fork 落与源会话同一 profile 库（header-first，回退 source_id 反解）。
        db = self._ensure_session_db(self._session_profile(request, source_id))
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        if err is not None:
            return err
        persist_message = body.get("persist_message")
        if persist_message is not None and not isinstance(persist_message, str):
            return web.json_response(
                _openai_error("persist_message must be a string", code="invalid_persist_message"),
                status=400,
            )
        system_prompt = body.get("system_message") or body.get("instructions")
        if system_prompt is not None and not isinstance(system_prompt, str):
            return web.json_response(_openai_error("system_message must be a string", code="invalid_system_message"), status=400)
        history = self._conversation_history_for_session(request, session_id)
        result, usage = await self._run_agent(
            ...
            session_id=session_id,
            gateway_session_key=gateway_session_key,
            profile=self._parse_profile_header(request),
            persist_user_message=persist_message,
        )
        effective_session_id = result.get("session_id") if isinstance(result, dict) else session_id
        final_response = result.get("final_response", "") if isinstance(result, dict) else ""
        self._ensure_session_turn_persisted(
            request,
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        seq = 0
        display_user_message = persist_message if isinstance(persist_message, str) and persist_message.strip() else user_message
        ...
        def _tool_progress(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs) -> None:
            if event_type == "reasoning.available":
                ...
                payload: Dict[str, Any] = {"message_id": message_id, "tool_name": tool_name, "preview": preview, "args": args}
                # CR-120 LT-034.04.1b：tool.completed/failed 一并转发 agent core 工具回调的
                # duration（秒，对齐 _make_run_event_callback round(...,3)）/is_error，供消费方
                # 据此填真实 tool_trace[].duration_ms 与 ok。纯增量加字段，事件名/既有字段不变。
                if event_type in {"tool.completed", "tool.failed"}:
                    if "duration" in kwargs and kwargs.get("duration") is not None:
                        payload["duration"] = round(kwargs["duration"], 3)
                    payload["is_error"] = bool(kwargs.get("is_error", False))
                _enqueue(event_name, payload)

        approval_session_key = gateway_session_key or session_id
        self._session_chat_approval_keys[session_id] = approval_session_key
        # FR-CHAT-07：持有 agent_ref，断连 / chat/stop 可 interrupt
        agent_ref: list = [None]
        self._session_chat_agents[session_id] = {
            "agent_ref": agent_ref,
            "run_id": run_id,
            "task": None,
        }
        ...
        async def _run_and_signal() -> None:
            interrupted = False
            try:
                await queue.put(_event_payload("run.started", {"user_message": {"role": "user", "content": display_user_message}}))
                ...
                result, usage = await self._run_agent(
                    ...
                    profile=self._parse_profile_header(request),
                    persist_user_message=persist_message,
                    approval_request_callback=_approval_notify,
                    agent_ref=agent_ref,
                )
                final_response = result.get("final_response", "") if isinstance(result, dict) else ""
                ...
                self._session_chat_approval_keys.pop(session_id, None)
                self._session_chat_agents.pop(session_id, None)
                self._active_run_agents.pop(run_id, None)
                await queue.put(_event_payload("done", {}))
                await queue.put(None)

        task = asyncio.create_task(_run_and_signal())
        entry = self._session_chat_agents.get(session_id)
        ...
        try:
            ...
            await self._interrupt_session_chat(session_id, reason="SSE client disconnected")
            task.cancel()
            raise
        except Exception as exc:
            logger.debug("[api_server] session SSE stream error: %s", exc)
            await self._interrupt_session_chat(session_id, reason="SSE stream error")
            task.cancel()
        return response

    async def _interrupt_session_chat(self, session_id: str, *, reason: str = "stop") -> bool:
        """Interrupt the active agent for a session chat/stream (FR-CHAT-07)."""
        entry = self._session_chat_agents.get(session_id) or {}
        agent_ref = entry.get("agent_ref")
        agent = None
        # agent 在线程内创建，短暂等待 ref 就绪
        for _ in range(50):
            if isinstance(agent_ref, list) and agent_ref and agent_ref[0] is not None:
                agent = agent_ref[0]
                break
        ...
        try:
            ...
            logger.debug("[api_server] session interrupt failed: %s", exc)
            return False
        task = entry.get("task")
        if task is not None and not task.done():
            task.cancel()
        logger.info("[api_server] interrupted session chat %s (%s)", session_id, reason)
        return True

    async def _handle_session_chat_stop(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/chat/stop — interrupt active stream (FR-CHAT-07)."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        session_id = request.match_info["session_id"]
        ok = await self._interrupt_session_chat(session_id, reason="Stop requested via API")
        return web.json_response({"session_id": session_id, "status": "stopping" if ok else "idle", "interrupted": ok})
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if provided_session_id:
            ...
            try:
                # 与本请求 agent 落库同一 profile 库（_run_agent 用同一 header profile）。
                db = self._ensure_session_db(self._parse_profile_header(request))
                if db is not None:
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            approval_token = None
            prev_user_message = os.environ.get("HERMES_SESSION_USER_MESSAGE")
            os.environ["HERMES_SESSION_USER_MESSAGE"] = user_message or ""
            ...
            try:
                if approval_request_callback and approval_session_key:
                    ...
                    register_gateway_notify(approval_session_key, approval_request_callback)
                ...
                if prev_user_message is None:
                    os.environ.pop("HERMES_SESSION_USER_MESSAGE", None)
                else:
                    os.environ["HERMES_SESSION_USER_MESSAGE"] = prev_user_message
                if approval_request_callback and approval_session_key:
                    try:
                        from tools.approval import (
                            reset_current_session_key,
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        # 渠道 profile（X-Hermes-Profile）→ per-profile state.db；缺省 default 落全局根。
        run_profile = self._parse_profile_header(request)
    ...
    async def _handle_session_chat_approval(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/chat/approval — resolve pending tool approval."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        approval_session_key = self._session_chat_approval_keys.get(session_id)
        ...
        try:
            ...
            logger.exception("[api_server] session chat approval failed for %s", session_id)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_get("/docs", self._handle_swagger_docs)
