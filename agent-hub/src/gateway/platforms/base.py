...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.marketing_hub/.env manually."
)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    secrets (``~/.marketing_hub/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.marketing_hub/.env,
    # ~/.marketing_hub/auth.json, etc. — so the obvious prompt-injection sites
