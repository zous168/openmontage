...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def _warn_once_no_ffmpeg(self) -> None:
        ...
        logger.warning(
            "[whatsapp_cloud] ffmpeg not found (MXAI_FFMPEG_EXE / imageio / PATH) — "
            "voice messages will be delivered as MP3 audio attachments instead of "
            "native voice notes (green waveform bubble). Install ffmpeg or set "
            "MXAI_FFMPEG_EXE to enable."
        )
