...
from gateway.platforms.outbound_queue import OutboundSendQueue
...
# Live Gateway adapters keyed by bot_id — reused by send_wecom_direct / send_message.
_LIVE_WECOM_ADAPTERS: Dict[str, "WeComAdapter"] = {}


def _wecom_live_adapter_key(bot_id: str) -> str:
    return str(bot_id or "").strip()


def _resolve_live_wecom_adapter(bot_id: str) -> Optional["WeComAdapter"]:
    """Return the in-process Gateway adapter when WS is up on this event loop."""
    key = _wecom_live_adapter_key(bot_id)
    if not key:
        return None
    live = _LIVE_WECOM_ADAPTERS.get(key)
    if live is None:
        return None
    ws = live._ws
...
def _queue_min_interval_seconds(extra: dict[str, Any]) -> float:
    ...
    if raw is None:
        raw = os.getenv("WECOM_OUTBOUND_MIN_INTERVAL_SECONDS", "2.0")
...
def _queue_enabled(extra: dict[str, Any]) -> bool:
    ...
    return os.getenv("WECOM_OUTBOUND_QUEUE_ENABLED", "1").strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._outbound_queue_enabled = _queue_enabled(extra)
        self._outbound_queue = OutboundSendQueue(
            min_interval_seconds=_queue_min_interval_seconds(extra),
            name="wecom-outbound",
        )

    async def _enqueue_outbound(self, deliver, *, chat_id: str) -> SendResult:
        if not self._outbound_queue_enabled:
            return await deliver()
        return await self._outbound_queue.enqueue(deliver, chat_id=chat_id)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if self._bot_id:
                _LIVE_WECOM_ADAPTERS[_wecom_live_adapter_key(self._bot_id)] = self
    ...
    async def disconnect(self) -> None:
        ...
        if self._bot_id:
            _LIVE_WECOM_ADAPTERS.pop(_wecom_live_adapter_key(self._bot_id), None)
        self._running = False
    ...
    async def _send_request(self, cmd: str, body: Dict[str, Any], timeout: float = REQUEST_TIMEOUT_SECONDS) -> Dict[str, Any]:
        ...
        self._last_outbound_req_id = req_id
    ...
    def _new_outbound_message_id(self) -> str:
        return f"{self.platform.value}-{uuid.uuid4().hex}"

    def _outbound_send_result(self, response: Dict[str, Any]) -> SendResult:
        """Build a user-facing message_id while preserving ws req_id for logs."""
        ws_req_id = str(getattr(self, "_last_outbound_req_id", "") or "").strip()
        raw: Any = response
        if ws_req_id and isinstance(response, dict):
            raw = {**response, "ws_req_id": ws_req_id}
        return SendResult(
            success=True,
            message_id=self._new_outbound_message_id(),
            raw_response=raw,
        )
    ...
    async def _on_message(self, payload: Dict[str, Any]) -> None:
        ...
        if self._bot_id:
            try:
                WeComTestSessionStore.for_bot(self._bot_id).record(
                    chat_id=chat_id,
                    reply_req_id=str(self._payload_req_id(payload) or ""),
                    chat_type="group" if is_group else "dm",
                )
            except Exception as exc:
                logger.debug("[%s] Failed to persist test session: %s", self.name, exc)
        try:
            from gateway.platforms.wecom.stats import record_wecom_received

            record_wecom_received(
    ...
    async def _deliver_send(
        ...
        try:
            from gateway.platforms.wecom.stats import record_wecom_replied

            record_wecom_replied()
        except Exception:
            pass
        return self._outbound_send_result(response)
    ...
    async def send(
        ...
        async def _deliver() -> SendResult:
            return await self._deliver_send(chat_id, content, reply_to=reply_to)

        return await self._enqueue_outbound(_deliver, chat_id=chat_id)

    @staticmethod
    def _resolve_api_chat_type(chat_type: str | int | None) -> Optional[int]:
        if chat_type is None or chat_type == "":
            return None
        if isinstance(chat_type, int):
            return chat_type if chat_type in {1, 2} else None
        normalized = str(chat_type).strip().lower()
        if normalized in {"dm", "single", "direct", "private", "1"}:
            return 1
    ...
    async def _deliver_proactive(
        ...
        content: str,
        *,
        chat_type: str | int | None = None,
    ) -> SendResult:
        """主动推送 markdown（``aibot_send_msg``），不依赖 inbound req_id。"""
        recipient = str(chat_id or "").strip()
        body = str(content or "").strip()
        if not recipient or not body:
            return SendResult(success=False, error="chat_id and message are required")

        payload: Dict[str, Any] = {
            "chatid": recipient,
            "msgtype": "markdown",
            "markdown": {"content": body[: self.MAX_MESSAGE_LENGTH]},
        }
        api_chat_type = self._resolve_api_chat_type(chat_type)
        if api_chat_type is not None:
            payload["chat_type"] = api_chat_type
        ...
        try:
            ...
            logger.error("[%s] Proactive send failed: %s", self.name, exc)
            return SendResult(success=False, error=str(exc))
    ...
    async def send_proactive(
        ...
        *,
        ...
        async def _deliver() -> SendResult:
            return await self._deliver_proactive(
                chat_id,
                content,
                chat_type=chat_type,
            )
...
# ---------------------------------------------------------------------------
# Dashboard channel test helpers
# ---------------------------------------------------------------------------

WECOM_TEST_MESSAGE = (
    "✅ 企业微信 AI Bot 通道测试\n"
    "收到此消息说明连接与发信正常，可以直接开始对话。"
)
WECOM_USER_MUST_MESSAGE_FIRST = (
    "请先在企微向机器人发送任意文字消息（如：你好），然后再试。"
)


class WeComTestSessionStore:
    """Disk-backed last inbound chat for dashboard channel test sends."""

    _FILENAME = "wecom-test-sessions.json"

    def __init__(self, bot_id: str, *, hermes_home: str | None = None):
        from hermes_constants import get_hermes_home

        self._bot_id = str(bot_id or "").strip()
        root = Path(hermes_home or get_hermes_home())
        self._path = root / self._FILENAME

    @classmethod
    def for_bot(cls, bot_id: str, *, hermes_home: str | None = None) -> "WeComTestSessionStore":
        return cls(bot_id, hermes_home=hermes_home)
    ...
    def record(
        ...
        *,
        ...
        normalized_chat_id = str(chat_id or "").strip()
        if not self._bot_id or not normalized_chat_id:
            return
        ...
        bots[self._bot_id] = {
            ...
            "reply_req_id": str(reply_req_id or "").strip(),
    ...
    def latest(self) -> dict[str, str] | None:
        if not self._bot_id:
            return None
        bots = self._read_all().get("bots")
        if not isinstance(bots, dict):
            return None
        entry = bots.get(self._bot_id)
        ...
        reply_req_id = str(entry.get("reply_req_id") or "").strip()
        if reply_req_id:
            result["reply_req_id"] = reply_req_id
        chat_type = str(entry.get("chat_type") or "").strip()
        if chat_type:
...
def resolve_wecom_test_target(*, bot_id: str) -> dict[str, str] | None:
    """Return the latest inbound chat recorded for this bot."""
    return WeComTestSessionStore.for_bot(bot_id).latest()


def _wecom_send_error_needs_user_initiate(error: str) -> bool:
...
async def send_wecom_direct(
    *,
    ...
    """Send via live Gateway adapter when available, else one-shot connect."""
    ...
    live = _resolve_live_wecom_adapter(bot_id)
    if live is not None:
        result = await live.send_proactive(recipient, body, chat_type=chat_type)
        if not result.success:
            return {"success": False, "error": result.error or "WeCom send failed"}
        ws_req_id = None
        if isinstance(result.raw_response, dict):
            ws_req_id = result.raw_response.get("ws_req_id")
        return {
            "success": True,
            "platform": "wecom",
            "chat_id": recipient,
    ...
    if not connected:
        return {
            "success": False,
            "error": adapter.fatal_error_message or "WeCom: failed to connect",
        }
    try:
        result = await adapter.send_proactive(recipient, body, chat_type=chat_type)
        if not result.success:
            return {"success": False, "error": result.error or "WeCom send failed"}
        ws_req_id = None
        if isinstance(result.raw_response, dict):
...
def _format_wecom_test_send_result(result: dict[str, Any]) -> tuple[bool, str]:
    if result.get("success"):
        return True, "测试消息已发送到企业微信，请查收。"
    error = str(result.get("error") or result)
    if _wecom_send_error_needs_user_initiate(error):
        return False, WECOM_USER_MUST_MESSAGE_FIRST
    return False, error


async def send_wecom_test_message_async(
    *,
    extra: dict[str, Any],
    chat_id: str,
    text: str = WECOM_TEST_MESSAGE,
    chat_type: str | int | None = None,
) -> tuple[bool, str]:
    """Send channel test ping via WeCom AI Bot WebSocket (async-safe)."""
    if not check_wecom_requirements():
        return False, "WeCom 依赖未满足，请安装 aiohttp 与 httpx。"
    ...
    try:
        ...
        logger.warning("WeCom test send failed: %s", exc, exc_info=True)
        return False, str(exc)

    return _format_wecom_test_send_result(result)


def send_wecom_test_message(
    *,
    extra: dict[str, Any],
    chat_id: str,
    text: str = WECOM_TEST_MESSAGE,
    ...
    """Sync wrapper for CLI/tests; safe when an event loop is already running."""
