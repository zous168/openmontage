...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.marketing_hub/.update_response. Set here so the cross-adapter gateway
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.marketing_hub/.update_response`` for the
    ...
    async def _convert_ffmpeg_to_wav(self, src_path: str, wav_path: str) -> Optional[str]:
        ...
        if not ffmpeg:
            logger.warning(
                "[%s] ffmpeg not found (MXAI_FFMPEG_EXE / imageio / PATH)",
                self._log_tag,
            )
            return None
        try:
    ...
    async def send_update_prompt(
        ...
        ``~/.marketing_hub/.update_response`` so the detached update process
        can read it.
        """
