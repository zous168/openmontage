...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, *, platform: Platform = Platform.WEIXIN):
        super().__init__(config, platform)
        extra = config.extra or {}
        hermes_home = str(get_hermes_home())
        self._hermes_home = hermes_home
        if platform == Platform.CLAWBOT:
            from gateway.platforms.clawbot.storage import (
                ClawbotContextTokenStore,
                load_clawbot_sync_buf,
                save_clawbot_sync_buf,
            )

            self._token_store = ClawbotContextTokenStore(hermes_home)
            self._sync_load = lambda: load_clawbot_sync_buf(hermes_home)
            self._sync_save = lambda buf: save_clawbot_sync_buf(hermes_home, buf)
        else:
            self._token_store = ContextTokenStore(hermes_home)
            self._sync_load = lambda: _load_sync_buf(hermes_home, self._account_id)
            self._sync_save = lambda buf: _save_sync_buf(hermes_home, self._account_id, buf)
        ...
        use_weixin_env = platform == Platform.WEIXIN
        self._account_id = str(
            extra.get("account_id")
            or (os.getenv("WEIXIN_ACCOUNT_ID", "") if use_weixin_env else "")
        ).strip()
        self._token = str(
            config.token or extra.get("token") or (os.getenv("WEIXIN_TOKEN", "") if use_weixin_env else "")
        ).strip()
        self._base_url = str(
            extra.get("base_url")
            or (os.getenv("WEIXIN_BASE_URL", ILINK_BASE_URL) if use_weixin_env else ILINK_BASE_URL)
        ).strip().rstrip("/")
        self._cdn_base_url = str(
            extra.get("cdn_base_url")
            or (os.getenv("WEIXIN_CDN_BASE_URL", WEIXIN_CDN_BASE_URL) if use_weixin_env else WEIXIN_CDN_BASE_URL)
        ).strip().rstrip("/")
        self._send_chunk_delay_seconds = float(
            extra.get("send_chunk_delay_seconds")
            or (os.getenv("WEIXIN_SEND_CHUNK_DELAY_SECONDS", "1.5") if use_weixin_env else "1.5")
        )
        self._send_chunk_retries = int(
            extra.get("send_chunk_retries")
            or (os.getenv("WEIXIN_SEND_CHUNK_RETRIES", "4") if use_weixin_env else "4")
        )
        self._send_chunk_retry_delay_seconds = float(
            extra.get("send_chunk_retry_delay_seconds")
            or (
                os.getenv("WEIXIN_SEND_CHUNK_RETRY_DELAY_SECONDS", "1.0")
                if use_weixin_env
                else "1.0"
            )
        )
        self._send_text_gate = asyncio.Lock()
        # When Clawbot outbound queue is active, rate-limit pacing is owned by the queue worker.
        self._defer_rate_limit_to_queue = False
        self._rate_limit_circuit_threshold = max(
            1,
            int(
                extra.get("rate_limit_circuit_threshold")
                or (
                    os.getenv("WEIXIN_RATE_LIMIT_CIRCUIT_THRESHOLD", "1")
                    if use_weixin_env
                    else "1"
                )
            ),
        ...
        self._rate_limit_circuit_window_seconds = float(
            ...
            or (
                os.getenv("WEIXIN_RATE_LIMIT_CIRCUIT_WINDOW_SECONDS", "30.0")
                if use_weixin_env
                else "30.0"
            )
        )
        self._rate_limit_circuit_open_seconds = float(
            ...
            or (
                os.getenv("WEIXIN_RATE_LIMIT_CIRCUIT_OPEN_SECONDS", "30.0")
                if use_weixin_env
                else "30.0"
            )
        )
        ...
        self._dm_policy = str(
            extra.get("dm_policy") or (os.getenv("WEIXIN_DM_POLICY", "open") if use_weixin_env else "open")
        ).strip().lower()
        self._group_policy = str(
            extra.get("group_policy")
            or (os.getenv("WEIXIN_GROUP_POLICY", "disabled") if use_weixin_env else "disabled")
        ).strip().lower()
        allow_from = extra.get("allow_from")
        if allow_from is None and use_weixin_env:
            allow_from = os.getenv("WEIXIN_ALLOWED_USERS", "")
        group_allow_from = extra.get("group_allow_from")
        if group_allow_from is None and use_weixin_env:
        ...
        self._split_multiline_messages = _coerce_bool(
            ...
            or (os.getenv("WEIXIN_SPLIT_MULTILINE_MESSAGES") if use_weixin_env else None),
            default=False,
        )
        ...
        if self._account_id and not self._token and self.platform != Platform.CLAWBOT:
    ...
    async def connect(self) -> bool:
        if not check_weixin_requirements():
            message = (
                "ClawBot startup failed: aiohttp and cryptography are required"
                if self.platform == Platform.CLAWBOT
                else "Weixin startup failed: aiohttp and cryptography are required"
            )
        ...
        if not self._token:
            message = (
                "ClawBot startup failed: complete QR bind (bot token missing in platforms.clawbot)"
                if self.platform == Platform.CLAWBOT
                else "Weixin startup failed: WEIXIN_TOKEN is required"
            )
        ...
        if not self._account_id:
            message = (
                "ClawBot startup failed: complete QR bind (account_id missing in platforms.clawbot)"
                if self.platform == Platform.CLAWBOT
                else "Weixin startup failed: WEIXIN_ACCOUNT_ID is required"
            )
            ...
            try:
                ...
                if ret not in {0, None} or errcode not in {0, None}:
                    if (ret == SESSION_EXPIRED_ERRCODE or errcode == SESSION_EXPIRED_ERRCODE
                        ...
                        # 重新绑定后旧 token 会过期；勿空睡 10 分钟挡消息，
                        # 交给 fatal→reconnect 用 config 里的新凭据重连。
                        logger.error(
                            "[%s] Session expired; disconnecting for reconnect",
                            self.name,
                        )
                        self._set_fatal_error(
                            "weixin_session_expired",
                            "Weixin/ClawBot session expired; reconnect required",
                            retryable=True,
                        )
    ...
    async def _process_message(self, message: Dict[str, Any]) -> None:
        ...
        if self.platform == Platform.CLAWBOT:
            try:
                ...
                record_clawbot_received(sender_id=sender_id)
            except Exception:
                logger.debug("[%s] clawbot received stat failed", self.name, exc_info=True)
    ...
    async def _send_text_chunk_locked(
        ...
        *,
        ...
        defer_queue = getattr(self, "_defer_rate_limit_to_queue", False)
        for attempt in range(self._send_chunk_retries + 1):
            ...
            if cooldown > 0:
                ...
                if self.platform == Platform.CLAWBOT:
                    logger.info(
                        "[%s] rate-limit cooldown %.1fs for %s; waiting before send",
                        self.name,
                        cooldown,
                        _safe_id(chat_id),
                    )
                        ...
                        if is_rate_limited:
                            ...
                            if self.platform == Platform.CLAWBOT:
                                wait = max(
                                    ...
                                    self._rate_limit_cooldown_remaining() or 0.0,
                                    30.0,
                                )
                                self._open_rate_limit_circuit()
                                logger.warning(
    ...
    def _new_outbound_client_id(self) -> str:
    ...
    async def _deliver_outbound(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = self._new_outbound_client_id()
            ...
            if self.platform == Platform.CLAWBOT:
                try:
                    from gateway.platforms.clawbot.stats import record_clawbot_replied

                    record_clawbot_replied()
                except Exception:
                    logger.debug("[%s] clawbot replied stat failed", self.name, exc_info=True)
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = self._new_outbound_client_id()
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
