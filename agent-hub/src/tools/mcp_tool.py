...
Configuration is read from ~/.marketing_hub/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.marketing_hub/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            data_root = (
                os.getenv("HUB_DATA_DIR", "").strip()
                or str(get_default_hermes_root())
            )
            hermes_home = os.path.expanduser(data_root)
            candidates = [
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ~/.marketing_hub/logs/mcp-stderr.log.
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.marketing_hub/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.marketing_hub/.env`` loaded at startup).
    """
    try:
