...
# resolved-absolute form is folded into the ~/.marketing_hub/ patterns at detection
...
_HERMES_DATA_DIR = r'\.(?:marketing_hub|hermes)'
_HERMES_ENV_PATH = (
    rf'(?:~/{_HERMES_DATA_DIR}/|'
    rf'(?:\$home|\$\{{home\}})/{_HERMES_DATA_DIR}/|'
    r'(?:\$hermes_home|\$\{hermes_home\})/)'
    r'\.env\b'
)
# ~/.marketing_hub/config.yaml IS the security policy: approvals.mode, yolo, and the
...
DANGEROUS_PATTERNS = [
    ...
    # In-place edit of a Hermes-managed security file (~/.marketing_hub/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.marketing_hub/ form so the Hermes config/env patterns catch it. In Docker and
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.marketing_hub/``.
    ...
    ``~/.marketing_hub/`` so the static ``_HERMES_CONFIG_PATH`` / ``_HERMES_ENV_PATH``
    ...
    try:
        ...
        display_prefix = display_hermes_home().rstrip("/") + "/"
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        ...
        # /home/user/.marketing_hub or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", display_prefix)
    return command
