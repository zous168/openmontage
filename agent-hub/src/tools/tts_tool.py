...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.marketing_hub/config.yaml``. Hermes
...
Configuration is loaded from ~/.marketing_hub/config.yaml under the 'tts:' key.
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.marketing_hub/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.marketing_hub/config.yaml.
...
def _ffmpeg_bin() -> Optional[str]:
    """与 Hub / Worker 同序：MXAI_FFMPEG_EXE → imageio-ffmpeg → PATH。"""
    try:
        from plugins.mxai.content.ffmpeg_bin import find_ffmpeg
...
def _convert_to_opus(mp3_path: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning("ffmpeg not found (MXAI_FFMPEG_EXE / imageio / PATH)")
    except Exception as e:
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.marketing_hub/cache/piper-voices/`` under the active
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.marketing_hub/config.yaml (tts: section).
