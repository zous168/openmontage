...
def _resolve_official_image_gateway():
    """官方算力渠道：llm-gateway image 面（无本机 FAL_KEY 时）。"""
    from tools.official_media_gateway import resolve_official_media_gateway

    return resolve_official_media_gateway()


def _generate_via_official_image_gateway(
    model_id: str,
    prompt: str,
    ...
    """经网关 ``/v1/images/generations``；返回 FAL 风格 ``{images: [{url,...}]}``。"""
    from tools.official_media_gateway import images_generations

    n = int(arguments.get("num_images") or arguments.get("n") or 1)
    extra = {
        k: v
        for k, v in arguments.items()
        if k not in {"prompt", "num_images", "n"} and v is not None
    }
    raw = images_generations(
        base_url=base_url,
        jwt=jwt,
        model=model_id,
        prompt=prompt,
    ...
    # OpenAI images 响应 → FAL images 列表
    ...
    if isinstance(data, list):
        for item in data:
            ...
            if isinstance(url, dict):
                url = url.get("url")
            if url:
                images.append({"url": url, "width": item.get("width", 0), "height": item.get("height", 0)})
    if not images and isinstance(raw.get("images"), list):
        images = [img for img in raw["images"] if isinstance(img, dict) and img.get("url")]
    if not images:
        raise ValueError("Official image gateway returned no image URLs")
    return {"images": images, "request_id": raw.get("id") or raw.get("request_id")}
...
def _postprocess_image_generate_result(raw: str, task_id: str | None = None) -> str:
    ...
    if not isinstance(image, str) or not _looks_like_absolute_file_path(image):
        if isinstance(image, str) and image.startswith(("http://", "https://")):
            payload.setdefault("preview_url", image)
        return raw

    from plugins.mxai.media import image_marker_from_cache_path

    marker = image_marker_from_cache_path(image)
    if marker:
        payload.setdefault("preview_marker", marker)
...
def image_generate_tool(
    ...
    try:
        ...
        if not (
            ...
            or _resolve_official_image_gateway()
        ):
        ...
        official = _resolve_official_image_gateway()
        if official is not None:
            base_url, jwt = official
            result = _generate_via_official_image_gateway(
                model_id, prompt.strip(), arguments, base_url, jwt
            )
        else:
...
def check_fal_api_key() -> bool:
    """True if FAL is reachable: BYOK FAL_KEY、官方网关、或 managed gateway。"""
    if fal_key_is_configured() or _resolve_managed_fal_gateway():
        return True
    try:
        return _resolve_official_image_gateway() is not None
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - official llm-gateway (device JWT)
    ...
    lines.append(
        "  - Or sign in with LLM_GATEWAY_BASE_URL configured "
        "(official channel; platform holds FAL_KEY)"
    )
    lines.append("")
    ...
    lines.append(
        "  2. Configure LLM_GATEWAY_BASE_URL and log in "
        "(official compute channel — no client FAL_KEY)"
    )
    if managed_nous_tools_enabled():
        lines.append(
            "  3. Sign in to a Nous account that has the managed FAL "
            "gateway enabled (`hermes setup`)"
        )
        lines.append(
            "  4. Configure a different image_gen provider via `hermes tools` "
...
def check_image_generation_requirements() -> bool:
    ...
    try:
        if check_fal_api_key():
            # Official gateway uses httpx only — no fal_client required.
            if _resolve_official_image_gateway() is not None and not fal_key_is_configured():
                if _resolve_managed_fal_gateway() is None:
                    return True
...
IMAGE_GENERATE_SCHEMA = {
    ...
    "description": (
        ...
        "path in the `image` field. Local cache files include `preview_marker` "
        "(`[mxai-image:filename]`) — append it verbatim to your reply so "
        "FloatingChat can show the image inline in the current session. "
        "You may also use markdown `![description](url-or-path)` or "
        "`MEDIA:/absolute/path` for gateway delivery."
    ),
    "parameters": {
        "type": "object",
        "properties": {
