...
def memory_tool(
    ...
    try:
        from hermes_cli.memory_routing import hub_memory_write_guard

        blocked = hub_memory_write_guard(action=action, target=target, content=content)
        if blocked:
...
def apply_memory_pending(payload: Dict[str, Any], store: "MemoryStore") -> Dict[str, Any]:
    ...
    try:
        ...
        blocked = hub_memory_write_guard(
...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "- User shares a stable preference, role, or business rule that applies across topics\n"
        "- You learn a durable project convention the user expects every session\n"
        "- You identify a stable fact that will be useful again in future sessions\n\n"
        "Do NOT save to MEMORY.md:\n"
        "- Terminal/shell debugging, WSL path workarounds, tool error messages, or one-off fixes\n"
        "- Task progress, session outcomes, completed-work logs, or temporary TODO state\n"
        "- Raw environment dumps easily re-discovered each session\n"
        "For technical notes retrieved on demand, use fact_store; for repeatable procedures, use skills.\n\n"
        "PRIORITY: User preferences and corrections > stable business/project rules.\n\n"
        "TWO TARGETS:\n"
        "- 'user': who the user is -- name, role, language/tone preferences\n"
        "- 'memory': stable cross-session notes -- business rules, project context "
        "(NOT tool debugging)\n\n"
