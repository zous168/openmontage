...
Copies bundled skills from the repo's skills/ directory into ~/.marketing_hub/skills/
...
The manifest lives at ~/.marketing_hub/skills/.bundled_manifest.
"""
...
# and by the installer's `--no-skills` flag (the default ~/.marketing_hub profile).
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.marketing_hub/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.marketing_hub/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.marketing_hub/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.marketing_hub) that wrote the
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.marketing_hub/skills/.curator_suppressed) is written when the curator
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.marketing_hub/skills/ ...")
