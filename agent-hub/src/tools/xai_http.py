...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.marketing_hub/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.marketing_hub/.env`` first, then ``os.environ``.
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    stored in ``~/.marketing_hub/.env`` (the standard Hermes location) are honored —
