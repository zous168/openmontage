...
#      saying "cat ~/.marketing_hub/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `cat ~/.marketing_hub/.env`, which tripped `read_secrets` and silently
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    # Only relative paths within ~/.marketing_hub/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.marketing_hub/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.marketing_hub/scripts/ and use just the filename."
        )
