...
Delivers text/media to enabled messaging platforms. The tool schema and
action='list' output only include platforms that are configured and ready
...
def _root_hermes_scope() -> Iterator[Path]:
    """Scope config reads to gateway SSOT under the data root, not a profile dir."""
    from agent.profile_scope import hermes_profile_scope
    from hermes_constants import get_default_hermes_root, get_hermes_home

    root = get_default_hermes_root()
    try:
        at_root = get_hermes_home().resolve() == root.resolve()
    except OSError:
        at_root = get_hermes_home() == root
    if at_root:
        yield root
        return
...
def _synthesize_clawbot_pconfig():
...
def _resolve_clawbot_default_chat_id() -> str | None:
    from gateway.platforms.clawbot.config import clawbot_credentials_ready, load_clawbot_config
...
def _resolve_wecom_default_chat_id() -> str | None:
...
_INFRA_SEND_PLATFORMS = frozenset({"api_server", "webhook", "local"})


def send_message_ready_platforms() -> list[str]:
    """Platforms that send_message can deliver to right now."""
    names: set[str] = set()
    try:
        ...
        for platform in load_gateway_config().get_connected_platforms():
            ...
            if value not in _INFRA_SEND_PLATFORMS:
                names.add(value)
    except Exception:
        logger.debug("send_message: gateway config unavailable", exc_info=True)

    if _synthesize_clawbot_pconfig():
        names.add("clawbot")
    if _synthesize_wecom_pconfig():
        names.add("wecom")
...
SEND_MESSAGE_SCHEMA = {
    ...
    "description": (
        ...
        "The enabled platform list is rebuilt at runtime — call action='list' to see "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target: 'platform' or 'platform:chat_id'. Enabled platforms are listed in the tool description."
            },
            "message": {
                "type": "string",
...
def _build_dynamic_send_message_schema() -> dict[str, Any]:
    """Rebuild tool description from currently enabled messaging platforms."""
    platforms = send_message_ready_platforms()
    if platforms:
        plat_text = ", ".join(platforms)
        description = (
            "Send a message to a connected messaging platform, or list available targets.\n\n"
            f"Currently available platforms: {plat_text}.\n"
            "Use target='<platform>' for the default recipient (bound user / home channel / last chat). "
            "WeChat notifications: target='clawbot'. Enterprise WeChat: target='wecom'.\n\n"
            "When the user names a specific person or channel (not just the platform), "
            "call send_message(action='list') first, then send to the matching target."
        )
        target_hint = (
            f"Delivery target. Enabled platforms: {plat_text}. "
            "Format: 'platform' (default recipient), 'platform:chat_id', or "
            "'platform:#channel-name'. Examples: "
            + ", ".join(f"'{name}'" for name in platforms[:4])
        )
        if len(platforms) > 4:
            target_hint += ", …"
    else:
        description = (
            "Send a message to a connected messaging platform, or list available targets.\n\n"
            "No messaging platforms are currently enabled. "
            "Call action='list' to confirm; configure channels in the dashboard first."
        )
        target_hint = (
            "Delivery target in the form 'platform' or 'platform:chat_id'. "
            "No messaging platforms are currently enabled."
        )

    parameters = {
        **SEND_MESSAGE_SCHEMA["parameters"],
        "properties": {
            k: dict(v)
            for k, v in SEND_MESSAGE_SCHEMA["parameters"]["properties"].items()
        },
    }
    parameters["properties"]["target"]["description"] = target_hint
...
def _handle_send(args):
    ...
    ready_platforms = send_message_ready_platforms()
    if ready_platforms and platform_name not in ready_platforms:
        return tool_error(
            f"Platform '{platform_name}' is not enabled. "
            f"Available platforms: {', '.join(ready_platforms)}"
        )
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.marketing_hub/config.yaml or environment variables.")
        elif platform_name == "clawbot":
            pconfig = _synthesize_clawbot_pconfig()
            if not pconfig:
                return tool_error(
                    "Platform 'clawbot' is not configured. Complete WeChat ClawBot QR bind first."
                )
        elif platform_name == "wecom":
            pconfig = _synthesize_wecom_pconfig()
            if not pconfig:
                return tool_error(
                    "Platform 'wecom' is not configured. Set WECOM_BOT_ID and WECOM_SECRET first."
                )
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.marketing_hub/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if not home and platform_name == "clawbot":
            default_chat = _resolve_clawbot_default_chat_id()
            if default_chat:
                from gateway.config import HomeChannel
                home = HomeChannel(platform=platform, chat_id=default_chat, name="Clawbot Bound User")
        if not home and platform_name == "wecom":
            default_chat = _resolve_wecom_default_chat_id()
            if default_chat:
                from gateway.config import HomeChannel
                home = HomeChannel(platform=platform, chat_id=default_chat, name="WeCom Last Chat")
        if home:
            chat_id = home.chat_id
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if platform in (Platform.WEIXIN, Platform.CLAWBOT):
...
async def _send_wecom(extra, chat_id, message, chat_type=None):
    """Send via WeCom — prefer live Gateway adapter + outbound queue."""
    try:
        from gateway.platforms.wecom.adapter import check_wecom_requirements, send_wecom_direct
    ...
    try:
        ...
        if result.get("ws_req_id"):
            payload["ws_req_id"] = result["ws_req_id"]
        return payload
    except Exception as e:
...
registry.register(
    ...
    dynamic_schema_overrides=_build_dynamic_send_message_schema,
    emoji="📨",
)
