...
def iter_sync_files(container_base: str = "/root/.marketing_hub") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.marketing_hub to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.marketing_hub", container_base, 1
        )
...
class FileSyncManager:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.marketing_hub/skills/a.md`` →
        ``~/.marketing_hub/skills/a.md``, a new remote file at
        ``/root/.marketing_hub/skills/b.md`` maps to ``~/.marketing_hub/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
