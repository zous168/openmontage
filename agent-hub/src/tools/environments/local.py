...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge context-local Agent data override into subprocess env."""
    try:
        ...
        if path.parent.name == "profiles":
            env["HERMES_PROFILE"] = path.name
            env.setdefault("HUB_DATA_DIR", str(path.parent.parent))
        else:
            env["HUB_DATA_DIR"] = value
    except Exception:
        pass
