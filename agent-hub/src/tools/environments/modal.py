...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.marketing_hub"),
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        ...
        Modal sandboxes always run as root, so /root/.marketing_hub is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.marketing_hub"
            )
