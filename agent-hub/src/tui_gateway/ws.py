...
async def handle_ws(ws: Any) -> None:
    ...
    try:
        ...
        if "code=1012" in disconnect_reason:
            _log.warning(
                "ws service_restart peer=%s — dashboard default chat may show "
                "'gateway exited' if the PTY websocket closes before reconnect",
                peer,
            )
        if dispatch_crashes or send_failures or parse_errors:
            _log.warning(
                "ws anomaly peer=%s dispatch_crashes=%d send_failures=%d parse_errors=%d",
                peer,
                dispatch_crashes,
                send_failures,
                parse_errors,
