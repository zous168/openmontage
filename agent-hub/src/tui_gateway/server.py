...
# appends every unhandled exception to {data_root}/logs/tui_gateway_crash.log
_CRASH_LOG = str(get_hub_logs_dir() / "tui_gateway_crash.log")
...
def _close_session_by_id(sid: str, *, end_reason: str = "tui_close") -> bool:
    ...
    if session.get("agent") is not None:
        agent_build = "ready"
    elif session.get("agent_error"):
        agent_build = "failed"
    elif session.get("agent_build_started"):
        ready_evt = session.get("agent_ready")
        agent_build = (
            "in_progress"
        ...
        agent_build = "not_started"
    logger.info(
        "session.close sid=%s session_key=%s end_reason=%s agent_build=%s "
        "running=%s agent_error=%r",
        sid,
...
def _schedule_ws_orphan_reap(sid: str) -> None:
    ...
    def _reap() -> None:
        ...
        with _session_resume_lock:
            ...
            logger.info(
                "session.orphan_reap sid=%s session_key=%s",
                sid,
...
def _close_sessions_for_transport(
    ...
    if owned:
        peer = getattr(transport, "peer", None) or "?"
        logger.info(
            "ws.disconnect transport peer=%s end_reason=%s sessions=%d detail=%s",
            peer,
            end_reason,
            len(owned),
            [
                {
                    ...
                    "close_on_disconnect": bool(s.get("close_on_disconnect")),
                    "agent_ready": bool(s.get("agent") is not None),
                    "agent_build_started": bool(s.get("agent_build_started")),
                    "agent_error": bool(s.get("agent_error")),
                }
                for sid, s in owned
            ],
        )
...
def _config_yaml_path(profile_home: Path | None = None) -> Path:
    """Resolve config.yaml — 全局 ``get_config_path()``."""
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    config, not the stale env var (issue #40334). In Hub integrated mode,
    ``terminal.cwd`` is read from the global config. Returns an absolute,
    existing directory, or None for placeholders / missing / invalid paths.
    """
    try:
        import yaml

        p = _config_yaml_path(profile_home)
        if not p.exists():
            return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        if current is None:
            logger.warning("agent_build.skip sid=%s reason=session_gone", sid)
            ready.set()
            return
        ...
        try:
            ...
            try:
                _model_trace(
                    ...
                    sid=sid,
                    session_key=key,
                    profile_home=profile_home or "",
                    hermes_profile=os.environ.get("HERMES_PROFILE", ""),
                    startup_model=_resolve_model(),
                )
            ...
            logger.info(
                "agent_build.complete sid=%s session_key=%s elapsed_ms=%d model=%s",
                sid,
                key,
                elapsed_ms,
                getattr(agent, "model", ""),
            ...
            if elapsed_ms >= 30_000:
                logger.warning(
                    "agent_build.slow sid=%s session_key=%s elapsed_ms=%d",
                    sid,
                    key,
                    elapsed_ms,
                )
            ...
            elapsed_ms = int((time.monotonic() - t0) * 1000)
            logger.exception(
                "agent_build.fail sid=%s session_key=%s elapsed_ms=%d err=%s",
                sid,
                key,
                elapsed_ms,
                e,
            ...
            if replaced:
                logger.warning(
                    "agent_build.discarded sid=%s session_key=%s build_state=%s",
                    sid,
                    key,
                    build_state,
                )
...
def _completion_cwd(params: dict | None = None) -> str:
    ...
    raw = (
        ...
        # Last resort is the profile's workspace, not the process cwd: this
        # gateway is spawned by the TUI/hub, so its launch dir is incidental.
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # Honor per-session profile override in standalone mode; integrated
        # mode always reads the global HUB_DATA_DIR config.
        override = get_hermes_home_override()
        profile_home = Path(override) if isinstance(override, str) and override else None
        p = _config_yaml_path(profile_home)
        ...
        if p.exists():
            ...
            data = {}
        try:
            from hermes_cli.config import _deep_merge, _integrated_global_config_overlay

            overlay = _integrated_global_config_overlay()
            if overlay:
                data = _deep_merge(overlay, data)
        except Exception:
            pass
        with _cfg_lock:
        ...
        m = data.get("model")
        model_default = (
            m.get("default") if isinstance(m, dict) else (m if isinstance(m, str) else "")
        )
        _model_trace(
            "load_cfg",
            path=str(p),
            ...
            model_provider=(m.get("provider") if isinstance(m, dict) else ""),
        )
        return data
    except Exception:
        logger.warning("[model-trace] load_cfg failed", exc_info=True)
    return {}
...
def _save_cfg(cfg: dict):
    ...
    override = get_hermes_home_override()
    profile_home = Path(override) if isinstance(override, str) and override else None
    path = _config_yaml_path(profile_home)
...
def _resolve_model() -> str:
    ...
    if env:
        _model_trace("resolve_model", source="env", model=env)
        return env
    m = _load_cfg().get("model", "")
    if isinstance(m, dict):
        resolved = str(m.get("default", "") or "").strip()
        if resolved:
            _model_trace(
                "resolve_model",
                source="config",
    ...
    if isinstance(m, str) and m:
        resolved = m.strip()
        _model_trace("resolve_model", source="config", model=resolved)
        return resolved
    _model_trace("resolve_model", source="hardcoded_fallback", model="anthropic/claude-sonnet-4")
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider:
        _model_trace(
            "resolve_startup_runtime",
            model=model,
            provider=explicit_provider,
            source="HERMES_TUI_PROVIDER",
        )
    ...
    if not explicit_model:
        _model_trace("resolve_startup_runtime", model=model, provider=None, source="config")
        return model, None
    ...
    try:
        ...
        if detected:
            ...
            _model_trace(
                ...
                provider=provider,
                source="detect_static_provider",
            )
            return detected_model, provider
    except Exception:
        pass
    _model_trace("resolve_startup_runtime", model=model, provider=None, source="config_fallback")
    return model, None
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if not isinstance(cfg, dict):
        return ""
    warnings: list[str] = []
    # Scalar nulls that mean "use default / unbounded" — not empty YAML sections.
    _null_scalar_ok = frozenset({"max_concurrent_sessions"})
    null_keys = sorted(
        k for k, v in cfg.items() if v is None and k not in _null_scalar_ok
    )
    if not null_keys:
        pass
    else:
...
def _make_agent(
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        _model_trace(
            ...
            sid=sid,
            session_key=key,
            source="startup_runtime"
            + ("+str_override" if isinstance(model_override, str) and model_override else "")
            + ("+provider_override" if provider_override else ""),
            model=model,
            provider=requested_provider,
        )
        runtime = resolve_runtime_provider(
    ...
    elapsed_ms = int((time.monotonic() - t0) * 1000)
    logger.info(
        "make_agent.done sid=%s session_key=%s elapsed_ms=%d model=%s provider=%s",
        sid,
        key,
        elapsed_ms,
        model,
    ...
    if elapsed_ms >= 30_000:
        logger.warning(
            "make_agent.slow sid=%s session_key=%s elapsed_ms=%d",
            sid,
            key,
            elapsed_ms,
        )
...
def _(rid, params: dict) -> dict:
    ...
    logger.info(
        "session.create sid=%s session_key=%s profile=%s cwd=%s lazy=True",
        sid,
        key,
        profile or "default",
        resolved_cwd,
    ...
    try:
        ...
        return _ok(
            rid,
            {
                ...
                "source": getattr(view, "source", "nous") or "nous",
            },
        )
    except Exception:
...
def _(rid, params: dict) -> dict:
    ...
    # (~/.marketing_hub/sessions/saved/) rather than the project/workspace CWD, and
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.marketing_hub/.env`` into the gateway process via
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"go to Settings → Main Model to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.marketing_hub/.env
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_config_yaml_path())],
                ],
            },
        ]
