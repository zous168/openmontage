...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.marketing_hub/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.marketing_hub/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.marketing_hub/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.marketing_hub/logs/curator/``)
    ...
    authored skill data in ``~/.marketing_hub/skills/``.
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.marketing_hub/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.marketing_hub/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.marketing_hub/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.marketing_hub/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    if synchronous:
        ...
        # Propagate ContextVars (incl. HERMES_HOME override) so a profile-scoped
        # curator pass consolidates skills into the correct profile's skills/.
        from tools.thread_context import propagate_context_to_thread

        t = threading.Thread(
            target=propagate_context_to_thread(_llm_pass), daemon=True, name="curator-review"
        )
        t.start()
