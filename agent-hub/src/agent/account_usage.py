...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                propagate_context_to_thread(get_nous_portal_account_info), force_fresh=True
...
class CreditsView:
    ...
    ``source``: ``platform`` = 设备登录后的商户算力点；``nous`` = Nous Portal。
    """
...
def _build_platform_credits_view(*, markdown: bool = False) -> Optional[CreditsView]:
    """设备已登录时展示平台算力点（device_auth 快照）；未登录返回 None。"""
    try:
        from core.platform.device.local_device_auth import LocalDeviceAuthStore

        auth = LocalDeviceAuthStore().load()
    except Exception:
        logger.debug("credits ▸ platform device auth load failed", exc_info=True)
        return None
    if auth is None or not (auth.access_token or "").strip():
        return None

    balance = float(auth.credit_balance or 0)
    tokens_per_point = int(auth.compute_point_tokens or 100)
    token_equiv = balance * tokens_per_point
    tenant = (auth.tenant_name or "").strip() or auth.tenant_id
    login = (auth.login_name or "").strip()

    if markdown:
        lines = [
            f"**余额**：{balance:,} 点",
            f"**等值 token**：≈ {token_equiv:,}（1 点 = {tokens_per_point} token）",
        ]
    else:
        lines = [
            f"余额：{balance:,} 点",
    ...
    identity_parts = [p for p in (tenant, login) if p]
    identity_line = ("商户 / 账号：" + " / ".join(identity_parts)) if identity_parts else None

    return CreditsView(
        logged_in=True,
        balance_lines=tuple(lines),
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    优先展示平台算力点（设备登录）；否则回退 Nous Portal。Fail-open →
    ``CreditsView(logged_in=False)``.
    """
    platform = _build_platform_credits_view(markdown=markdown)
