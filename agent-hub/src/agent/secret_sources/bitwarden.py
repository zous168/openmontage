...
so they don't have to live in plaintext in ``~/.marketing_hub/.env``.
...
* The access token is stored in ``~/.marketing_hub/.env`` as
...
# never the access token. It's plaintext-equivalent to ~/.marketing_hub/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    to `$HERMES_HOME` / `~/.marketing_hub` keeps direct callers working too.
    """
...
def fetch_bitwarden_secrets(
    *,
    ...
    back to ``$HERMES_HOME`` / ``~/.marketing_hub``.
