...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.marketing_hub/auth.json active provider)
...
from hermes_constants import OPENROUTER_BASE_URL, display_hermes_env_path
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.marketing_hub/auth.json for an active Nous provider.
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    old OPENAI_BASE_URL lingers in ~/.marketing_hub/.env. ──
    ...
    if (main_provider and main_model
        ...
        elif main_provider == "official":
            # 系统内置：钉住网关 base_url；JWT 不钉死（短 TTL，resolve 时现读）
...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "external_process":
    ...
    elif pconfig.auth_type == "device_jwt":
        # 系统内置（official）：设备 JWT + 平台 LLM 网关，与主对话同源。
        # 票 TTL 约 15 分钟：必须现读（含临近过期 refresh），禁止沿用
        # set_runtime_main / 调用方传入的过期 explicit_api_key，否则文案改写等
        # call_llm 会稳定打出 JWT expired。
        try:
            from hermes_cli.runtime_provider import resolve_runtime_provider
            from hermes_cli.official_gateway_models import get_official_default_model
        except ImportError:
            logger.warning(
                "resolve_provider_client: device_jwt provider %s unavailable "
                "(runtime helpers missing)",
                provider,
            )
            return None, None
        try:
            ...
            logger.warning(
                "resolve_provider_client: device_jwt provider %s resolve failed: %s",
                provider,
                exc,
            )
            return None, None
        api_key = (rt.get("api_key") or "").strip()
        if explicit_api_key and explicit_api_key.strip() and explicit_api_key.strip() != api_key:
            logger.debug(
                "resolve_provider_client: ignore stale explicit device jwt for %s",
                provider,
            )
        ...
        if not final_model:
            logger.warning(
                "resolve_provider_client: device_jwt provider %s has no model",
                provider,
            )
            return None, None
        extra: Dict[str, Any] = {}
...
def _persisted_main_runtime_cache_key() -> tuple[str, str, str, str, str, str]:
    """Snapshot of persisted main model routing for ``provider=auto`` cache keys."""
    try:
        from hermes_cli.config import load_config
        ...
        if isinstance(model_cfg, dict):
            return (
                ...
                str(model_cfg.get("api_mode") or "").strip(),
                str(model_cfg.get("auth_mode") or "").strip(),
            )
    except Exception:
        pass
    return ("", "", "", "", "", "")
...
def _device_jwt_cache_fingerprint() -> str:
    """设备 JWT 指纹：票轮换后 cache key 变化，避免沿用已过期客户端。

    后台 ``ensure_device_access_fresh`` 只更新磁盘票；``call_llm`` 若命中
    不含 JWT 的 cache key，会继续用创建时写死的旧 ``api_key`` → 网关 401。
    """
    try:
        from core.platform.device.device_auth_service import get_fresh_device_access_token

        tok = get_fresh_device_access_token()
    except Exception:
        return ""
    if not tok:
        return ""
...
def _uses_device_jwt_channel(
    provider: str,
    *,
    main_runtime: Optional[Dict[str, Any]] = None,
    runtime_key: tuple = (),
) -> bool:
    """当前解析是否走官方 device_jwt（短 TTL，缓存必须随票失效）。"""
    prov = _normalize_aux_provider(provider)
    if prov == "official":
        return True
    if prov != "auto":
        return False
    runtime = _normalize_main_runtime(main_runtime)
    main_prov = str(runtime.get("provider") or "").strip().lower()
    if not main_prov and runtime_key:
        main_prov = str(runtime_key[0] or "").strip().lower()
    if not main_prov:
        main_prov = str(_persisted_main_runtime_cache_key()[0] or "").strip().lower()
    return main_prov == "official"
    ...
    *,
    ...
    if provider == "auto":
        ...
        if not any(runtime_key):
            runtime_key = _persisted_main_runtime_cache_key()
    pool_hint = _pool_cache_hint(provider, main_runtime=main_runtime)
    jwt_fp = ""
    if _uses_device_jwt_channel(provider, main_runtime=main_runtime, runtime_key=runtime_key):
        jwt_fp = _device_jwt_cache_fingerprint()
    return (
        provider,
        async_mode,
        base_url or "",
