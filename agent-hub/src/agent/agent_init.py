...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.marketing_hub/SOUL.md as the primary
    ...
    # both live under ~/.marketing_hub/logs/.  Idempotent, so gateway mode
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Go to Settings → Main Model to "
    ...
    # Session logs go into ~/.marketing_hub/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.marketing_hub/sessions/session_{sid}.json)
    ...
    # Hermes self-help guidance ("You run on Hermes Agent…"). Default True.
    # MxAI business Agent profiles set False in config.yaml for lean prompts.
