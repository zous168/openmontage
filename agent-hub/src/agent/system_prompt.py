...
def _append_skills_index(agent: Any, stable_parts: List[str], *, lean: bool) -> None:
    """Inject profile-scoped skills index (Hermes ``build_skills_system_prompt``)."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # ``agent.hermes_help_guidance: false`` → lean business prompt: SOUL +
    # INSTRUCTIONS（纪律提示词）, skip Hermes framework blocks. Skills index
    # still injected below via ``build_skills_system_prompt(lean=True)``.
    _hermes_framework = getattr(agent, "_hermes_help_guidance", True)
    ...
    # lean 业务 Agent：注入 profile INSTRUCTIONS.md（GUI「纪律提示词」），
    # 否则仅 SOUL 生效、微信/企微强制读技能等铁律不会进 system prompt。
    if not _hermes_framework and (agent.load_soul_identity or not agent.skip_context_files):
        _load_instructions = getattr(_r, "load_instructions_md", None)
        _instructions = _load_instructions() if callable(_load_instructions) else None
        if _instructions:
            stable_parts.append(f"纪律与边界：\n{_instructions}")

    if not _soul_loaded:
        # Fallback to hardcoded identity (assistant/CLI only — lean profiles
        # rely on SOUL.md and skip the generic Hermes persona).
        if _hermes_framework:
    ...
    if _hermes_framework:
        ...
        # under so it doesn't conflate ~/.marketing_hub/skills/ (default profile) with
        # ~/.marketing_hub/profiles/<active>/skills/ (this profile's). Deterministic
        ...
        if active_profile == "default":
            stable_parts.append(
                ...
                "under ~/.marketing_hub/profiles/<name>/. Each profile has its own "
            ...
            stable_parts.append(
                ...
                f"and writes ~/.marketing_hub/profiles/{active_profile}/. The default "
                f"profile's data lives at ~/.marketing_hub/skills/, ~/.marketing_hub/plugins/, "
                f"~/.marketing_hub/cron/, ~/.marketing_hub/memories/ — those belong to a "
    ...
    _append_skills_index(agent, stable_parts, lean=not _hermes_framework)
    ...
    if not agent.skip_context_files and _hermes_framework:
    ...
    if _hermes_framework and agent._memory_store:
    ...
    if _hermes_framework and agent._memory_manager:
        try:
