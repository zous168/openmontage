...
def _hermes_home_path() -> Path:
    ...
    try:
        ...
        hub = os.environ.get("HUB_DATA_DIR", "").strip()
        if hub:
            return Path(hub)
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.marketing_hub/.env`` and exfiltrate the file. The read-deny exists
...
# edited skills under both ``~/.marketing_hub/profiles/hermes-security/skills/``
# AND ``~/.marketing_hub/skills/`` (the default profile's skills) without realizing
...
def _resolve_active_profile_name() -> str:
    ...
    ``~/.marketing_hub``              -> ``"default"``
    ``~/.marketing_hub/profiles/X``  -> ``"X"``
...
# (the agent's view inside the container is plain ``/root/.marketing_hub/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    for i, part in enumerate(parts):
        ...
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] in {".hermes", ".marketing_hub"}:
            return i + 4
    return None
...
# that prefix: the agent sees plain ``/root/.marketing_hub/…``.  The root:root
