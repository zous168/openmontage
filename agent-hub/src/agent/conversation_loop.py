...
def _official_credits_insufficient_message() -> str:
    """官方网关算力不足文案；catalog 缺失时仍返回可读中文。"""
    from agent.credit_messages import official_credits_insufficient_message

    return official_credits_insufficient_message()


def _is_official_gateway_route(provider: str, base_url: str = "") -> bool:
    """官方算力渠道（providers.official → llm-gateway）；402 = 平台算力点不足。"""
    if (provider or "").strip().lower() == "official":
        return True
    # 未标 official 但指向配置的 LLM 网关时同样按平台算力处理
    gw = resolve_llm_gateway_base_url()
    if not gw:
        return False
    base = str(base_url or "").strip().rstrip("/")
    return bool(base) and base == gw
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_official_gateway_route(provider, base_url):
        # LT-045.04：网关 402 → 会话内友好提示（i18n）；不引导去第三方充值页
        return _official_credits_insufficient_message()
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if response_invalid:
                    ...
                    try:
                        from agent.llm_request_log import record_llm_api_request_from_agent
                        ...
                        record_llm_api_request_from_agent(
                            ...
                            api_kwargs=api_kwargs,
                            response=response,
                            error_details=", ".join(error_details) or "Invalid API response",
                            status="invalid_response",
                            latency_ms=api_duration * 1000.0,
                        )
                    except Exception:
                        pass
                ...
                try:
                    ...
                    _log_cost = None
                    if hasattr(response, "usage") and response.usage:
                        _log_usage = normalize_usage(
                            response.usage,
                            provider=agent.provider,
                            api_mode=agent.api_mode,
                        )
                        _log_cost = estimate_usage_cost(
                            agent.model,
                            _log_usage,
                            provider=agent.provider,
                            base_url=agent.base_url,
                    ...
                    record_llm_api_request_from_agent(
                        ...
                        status="success",
                        latency_ms=api_duration * 1000.0,
                        input_tokens=_log_usage.input_tokens if _log_usage else None,
                        output_tokens=_log_usage.output_tokens if _log_usage else None,
                        cache_read_tokens=_log_usage.cache_read_tokens if _log_usage else None,
                        cache_write_tokens=_log_usage.cache_write_tokens if _log_usage else None,
                        reasoning_tokens=_log_usage.reasoning_tokens if _log_usage else None,
                        estimated_cost_usd=float(_log_cost.amount_usd)
                        if _log_cost and _log_cost.amount_usd is not None
                        else None,
                    )
                except Exception:
                    pass
                ...
                # 官方网关 402：算力点不足 — 不旋转凭证、不重试透支
                _official_credit_exhausted = (
                    classified.reason == FailoverReason.billing
                    and _is_official_gateway_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                )
                if not _official_credit_exhausted:
                ...
                try:
                    ...
                    record_llm_api_request_from_agent(
                        ...
                        error=api_error,
                        status="error",
                        latency_ms=elapsed_time * 1000.0,
                    )
                except Exception:
                    pass
                ...
                # 官方网关算力不足：跳过 fallback 链，直接会话内提示（不重试透支）
                if (
                    is_rate_limited
                ...
                if is_client_error:
                    # 官方网关 402：不尝试 fallback（不重试透支），直接友好提示
                    _skip_fallback_for_official_credits = (
                        classified.reason == FailoverReason.billing
                        and _is_official_gateway_route(_provider, str(_base))
                    )
                    ...
                    if not _skip_fallback_for_official_credits and agent._has_pending_fallback():
                        if classified.reason == FailoverReason.content_policy_blocked:
                            agent._buffer_status("⚠️ Provider safety filter blocked this request — trying fallback...")
                        else:
                            agent._buffer_status(f"⚠️ Non-retryable error (HTTP {status_code}) — trying fallback...")
                    if not _skip_fallback_for_official_credits and agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                        _retry.primary_recovery_attempted = False
                        continue
                    ...
                    elif _skip_fallback_for_official_credits:
                        agent._emit_status(
                            "❌ "
                    ...
                    if classified.reason == FailoverReason.billing:
                        _credit_msg = _billing_or_entitlement_message(
                            capability="model access",
                            provider=_provider,
                            base_url=str(_base),
                            model=_model,
