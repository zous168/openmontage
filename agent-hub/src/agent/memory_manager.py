...
class MemoryManager:
    ...
    def _submit_background(self, fn) -> None:
        ...
        if executor is None:
            ...
            return
        try:
            # Propagate ContextVars (incl. HERMES_HOME override) so a
            # profile-scoped agent's memory sync writes MEMORY.md/USER.md to the
            # correct profile's memories/, not the default one. The inline
            # fallbacks above/below run on the caller thread (override already
            # active) and need no wrapping.
            from tools.thread_context import propagate_context_to_thread

            executor.submit(propagate_context_to_thread(fn))
        except RuntimeError:
