...
def strip_think_blocks(agent, content: str) -> str:
    ...
    # 1b2. Bracket-style tool-call leaks some models emit in plain text
    # (e.g. ``[TOOL_CALL] {tool => "vision_analyze", ...} [/TOOL_CALL]``)
    # instead of structured ``tool_calls`` — strip before outbound display.
    content = re.sub(
