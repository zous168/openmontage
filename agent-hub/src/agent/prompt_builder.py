...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.marketing_hub/kanban.db`. Your task id is in "
...
PLATFORM_HINTS = {
    ...
    "api_server": (
        "FloatingChat / API server channel. To preview a local file inline, "
        "include MEDIA:/absolute/path in your reply (confirm path with execute_code "
        "or terminal first if needed). Materials library tools are for assets already "
        "in the library — not for previewing arbitrary local files. "
...
def _lean_index_category_roots() -> frozenset[str]:
    """Top-level categories for lean business profile skill index (default: mxai only)."""
    try:
        ...
        if config_path.is_file():
            ...
            if isinstance(parsed, dict):
                ...
                if isinstance(skills_cfg, dict):
                    raw = skills_cfg.get("lean_index_categories")
        ...
        pass
    return frozenset({"mxai"})


def _filter_skills_by_category_roots(
    ...
    return {
        ...
        if cat.split("/", 1)[0] in allowed_roots
    }
...
def build_skills_system_prompt(
    ...
    *,
    ...
    scanned alongside the local ``~/.marketing_hub/skills/`` directory.  External dirs
    are read-only — they appear in the index but new skills are always created
    in the local dir.  Local skills take precedence when names collide.

    Skills in ``skills.disabled`` (profile ``config.yaml``) are omitted.
    ...
    ``lean=True`` — shorter header for business profiles (``hermes_help_guidance:
    false``): only skills under ``skills.lean_index_categories`` (default
    ``["mxai"]``), respecting ``skills.disabled``; no ``skill_manage`` /
    hermes-agent boilerplate.
    """
    ...
    if lean:
        allowed_roots = _lean_index_category_roots()
        skills_by_category = _filter_skills_by_category_roots(
            skills_by_category, allowed_roots
        )
        category_descriptions = {
            cat: desc
    ...
    if not skills_by_category:
        ...
        result = (
            (
                "## Skills（必读）\n"
                "下列技能已为本档案启用。正文**不会**自动注入——"
                "若与当前对话场景匹配（哪怕部分相关），回复前 **必须先** 调用 "
                "skill_view(name) 自行加载并遵循其剧本，禁止只看索引名就直接作答。\n"
                "按 SOUL 定位选择最匹配项（销售/转化类或客服/售后类）；勿编造未启用技能的内容。\n"
                "\n"
                "<available_skills>\n"
                + "\n".join(index_lines)
                + "\n</available_skills>"
...
def load_instructions_md() -> Optional[str]:
    """Load INSTRUCTIONS.md from HERMES_HOME（GUI「纪律提示词」）."""
    try:
        from hermes_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure HERMES_HOME before loading INSTRUCTIONS.md: %s", e)

    path = get_hermes_home() / "INSTRUCTIONS.md"
    if not path.exists():
        return None
    try:
        ...
        if not content:
            return None
        content = _scan_context_content(content, "INSTRUCTIONS.md")
        content = _truncate_content(content, "INSTRUCTIONS.md")
        return content
    except Exception as e:
        logger.debug("Could not read INSTRUCTIONS.md from %s: %s", path, e)
        return None
