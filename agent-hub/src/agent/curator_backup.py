...
A pre-run snapshot of ``~/.marketing_hub/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.marketing_hub/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.marketing_hub/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.marketing_hub/cron/jobs.json``)."""
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.marketing_hub/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.marketing_hub/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.marketing_hub/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.marketing_hub/skills/``.
