...
Jobs are stored in ~/.marketing_hub/cron/jobs.json
Output is saved to ~/.marketing_hub/cron/output/{job_id}/{timestamp}.md
"""
...
def _cron_dir() -> Path:
    """Resolve cron storage at call time (honours profile ContextVar override)."""
...
# Import-time aliases kept for backward compatibility (CLI introspection, tests).
# Runtime code must call the helpers above so agent/profile scopes apply.
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    # Human display round-trip from schedule_display (e.g. "once in 5m" → "5m").
    once_in = re.match(r"^once in (.+)$", schedule_lower)
    if once_in:
        schedule = once_in.group(1).strip()
        original = schedule
        schedule_lower = schedule.lower()
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.marketing_hub/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        id: Optional explicit, deterministic job id. When provided it is used
                verbatim instead of a random uuid — enabling idempotent
                create-if-absent by callers that manage jobs by a stable id
                (``resolve_job_ref(id)`` before create). Must be a single safe
                path component (no ``/`` ``\\`` ``..`` or drive letter), since the
                id is used as the job's output directory name.
    ...
    # Callers may supply a deterministic, path-safe id (e.g. plugin-managed jobs that
    # want idempotent create-if-absent by a stable id). Otherwise a random uuid is used.
    if id is not None:
        candidate = str(id).strip()
        if (
            not candidate
            or candidate in {".", ".."}
            or "/" in candidate
            ...
            or Path(candidate).drive
        ):
            raise ValueError(f"Invalid explicit cron job id: {id!r}")
        job_id = candidate
    else:
    ...
    # HTTP job type: on tick, make an HTTP request to a URL instead of running an
    # agent/script. Response status → success, response body → output/receipt.
    normalized_http: Optional[Dict[str, Any]] = None
    if http is not None:
        if not isinstance(http, dict) or not str(http.get("url") or "").strip():
            raise ValueError("http job requires a dict with a non-empty 'url'")
        normalized_http = {
            "url": str(http["url"]).strip(),
            "method": str(http.get("method") or "POST").upper(),
            "headers": dict(http.get("headers") or {}),
            "body": http.get("body"),
            "timeout": float(http.get("timeout") or 120),
        }

    # no_agent jobs are meaningless without a script — the script IS the job.
    # (HTTP jobs are a distinct execution mode and are exempt from this check.)
    # Surface this as a clear ValueError at create time so bad configs never
    # reach the scheduler.
    if normalized_no_agent and not normalized_script and normalized_http is None:
        raise ValueError(
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for i, job in enumerate(jobs):
        ...
        # http is a first-class execution type — normalize the same way create_job
        # does so an edit (or a mode switch) stores a consistent shape. None/""/False
        # clears it (e.g. switching a job back to agent/script mode).
        if "http" in updates:
            raw_http = updates["http"]
            if raw_http in (None, "", False):
                updated["http"] = None
            elif isinstance(raw_http, dict):
                if not str(raw_http.get("url") or "").strip():
                    raise ValueError("http job requires a dict with a non-empty 'url'")
                updated["http"] = {
                    "url": str(raw_http["url"]).strip(),
                    "method": str(raw_http.get("method") or "POST").upper(),
                    "headers": dict(raw_http.get("headers") or {}),
                    "body": raw_http.get("body"),
                    "timeout": float(raw_http.get("timeout") or 120),
                }
            else:
                raise ValueError("http must be a dict or null")
...
def append_job_output_for_run(
    job_id: str, run_id: str, appendix: str, max_scan: int = 12
) -> bool:
    """把 ``appendix`` 追加到本次执行（含 ``Run ID: {run_id}`` 标记）的 output 文件末尾。

    用于让**异步回调**把「Callback 返回」段落追加到那次执行的**同一条记录**里（而非另建
    一条倒序记录）。找不到匹配文件返回 ``False``（调用方可回退 ``save_job_output``）。"""
    ...
    if not job_output_dir.is_dir():
        return False
    marker = f"Run ID:** {rid}"
    files = sorted(job_output_dir.glob("*.md"), reverse=True)[:max_scan]
    for f in files:
        try:
...
def _parse_output_filename_timestamp(stem: str) -> Optional[str]:
    """Parse ``YYYY-MM-DD_HH-MM-SS`` output filenames into ISO timestamps."""
    try:
...
def _safe_output_id(output_id: str) -> str:
...
def _summarize_job_output_markdown(text: str) -> tuple[str, str, str]:
    """Return (status, preview, section) from a saved cron output file."""
    failed = "(FAILED)" in text.split("\n", 1)[0] or "## Error" in text
    ...
    if "## Response" in text:
        ...
        section = "error"
        body = text.split("## Error", 1)[1].strip()
    preview = body.strip().replace("\r\n", "\n")
    if len(preview) > 400:
        preview = preview[:400].rstrip() + "…"
    return status, preview, section


def list_job_outputs(job_id: str, limit: int = 30) -> List[Dict[str, Any]]:
    """List saved markdown outputs for a cron job, newest first."""
    ensure_dirs()
    ...
    for path in files[:limit_n]:
        try:
            text = path.read_text(encoding="utf-8")
        except OSError as exc:
            logger.warning("Failed to read cron output %s: %s", path, exc)
            continue
        status, preview, section = _summarize_job_output_markdown(text)
        runs.append(
            {
                "output_id": path.stem,
                "filename": path.name,
                "run_at": _parse_output_filename_timestamp(path.stem),
                "status": status,
                "section": section,
                "preview": preview,
            }
...
def get_job_output(job_id: str, output_id: str) -> Optional[Dict[str, Any]]:
    """Return one saved cron output file by id (filename stem)."""
    safe_id = _safe_output_id(output_id)
    path = _job_output_dir(job_id) / f"{safe_id}.md"
    if not path.is_file():
        return None
    try:
        ...
        logger.warning("Failed to read cron output %s: %s", path, exc)
        return None
    status, preview, section = _summarize_job_output_markdown(content)
    return {
        "output_id": safe_id,
        "filename": path.name,
        "run_at": _parse_output_filename_timestamp(safe_id),
        "status": status,
        "section": section,
        "preview": preview,
        "content": content,
