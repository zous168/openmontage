...
Uses a file-based lock (~/.marketing_hub/cron/.tick.lock) so only one tick
...
_KNOWN_DELIVERY_PLATFORMS = frozenset({
    ...
    "wecom", "wecom_callback", "weixin", "clawbot", "sms", "email", "webhook", "bluebubbles",
    "qqbot", "yuanbao",
})

# Bot DM platforms: cron delivers like dashboard "send test message" — bound user /
# latest inbound chat, not necessarily a configured *_HOME_CHANNEL env var.
_DIRECT_BOT_PLATFORMS = frozenset({"wecom", "clawbot"})
...
@contextmanager
def _root_hermes_scope() -> Iterator[Path]:
    """Temporarily scope ``HERMES_HOME`` to the data root (gateway SSOT).

    Gateway platform blocks (``platforms.clawbot`` / ``platforms.wecom``) and
    the root ``model.default`` live under ``HUB_DATA_DIR``, not under named
    profile directories. Cron jobs may execute under a profile scope while
    still needing those root settings for model resolution and bot delivery.
    """
    from hermes_constants import get_default_hermes_root

    root = get_default_hermes_root()
    try:
        at_root = _get_hermes_home().resolve() == root.resolve()
    except OSError:
        at_root = _get_hermes_home() == root
    if at_root:
        yield root
        return
    from agent.profile_scope import hermes_profile_scope

    with hermes_profile_scope(root):
        yield root
...
def _iter_cron_profile_homes():
    """Yield HERMES_HOME paths that may hold cron jobs (default + named profiles)."""
    from hermes_constants import get_default_hermes_root
    ...
    try:
        ...
        for info in profiles_mod.list_profiles():
            ...
            if not name or str(name).strip().lower() == "default":
                continue
            home = profiles_mod.get_profile_dir(str(name)).resolve()
            if home not in seen:
                seen.add(home)
                yield home
    except Exception:
        logger.debug("Failed to enumerate profiles for cron tick", exc_info=True)
...
def _run_due_batch(
    ...
    *,
    ...
    """Execute a batch of due jobs under the current profile scope."""
    if verbose:
        logger.info(
            "Running %d job(s) in parallel (profile=%s)",
            len(due_jobs),
            _get_hermes_home(),
        )
    ...
    if verbose:
        logger.info(
            "Dispatching %d job(s) (max_workers=%s)",
            len(due_jobs),
    ...
    def _process_job(job: dict) -> bool:
        ...
        try:
            ...
            # http job 的「回执」不是给用户看的通知——投递交给业务执行端的完成回调
            # （callback → cron 回调端点，用 job.deliver 通道发 message）。这里跳过同步投递，
            # 否则「http 回执 JSON」+「callback message」双份投到通道。状态仍照常 mark。
            if job.get("http"):
...
def _resolve_wecom_bot_target() -> Optional[dict]:
    """Resolve WeCom delivery target from the latest inbound chat (test-send path)."""
    try:
        from gateway.platforms.wecom import resolve_wecom_test_target
        from gateway.platforms.wecom.config import load_wecom_config

        with _root_hermes_scope():
            cfg = load_wecom_config()
            bot_id = str(cfg.get("bot_id") or "").strip()
            if not bot_id:
                return None
            session = resolve_wecom_test_target(bot_id=bot_id)
        if not session:
            return None
        out: dict = {
            "platform": "wecom",
        ...
        if session.get("chat_type"):
            out["chat_type"] = session["chat_type"]
        return out
    except Exception:
        logger.debug("wecom bot delivery target unavailable", exc_info=True)
        return None


def _resolve_clawbot_target() -> Optional[dict]:
    """Resolve ClawBot delivery target from the bound WeChat user (test-send path)."""
    try:
        from gateway.platforms.clawbot.config import clawbot_credentials_ready, load_clawbot_config

        with _root_hermes_scope():
            cfg = load_clawbot_config()
        if not clawbot_credentials_ready(cfg):
            return None
        user_id = str(cfg.get("user_id") or cfg.get("bound_wxid") or "").strip()
        if not user_id:
            return None
        return {"platform": "clawbot", "chat_id": user_id, "thread_id": None}
    except Exception:
        logger.debug("clawbot delivery target unavailable", exc_info=True)
        return None


def _resolve_platform_delivery_target(platform_name: str) -> Optional[dict]:
    """Resolve where a cron job should deliver for a named platform."""
    name = platform_name.lower()
    if name == "wecom":
        direct = _resolve_wecom_bot_target()
        if direct:
            return direct
    elif name == "clawbot":
        direct = _resolve_clawbot_target()
        if direct:
            return direct
...
def _platform_delivery_ready(platform_name: str) -> bool:
    return _resolve_platform_delivery_target(platform_name) is not None
...
def _iter_home_target_platforms():
    ...
    for name in _HOME_TARGET_ENV_VARS:
        if name not in seen:
            seen.add(name)
            yield name
    for name in _DIRECT_BOT_PLATFORMS:
        if name not in seen:
            seen.add(name)
            yield name
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    for name in _iter_home_target_platforms():
        if name not in connected:
            # Direct-bot platforms (wecom/clawbot) are configured via their own
            # plugin config loaders (load_wecom_config / load_clawbot_config),
            # invisible to the base gateway config's get_connected_platforms().
            # Surface them anyway when their direct resolver can actually reach a
            # delivery target — otherwise the cron dropdown wrongly shows Local-only.
            if not (name in _DIRECT_BOT_PLATFORMS and _resolve_platform_delivery_target(name)):
                continue
        ...
        targets.append(
            {
                ...
                "home_target_set": _platform_delivery_ready(name),
...
_NON_PUSH_PLATFORMS = frozenset({"api_server", "webhook"})


def _bot_delivery_fallback(job: dict) -> Optional[dict]:
    """Prefer WeChat / WeCom bot DM when origin/dashboard can't receive push."""
    for name in ("clawbot", "wecom"):
        target = _resolve_platform_delivery_target(name)
        if target:
            logger.info(
                "Job '%s': using %s bot delivery instead of a non-push channel",
...
def _finalize_delivery_target(job: dict, target: Optional[dict]) -> Optional[dict]:
    """Remap targets that cannot receive proactive cron notifications."""
    if not target:
        return target
    platform = str(target.get("platform") or "").lower()
    if platform not in _NON_PUSH_PLATFORMS:
        return target
    return _bot_delivery_fallback(job)
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if deliver_value == "origin":
        if origin:
            return _finalize_delivery_target(
                job,
                {
        ...
        for platform_name in _iter_home_target_platforms():
            effective = _resolve_platform_delivery_target(platform_name)
            if effective:
                logger.info(
                ...
                return _finalize_delivery_target(job, effective)
        return _bot_delivery_fallback(job)
    ...
    if not _is_known_delivery_platform(platform_name):
        return None
    return _finalize_delivery_target(job, _resolve_platform_delivery_target(platform_name))
...
def _expand_routing_tokens(part: str) -> List[str]:
    ...
    expanded: List[str] = []
    for platform_name in _iter_home_target_platforms():
        if _platform_delivery_ready(platform_name):
...
def _synthesize_direct_bot_pconfig(platform_name: str):
    """Build a minimal PlatformConfig for bot DM delivery from root credentials."""
    ...
    if name == "clawbot":
        ...
        return PlatformConfig(
            ...
            extra={
                "bind_status": True,
                "user_id": str(cfg.get("user_id") or cfg.get("bound_wxid") or ""),
...
def _model_from_config_dict(cfg: dict) -> str:
    """Extract non-empty model.default from a load_config() snapshot."""
...
def _resolve_cron_model_from_inventory() -> str:
    """Last resort when config.model is blank: use an authenticated provider."""
    try:
        ...
        if ctx.current_model.strip():
            return ctx.current_model.strip()
        ...
        preferred = ctx.current_provider.strip().lower()
        if preferred:
            rows = sorted(
                rows,
                key=lambda row: 0 if str(row.get("slug") or "").lower() == preferred else 1,
            )

        _non_chat_markers = ("image", "tts", "asr", "ocr", "flux", "vidu", "wan", "audio")

        for row in rows:
            slug = str(row.get("slug") or "").lower()
            live = [str(m) for m in (row.get("models") or []) if str(m).strip()]
            if not live:
                continue
            for candidate in _PROVIDER_MODELS.get(slug) or []:
        ...
        logger.debug("cron model inventory fallback failed", exc_info=True)
    return ""


def _resolve_cron_model(job: dict, cfg: dict) -> str:
    """Resolve the LLM model for a cron job (job override → merged config)."""
    explicit = str(job.get("model") or "").strip()
    if explicit:
        return explicit
    ...
    try:
        ...
        resolved = str(_read_main_model() or "").strip()
        if resolved:
            return resolved
    except Exception:
        pass

    resolved = _model_from_config_dict(cfg)
    if resolved:
        return resolved

    # Named profiles may have empty local model while root config still carries
    # model.default (gateway uses the same overlay fallback in run.py).
    try:
        from hermes_constants import get_default_hermes_root

        root = get_default_hermes_root()
        if _get_hermes_home().resolve() != root.resolve():
            with _root_hermes_scope():
                from hermes_cli.config import load_config as _load_root_cfg

                resolved = _model_from_config_dict(_load_root_cfg() or {})
                if resolved:
                    return resolved
    except Exception:
        pass

    return _resolve_cron_model_from_inventory()
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    for target in targets:
        ...
        is_direct_bot = platform_name in _DIRECT_BOT_PLATFORMS
        if is_direct_bot:
            if not _platform_delivery_ready(platform_name):
                msg = f"platform '{platform_name}' bot not configured"
                logger.warning("Job '%s': %s", job["id"], msg)
                delivery_errors.append(msg)
                continue
            pconfig = config.platforms.get(platform) or _synthesize_direct_bot_pconfig(platform_name)
            if not pconfig:
                msg = f"platform '{platform_name}' bot credentials unavailable"
                logger.warning("Job '%s': %s", job["id"], msg)
                delivery_errors.append(msg)
                continue
        else:
        ...
        if runtime_adapter is not None and loop is not None and getattr(loop, "is_running", lambda: False)():
            ...
            try:
                ...
                if text_to_send:
                    from agent.async_utils import safe_schedule_threadsafe
                    if platform_name == "wecom" and chat_type:
                        send_coro = runtime_adapter.send_proactive(
                            chat_id, text_to_send, chat_type=chat_type,
                        )
                    else:
...
# http job 占位符：执行时展开，故 jobs.json / UI 不落敏感真值（token / 密钥），
# 且轮换即时生效。命名空间：{cronId} {cronName} / {env.VAR}（任意环境变量）/
# {ipc_token}（本机 IPC token，供内部 loopback 走 MxAI 标准鉴权）。
# 未知占位符原样保留——避免误伤 body 里用户自己的 {...} 字面量。
_CRON_PLACEHOLDER_RE = re.compile(r"\{([a-zA-Z0-9_.:-]+)\}")


def _expand_cron_placeholders(value, job: dict):
    """Recursively expand ``{cronId}`` / ``{cronName}`` / ``{env.VAR}`` /
    ``{ipc_token}`` in a str (or nested dict/list of strs). Non-str leaves
    ...
    if isinstance(value, str):
        ...
        def _resolve(m: "re.Match[str]") -> str:
            ...
            if key == "ipc_token":
                # 本机 IPC token（MxAI 标准鉴权）——执行时读，不落 jobs.json。
                from core.platform.device.local_ipc import get_or_create_ipc_token

                return get_or_create_ipc_token()
            if key.startswith("env."):
                return os.getenv(key[4:]) or ""
            return m.group(0)  # unknown → leave literal

        return _CRON_PLACEHOLDER_RE.sub(_resolve, value)
    if isinstance(value, dict):
        return {k: _expand_cron_placeholders(v, job) for k, v in value.items()}
    if isinstance(value, list):
        return [_expand_cron_placeholders(v, job) for v in value]
    return value


def _run_http_job(job: dict, http_cfg: dict) -> tuple[bool, str, str, Optional[str]]:
    """HTTP job type: make an HTTP request on tick; response IS the result.

    status 2xx → success; response body → output (saved) + final_response;
    non-2xx / timeout / connection error → failure with the error captured.
    Integrates with the standard save_job_output + mark_job_run flow, so status
    and receipt land in Hermes' native records (jobs.json + cron/output).
    """
    job_id = job["id"]
    job_name = str(job.get("name") or job.get("prompt") or job_id or "cron job")
    now = _hermes_now()
    now_iso = now.strftime("%Y-%m-%d %H:%M:%S")
    # 每次执行唯一 run_id（时间戳 + 短随机）——随 callback 下发，执行记录显示，供按执行对账。
    import uuid

    run_id = f"{now.strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:6]}"

    url = _expand_cron_placeholders(str(http_cfg.get("url") or "").strip(), job)
    if not url:
        return False, "", "", "http job has no url"
    method = str(http_cfg.get("method") or "POST").upper()
    headers = _expand_cron_placeholders(dict(http_cfg.get("headers") or {}), job)
    body = _expand_cron_placeholders(http_cfg.get("body"), job)
    try:
        timeout = float(http_cfg.get("timeout") or 120)
    except (TypeError, ValueError):
        timeout = 120.0

    # callback URL 执行时**动态构造并传递**给执行端（不入配置）：Hermes cron 回调端点
    # 与业务 url 在同一网关，从 url 的 base + 本 job id 拼出，随请求头 ``X-Cron-Callback``
    # 下发。执行端（如 MxAI cron_run）跑完读此头、主动调 callback(success, message)。
    from urllib.parse import urlsplit

    _parts = urlsplit(url)
    callback_url = ""
    if _parts.scheme and _parts.netloc:
        callback_url = f"{_parts.scheme}://{_parts.netloc}/api/cron/jobs/{job_id}/callback"
        # 随请求下发 callback URL + 本次执行 run_id（执行端回调时带上，供按执行对账）。
        headers = {**headers, "X-Cron-Callback": callback_url, "X-Cron-Run-Id": run_id}

    try:
        import httpx

        _host = (urlsplit(url).hostname or "").lower()
        # loopback 不走系统代理（Windows 会把 127.0.0.1 丢给代理 → 502），与 MxAI automan 出站一致。
        _loopback = _host in {"127.0.0.1", "localhost", "::1"}

        kwargs: dict = {"headers": headers, "timeout": timeout}
        if body is not None and body != "":
            if isinstance(body, str):
                # 字符串 body 按真实请求原样发（Content-Type 由 headers 决定，如 application/json）
                kwargs["content"] = body.encode("utf-8")
            else:
                kwargs["json"] = body  # dict / list → JSON 编码
        resp = httpx.request(method, url, trust_env=not _loopback, **kwargs)
        ok = 200 <= resp.status_code < 300
        response_text = (resp.text or "").strip()
        # 一条记录顺序：执行信息 → 同步返回 →（异步回调稍后追加）Callback 返回。
        doc = (
            f"# Cron Job: {job_name}\n\n"
            f"**Job ID:** {job_id}\n"
            f"**Run ID:** {run_id}\n"
        ...
        if ok:
            # final_response must be non-empty or _process_job flags success=False.
            return True, doc, response_text or "(ok)", None
        return False, doc, response_text, f"HTTP {resp.status_code}"
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    # ---------------------------------------------------------------
    # HTTP job type — the job IS an HTTP request; no agent, no script.
    # Response status → success, response body → output/receipt. Handled
    # before importing the agent/script machinery it doesn't need.
    # ---------------------------------------------------------------
    http_cfg = job.get("http")
    if http_cfg:
        return _run_http_job(job, http_cfg)
    ...
    _session_db = None
    try:
        from agent.profile_scope import profile_session_db

        _session_db = profile_session_db(_get_hermes_home())
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            logger.warning("Job '%s': failed to expand config env vars: %s", job_id, e)

        model = _resolve_cron_model(job, _cfg)
        if not model:
            profile_home = _get_hermes_home()
            raise RuntimeError(
                "Cron job has no model configured. Set model.default in "
                f"{profile_home / 'config.yaml'} or the root config.yaml, "
                "or pin model on the job."
            )
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        for profile_home in _iter_cron_profile_homes():
            with hermes_profile_scope(profile_home):
                ...
                if verbose:
                    logger.info(
                        "%s - %s job(s) due (profile=%s)",
                        _hermes_now().strftime('%H:%M:%S'),
                        len(due_jobs),
                        profile_home.name or "default",
                    )
                executed_total += _run_due_batch(
                    due_jobs,
                    verbose=verbose,
                    adapters=adapters,
                    loop=loop,
        ...
        if verbose and not saw_any_due:
