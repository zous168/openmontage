...
class AIAgent:
    ...
    def _spawn_background_review(
        ...
        # Propagate the current ContextVars (incl. the HERMES_HOME override) into
        # the worker thread so a profile-scoped agent's background review writes
        # memories/skills to the *correct* profile, not the default one.
        t = threading.Thread(
    ...
    def _persist_user_message_override_for_index(
        self, messages: List[Dict], absolute_idx: int
    ):
        """若该下标是本轮需落库改写的 user，返回 override 正文；否则 None。"""
        idx = getattr(self, "_persist_user_message_idx", None)
        override = getattr(self, "_persist_user_message_override", None)
        if override is None or idx is None or absolute_idx != idx:
            return None
        if not (0 <= absolute_idx < len(messages)):
            return None
        msg = messages[absolute_idx]
        if isinstance(msg, dict) and msg.get("role") == "user":
            return override
        return None

    def _apply_persist_user_message_override(self, messages: List[Dict]) -> None:
        """Turn 结束后把 API 侧 user 正文改回落库原文（仅改内存，供返回/续聊）.

        注意：落库/写 JSON 快照时用 ``_persist_user_message_override_for_index``
        取正文，**禁止**在打 LLM 前调用本方法——早期 ``_persist_session``
        （turn 开头 crash-resilience）若原地改写，会把包裹后的 API 正文冲掉，
        导致真实请求与诊断日志都只剩原文。
        """
        idx = getattr(self, "_persist_user_message_idx", None)
        if idx is None:
            return
        override = self._persist_user_message_override_for_index(messages, idx)
        if override is None:
            return
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        不改写内存中的 API messages；落库时单独套用 persist_user_message。
        """
    ...
    def _flush_messages_to_session_db(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        try:
            ...
            for absolute_idx in range(flush_from, len(messages)):
                msg = messages[absolute_idx]
                role = msg.get("role", "unknown")
                content = msg.get("content")
                override = self._persist_user_message_override_for_index(
                    messages, absolute_idx
                )
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.marketing_hub/sessions/session_{sid}.json``
        ...
        try:
            cleaned = []
            for absolute_idx, msg in enumerate(messages):
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.marketing_hub/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
