...
export function ArtLines({ lines, maxWidth }: { lines: [string, string][]; maxWidth?: number }) {
  const natural = artWidth(lines)
  const w = maxWidth ? Math.min(natural, maxWidth) : natural

  return (
    <Box flexDirection="column" height={lines.length} opaque width={w}>
      {lines.map(([c, text], i) => (
        <Text color={c} key={i} wrap="truncate-end">
          {maxWidth ? clip(text, maxWidth) : text}
        </Text>
      ))}
    </Box>
  )
...
export function Banner({ maxWidth, t }: { maxWidth?: number; t: Theme }) {
  ...
  const tagline = (t.brand.tagline ?? '').trim()
  ...
  // Full ASCII only when every art line fits; tagline is a separate centered row
  // (no icon prefix — avoids CJK truncation and bleed from clipped box-drawing).
  if (cols >= logoW) {
    return (
      <Box flexDirection="column" marginBottom={1} width={cols}>
        <ArtLines lines={logoLines} maxWidth={cols} />
        {tagline ? <Text color={t.color.muted}>{centerIn(tagline, cols)}</Text> : null}
      </Box>
    )
  }
  ...
  const showTag = cols >= 46 && tagline

  return (
    <Box flexDirection="column" marginBottom={1} width={cols}>
      <Text bold color={t.color.primary}>{centerIn(name, cols)}</Text>
      {showTag ? <Text color={t.color.muted}>{centerIn(tagline, cols)}</Text> : null}
    </Box>
  )
}
