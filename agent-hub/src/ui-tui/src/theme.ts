...
export function fromSkin(
  ...
  return normalizeThemeForAnsiLightTerminal({
    ...
    brand: {
      name: process.env.HERMES_TUI_AGENT_NAME?.trim() || branding.agent_name || d.brand.name,
      icon: d.brand.icon,
      prompt: cleanPromptSymbol(branding.prompt_symbol, d.brand.prompt),
      welcome: process.env.HERMES_TUI_WELCOME?.trim() || branding.welcome || d.brand.welcome,
      goodbye: branding.goodbye ?? d.brand.goodbye,
      tool: toolPrefix || d.brand.tool,
      helpHeader: branding.help_header ?? (helpHeader || d.brand.helpHeader),
      tagline:
        process.env.HERMES_TUI_TAGLINE?.trim() ||
        branding.agent_tagline?.trim() ||
        d.brand.tagline
    },
