...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _user_blocking_modal_active(self) -> bool:
        """True while clarify / approval / sudo / secret / slash-confirm owns the UI.

        Live tool spinner + scrollback prints must yield to these panels;
        otherwise Windows + prompt_toolkit stacks orange choices with a
        ticking ``(  0.2s)`` status line and torn ``.2s)`` residue.
        """
        return bool(
    ...
    def _quiet_tool_chrome_for_modal(self) -> None:
        """Clear live tool status so modal panels are not competed with."""
        self._spinner_text = ""
    ...
    def _render_spinner_text(self) -> str:
        ...
        if self._user_blocking_modal_active():
            return ""
    ...
    def _on_tool_gen_start(self, tool_name: str) -> None:
        ...
        # Do not compete with clarify / approval panels for the same rows.
        if self._user_blocking_modal_active():
            return
    ...
    def _on_tool_progress(self, event_type: str, function_name: str = None, preview: str = None, function_args: dict = None, **kwargs):
        ...
        While a user-blocking modal (clarify / approval / …) is up, spinner
        updates and scrollback prints are suppressed so the choice panel is
        not overdrawn by live ``(  0.2s)`` tool chrome.
        """
        modal_quiet = self._user_blocking_modal_active()

        if event_type == "tool.completed":
            self._tool_start_time = 0.0
            # Always drain pending args so a muted modal interval cannot leak
            # stale previews into a later scrollback line.
            stored_args = {}
            if function_name:
            ...
            if (
                not modal_quiet
                and function_name
                and self.tool_progress_mode in {"all", "new"}
            ):
        ...
        if function_name and not function_name.startswith("_"):
            # Store args even while muted so completion can drain cleanly.
    ...
    def _clarify_callback(self, question, choices):
        ...
        # Drop live tool chrome before the panel paints — clarify's own
        # tool.started already filled the spinner with a ticking timer.
        self._quiet_tool_chrome_for_modal()
        self._clarify_state = {
            "question": question,
