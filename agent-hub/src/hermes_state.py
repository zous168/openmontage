...
CREATE INDEX IF NOT EXISTS idx_llm_api_requests_session_created
    ON llm_api_requests(session_id, created_at DESC);
"""
...
class SessionDB:
    ...
    def insert_llm_api_request(
        ...
        *,
        ...
        """Persist one LLM HTTP attempt with raw request/response JSON."""

        def _do(conn):
            cur = conn.execute(
                """
                INSERT INTO llm_api_requests (
                    session_id, api_request_id, attempt, turn_id, api_call_number,
                    provider, base_url, model, api_mode, status, latency_ms,
    ...
    def list_llm_api_requests(
        ...
        *,
        ...
        columns = (
            "id, session_id, api_request_id, attempt, turn_id, api_call_number, "
            "provider, base_url, model, api_mode, status, latency_ms, "
            "input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, "
            "reasoning_tokens, estimated_cost_usd, created_at"
        )
        if include_bodies:
            columns += ", request_json, response_json, error_json"
    ...
    def count_llm_api_requests(self, session_id: str) -> int:
        with self._lock:
            row = self._conn.execute(
                "SELECT COUNT(*) FROM llm_api_requests WHERE session_id = ?",
                (session_id,),
            ).fetchone()
        return int(row[0])
    ...
    # WeChat-style list preview: newest user message (cleaned in Python).
    _PREVIEW_RAW_SUBQUERY = """
        COALESCE(
    ...
    def _strip_reasoning_leaks(text: str) -> str:
        """Remove model reasoning / tool-call XML from session list snippets."""
        from agent.agent_runtime_helpers import strip_think_blocks

        return strip_think_blocks(None, text or "")

    @classmethod
    def clean_session_snippet(cls, text: Optional[str], *, max_len: int = 60) -> str:
        """One-line session preview: no think blocks, no channel prefixes."""
        cleaned = cls._strip_reasoning_leaks(text or "")
        cleaned = re.sub(r"^\[微信\]\s*", "", cleaned)
    ...
    @classmethod
    def _finalize_list_row(cls, row: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize title + preview for session list consumers."""
        raw = str(row.pop("_preview_raw", "") or "").strip()
        row["preview"] = cls.clean_session_snippet(raw)
        title = row.get("title")
        if title:
            row["title"] = cls.sanitize_title(cls._strip_reasoning_leaks(title))
    ...
    def sanitize_title(title: Optional[str]) -> Optional[str]:
        ...
        title = SessionDB._strip_reasoning_leaks(title)
        if not title.strip():
            return None
    ...
    def list_sessions_rich(
        ...
        message_count, preview (cleaned last user message, up to 60 chars),
        ...
        if order_by_last_active:
            ...
            query = f"""
                ...
                SELECT s.*,
                    {SessionDB._PREVIEW_RAW_SUBQUERY},
                    COALESCE(
        ...
        sessions = []
        for row in rows:
            sessions.append(self._finalize_list_row(dict(row)))
    ...
    def list_sessions_by_id_prefix(
        ...
        *,
        ...
        """List sessions whose id starts with ``prefix``.

        Used by the mxai channel to discover customer conversations directly
        from SessionDB (id = ``mxai-{profile}-inbound-{hash}``), carrying
        ``user_id`` (customer_uid), ``title`` (display name) and
        ``last_active`` (max message timestamp). ``roots_only`` excludes
        compression-fork children (``parent_session_id IS NOT NULL``) so each
        customer surfaces once at its root session.
        """
        if not prefix:
            return []
        esc = prefix.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
        where = ["id LIKE ? ESCAPE '\\'", "archived = 0"]
        params: List[Any] = [esc + "%"]
        if min_message_count > 0:
        ...
        query = f"""
            SELECT id, user_id, title, message_count, started_at, ended_at, end_reason,
                COALESCE(
    ...
    def list_cron_job_runs(
        ...
        for row in rows:
            runs.append(self._finalize_list_row(dict(row)))
        return runs
    ...
    def _get_session_rich_row(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not row:
            return None
        return self._finalize_list_row(dict(row))
    ...
    def get_compression_chain_nodes(self, session_id: str) -> List[Dict[str, Any]]:
        """Return the full compression family for *session_id*.

        Yields the lineage root plus every compression continuation descended
        from it, each annotated with a compaction-summary preview. Handles both
        shapes:

        * **Linear chain** (root→child→grandchild) — native Hermes, where each
          compaction forks off the live tip.
        * **Sibling fork** (root + several direct children) — arises when a
          stateless caller such as the mxai api_server reloads the same
          deterministic source session every turn and forks a fresh
          continuation off the *root* on each compaction.

        A continuation edge matches ``get_compression_tip`` semantics: the
        parent ended with ``end_reason='compression'`` and the child started at
        or after the parent ended. Nodes are ordered root-first, then by start
        time. Returns ``[]`` for a falsy ``session_id``.
        """
        if not session_id:
            return []
        lineage = self._session_lineage_root_to_tip(session_id)
        root_id = lineage[0] if lineage else session_id

        ordered: List[str] = [root_id]
        seen = {root_id}
        frontier = [root_id]
        with self._lock:
            # Breadth-first walk forward through compression-continuation edges
            # so linear chains and sibling forks both surface fully. 200 caps a
            # pathological fan-out; real families are tiny.
            for _ in range(200):
                if not frontier:
                    break
                nxt: List[str] = []
            ...
            tip_id = ordered[-1] if ordered else root_id
            nodes: List[Dict[str, Any]] = []
            for idx, sid in enumerate(ordered):
                meta = self._conn.execute(
                    "SELECT title, message_count, started_at, ended_at, end_reason "
                    "FROM sessions WHERE id = ?",
                    (sid,),
                ).fetchone()
                title = ((meta["title"] if meta else "") or "") if meta else ""
                gen_m = re.search(r"#(\d+)\s*$", title)
                generation = int(gen_m.group(1)) if gen_m else idx + 1
                summary_preview = ""
                has_summary = False
                ...
                if srow is not None:
                    raw = srow["content"] if hasattr(srow, "keys") else srow[0]
                    if isinstance(raw, str) and raw:
                        has_summary = True
                        summary_preview = raw[:280]
                nodes.append(
                    {
                        "session_id": sid,
                        ...
                        "message_count": (
                            ...
                            if meta and meta["message_count"] is not None
                            else 0
                        ),
                        "started_at": meta["started_at"] if meta else None,
                        "end_reason": meta["end_reason"] if meta else None,
    ...
    def delete_session(
        ...
        def _do(conn):
            ...
            conn.execute(
                "DELETE FROM llm_api_requests WHERE session_id = ?",
                (session_id,),
            )
    ...
    def delete_sessions(
        ...
        * On-disk transcript / ``request_dump_*`` files are cleaned up
        ...
        def _do(conn):
            ...
            conn.execute(
                f"DELETE FROM llm_api_requests WHERE session_id IN ({existing_placeholders})",
                existing,
            )
            conn.execute(
