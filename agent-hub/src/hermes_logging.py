...
gateway call early in their startup path.  All log files live under ``{data_root}/logs/`` (dev: ``{repo}/.data/logs``;
packaged: ``{install}/data/logs``).  Profile subdirs are **not** used for logs.
...
from hermes_constants import get_config_path, get_hub_logs_dir
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Ignored for log file location (logs always under ``get_hub_logs_dir()``).
    ...
    max_size_mb
        Maximum size of each log file in megabytes before delete-and-reopen.
        Defaults to 10 or the value from config.yaml ``logging.max_size_mb``.
    backup_count
        Ignored（产品策略：超限直接删除，不留 ``.1``/``.2`` 备份）;
    ...
    max_bytes = (max_size_mb or cfg_max_size or 10) * 1024 * 1024
    # backup_count 参数保留兼容；超限策略为删除重开，不写备份文件
    _ = backup_count or cfg_backup
