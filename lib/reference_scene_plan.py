...
def build_reference_driven_scene_plan(
    ...
    *,
    ...
    """Assemble scene_plan: one storyboard scene per video-gen API clip (~10s default)."""
    total = float(
    ...
    for index, (start, end) in enumerate(ranges):
        ...
        scenes.append({
            ...
            "framing": f"9:16 竖屏 {gen_clip:.0f}s 生成单元，快切在 edit 修剪",
            "movement": "dolly_in" if hero else "handheld",
    ...
    return {
        ...
        "metadata": {
            ...
            "timing_note": (
                f"{len(scenes)} video-gen clip(s) at ~{gen_clip:.0f}s each. "
            ...
            "executable_prompt_profile": "ugc_native_executable",
        },
    }
...
def sync_asset_manifest_prompts(
    ...
    for asset in manifest.get("assets") or []:
        ...
        if not scene:
            continue
        prompt = canonical_video_prompt_from_scene(scene)
        if not prompt:
            continue
        tool = str(asset.get("source_tool") or "")
        if tool in {"", "frame_sampler", "image_selector"} or asset.get("type") in {"image", "video"}:
            asset["prompt"] = prompt
            asset["prompt_profile"] = (
                (scene_plan.get("metadata") or {}).get("executable_prompt_profile")
                or "ugc_native_executable"
            )
            video_asset = next(
                (
            ...
            if isinstance(video_asset, dict):
                if video_asset.get("duration"):
                    asset["duration"] = video_asset["duration"]
                if video_asset.get("editorial_duration_seconds") is not None:
                    asset["editorial_duration_seconds"] = video_asset["editorial_duration_seconds"]
            if tool == "frame_sampler":
                asset["generation_summary"] = (
                    "Placeholder keyframe; executable video prompt from scene_plan"
                )
    meta = manifest.setdefault("metadata", {})
    meta["prompt_synced_from"] = "scene_plan.required_assets.executable"
    return manifest
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"({len(scene_plan['scenes'])} scenes, {gen_unit}s gen units)"
    )
    return 0
