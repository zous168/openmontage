...
const STRINGS = {
  ...
  reviewInChat: "在此审阅产物，然后批准或驳回（页面按钮），或在聊天中回复。",
  selfReview: "自检  ",
  ...
  stageRunNext: "运行下一阶段：{stage}",
  stageRunAutoNext: "自动进入 {stage}…",
  stageRunNextConfirm: "将在后台启动无头 agent 执行「{stage}」。继续？",
  stageRunRunning: "无头 agent 运行中（{stage}）",
  stageRunCancel: "取消运行",
  stageRunCancelConfirm: "取消本次运行？已完成的工件会保留。",
  stageRunLog: "查看日志",
  stageRunLogTitle: "运行日志 — {stage}",
  stageRunLogRefresh: "刷新",
  stageRunLogLive: "实时",
  stageRunLogEmpty: "（暂无输出）",
  stageRunApprove: "批准该阶段",
  stageRunApproveConfirm: "批准「{stage}」？将写入正式 checkpoint。",
  stageRunApproved: "已批准 {stage}",
  stageRunRejected: "已驳回 {stage}",
  stageRunReject: "驳回修改",
  stageRunRejectTitle: "驳回 {stage} — 修改意见将反馈给下一次运行的 agent",
  stageRunRejectFeedback: "修改意见（必填，5–4000 字符）",
  stageRunRejectSubmit: "提交驳回",
  stageRunRejectCancel: "取消",
  stageRunBusy: "该项目已有任务在运行，请等待完成",
  stageRunRevisionLimit: "已驳回 {n} 次，达到本流水线上限，请与 agent 在聊天中处理",
  stageRunFailed: "无头 agent 执行失败：{error}",
  stageRunServerLost: "服务重启导致任务状态丢失",
  stageResetFromStart: "从第一步重新跑（{stage}）",
  stageResetFromStartConfirm: "将把流水线重置到第一阶段「{stage}」。当前 checkpoint 会归档到 history/，磁盘上的产物会保留。继续？",
  stageResetDone: "已重置，下一步：{stage}",
  stageResetFailed: "重置失败",
  ...
  editNleLivePreview: "实时预览",
  editNlePreviewDraft: "预览草稿",
  editNleApply: "应用编辑",
  editNleDiscard: "放弃",
  editNleDirty: "有未应用的编辑",
  editNleResize: "拖拽调整时长（≥0.4s）",
  editNleApplyConfirm: "确认将以下剪辑调整写入 edit_decisions？",
  editNleFfmpegLocked: "FFmpeg 拼接模式不支持拖拽编辑（时间轴为累计拼接语义）。",
  ...
  stageMedia: "媒体文件",
  flowView: "流程图",
  flowViewTitle: "以流程图模式查看管线(节点=阶段,可运行/审批)",
  ...
  inChat: "聊天中",
  approveOrChange: " 回复，或在页面批准或要求修改。",

  // 回放
  scrubRun: "回放整条制作",
  replayRun: "▶ 回放制作",
  replayRunHint: "仅回放历史进度，不会重新执行 agent",
  exitReplay: "✕ 实时",
