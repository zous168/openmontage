...
function HyperframesLogo() {
  ...
  return (
    <svg
      ...
      aria-label={t("brand.hyperframes")}
    >
      <defs>
        <linearGradient
          id="hf-g0"
      ...
      <div className="flex items-center gap-1.5">
        ...
        <Tooltip label={capturing ? "Capturing frame…" : "Capture current frame"} side="bottom">
          <a
            ...
            aria-label={capturing ? t("action.capturingFrame") : t("action.captureFrame")}
          >
            {capturing ? (
              <svg
        ...
        <Tooltip label={t("panel.inspector")} side="bottom">
          <button
            type="button"
            onClick={() => {
